-- The first existing-approval acknowledgement was deliberately limited to the
-- resettable QA listing. The live Jenny handoff (10517) uses the same approved,
-- unclaimed capability flow, so allow the two current handoff listings while
-- preserving every token, decision, expiry and claim-state guard.
...
begin
  ...
  if p.listing_no not in (10517, 10518) then
