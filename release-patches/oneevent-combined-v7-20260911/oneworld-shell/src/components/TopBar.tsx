...
function RevealHeader({ config, rightSlot }: { config: AppConfig; rightSlot?: React.ReactNode }) {
  ...
  const [glass, setGlass] = useState(false), [drawer, setDrawer] = useState(false), [notice, setNotice] = useState("");
  ...
  const spanish = lang === "es" || lang === "co"; const away = hidden && !pinned && !drawer;
  ...
  const togglePin = () => { clearTimeout(single.current); lastTap.current = 0; const next = !pinned; setPinned(next); setExpanded(next); showChrome(); setNotice(next ? (spanish ? "Encabezado fijado" : "Header pinned") : (spanish ? "Ocultación automática restaurada" : "Auto-hide restored")); };
  const tap = (detail: number) => { if (detail === 0) { setExpanded(v => !v); setPinned(false); return; } const now = Date.now(); if (now - lastTap.current < 320) { togglePin(); return; } lastTap.current = now; single.current = setTimeout(() => { setExpanded(v => !v); setPinned(false); setNotice(spanish ? "Doble toque para fijar" : "Double-tap to pin"); lastTap.current = 0; }, 320); };
  ...
  return <><div className="ow-reveal-space"/><header data-brand={wordmark.brand} ref={header} data-chrome-lock={drawer ? "true" : undefined} data-expanded={expanded} data-pinned={pinned} aria-hidden={away || undefined} className={`ow-reveal ${expanded ? "is-expanded" : ""} ${glass ? "has-glass" : ""} ${away ? "is-away" : ""}`} onKeyDown={e => { if (e.key === "Escape" && !drawer && !header.current?.querySelector('[role="listbox"]')) { clearTimeout(single.current); setExpanded(false); setPinned(false); logo.current?.focus(); } }}>
    <button ref={logo} className="ow-reveal-logo" aria-expanded={expanded} aria-controls="ow-header-controls" aria-label={expanded ? (spanish ? "Contraer encabezado" : "Collapse header") : (spanish ? "Expandir encabezado" : "Expand header")} title={spanish ? "Doble toque para fijar o liberar" : "Double-tap to pin or unpin"} onClick={e => tap(e.detail)}><img className="ow-reveal-mark" src={src} alt=""/><span className="ow-reveal-wordmark"><Wordmark wordmark={wordmark} h={38}/></span></button>
    <div ref={controls} id="ow-header-controls" className="ow-reveal-controls" aria-hidden={!expanded || undefined}>{rightSlot}<NotificationBell to={config.notificationsPath}/><CountryButton dictionary={config.dictionary}/><button className="ow-tap ow-reveal-menu" aria-label={t("menu")} onClick={() => setDrawer(true)}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M4 7h16M4 12h16M4 17h16"/></svg></button><button className="ow-reveal-pin" aria-pressed={pinned} onClick={togglePin} title={spanish ? "Fijar o liberar encabezado" : "Pin or unpin header"}>{pinned ? (spanish ? "Liberar" : "Unpin") : (spanish ? "Fijar" : "Pin")}</button></div>
  </header><div role="status" className="ow-reveal-notice">{notice}</div><Drawer config={config} open={drawer} onClose={() => setDrawer(false)}/></>;
}
