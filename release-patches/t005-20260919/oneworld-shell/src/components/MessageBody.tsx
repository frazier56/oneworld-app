    ...
    <>
      ...
      {cards.map(c => (
        <button key={c.path} type="button" onClick={() => nav(c.path)}
          ...
          <span className={`grid h-16 w-16 shrink-0 place-items-center overflow-hidden ${
            ...
            {c.photo
              ? <img decoding="async" src={c.photo} alt="" className="h-full w-full object-cover"
      ...
      {media.map(m => (
        <a key={m.href} href={m.href} target="_blank" rel="noreferrer noopener"
          ...
          {m.kind === "image" ? (
            <img decoding="async" src={m.href} alt="" loading="lazy" className="max-h-72 w-full object-cover" />
          ) : m.kind === "video" ? (
