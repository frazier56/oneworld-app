...
/* Her colour vocabulary, scoped to her. See the file header — without it she is invisible on
   every product except OneEvent. */
import "./vaia-panel.css";
type Msg = { role: "user" | "assistant"; content: string };

/* ── HER OPENING LINE HAS TO MATCH THE PRODUCT SHE WAS OPENED FROM ───────────────────────────
   Caught in a screenshot on 14 Sep 2026, the first time this panel was opened outside OneEvent:
   the knowledge base was correctly OneHome's, and the panel still greeted a Colombian property
   manager with *"Ask me anything about your events — hosting tips, ticketing"* and three starter
   chips about selling tickets. The server-side prompt was right and the hard-coded copy around it
   was wrong, which is the same "two correct halves, never introduced" shape as `product` itself.

   Spanish alongside English because OneHome IS a Spanish-language product — a greeting in English
   over a page in Spanish is the half-translated screen Lee counts as a defect. */
type VaiaIntro = { role: { en: string; es: string }; hello: { en: string; es: string }; starters: { en: string[]; es: string[] } };

const VAIA_INTRO: Record<string, VaiaIntro> = {
  oneevent: {
    role: { en: "Your AI event assistant", es: "Su asistente de eventos con IA" },
    hello: {
      en: "Hi! I'm VAIA. Ask me anything about your events — hosting tips, ticketing, your OneScore, and more.",
      es: "¡Hola! Soy VAIA. Pregúnteme lo que quiera sobre sus eventos: cómo organizarlos, boletería, su OneScore y más.",
    },
    starters: {
      en: ["Help me plan an event", "How do I sell more tickets?", "How can I raise my OneScore?"],
      es: ["Ayúdeme a planear un evento", "¿Cómo vendo más boletas?", "¿Cómo subo mi OneScore?"],
    },
  },
  onehome: {
    role: { en: "Your AI property assistant", es: "Su asistente inmobiliario con IA" },
    hello: {
      en: "Hi! I'm VAIA. Ask me anything about listing a place, reviewing a tenant, or what goes in the agreement.",
      es: "¡Hola! Soy VAIA. Pregúnteme lo que quiera sobre publicar un inmueble, revisar a un arrendatario o qué va en el contrato.",
    },
    starters: {
      en: ["Can I ask for a deposit?", "Why can't I accept this tenant?", "Where is my draft listing?"],
      es: ["¿Puedo pedir depósito?", "¿Por qué no puedo aceptar a este arrendatario?", "¿Dónde está mi borrador?"],
    },
  },
  onesocial: {
    role: { en: "Your AI career assistant", es: "Su asistente de carrera con IA" },
    hello: {
      en: "Hi! I'm VAIA. Ask me anything about your profile, your OneScore, or getting hired.",
      es: "¡Hola! Soy VAIA. Pregúnteme lo que quiera sobre su perfil, su OneScore o cómo conseguir trabajo.",
    },
    starters: {
      en: ["How do I improve my profile?", "How can I raise my OneScore?", "How do I get more work?"],
      es: ["¿Cómo mejoro mi perfil?", "¿Cómo subo mi OneScore?", "¿Cómo consigo más trabajo?"],
    },
  },
};

/** OneHome is one app with two sections, so both keys wear the same face. */
const introFor = (product: string): VaiaIntro =>
  VAIA_INTRO[product === "onerental" || product === "onesale" ? "onehome" : product] ?? VAIA_INTRO.onesocial;


const CHAT_URL = "https://wseblryyqxawvbjmylbo.supabase.co/functions/v1/vaia-chat";

/* ── ONE VAIA, EVERY PRODUCT — Lee, 14 September 2026 ────────────────────────────────────────
   *"For VAIA you need to copy the same code that we're using for VAIA in the OneJob app and also
   the OneEvent app. VAIA should work the same. Why would you have different models of VAIA?"*

   This IS that code. It was OneEvent's alone — the voice-first panel with live transcription,
   ElevenLabs speech, markdown answers. Everywhere else the "Tap for insights" pill dispatched
   `ow-vaia-open` into an empty room. I had started a second, plainer panel in the shell; that was
   the wrong instinct and it is deleted. One panel, one backend, one identity; only the KNOWLEDGE
   changes, and the knowledge is server-side — `product` picks the system prompt in `vaia-chat`.

   TWO CHANGES MADE IT PORTABLE, and nothing else was touched:
     1. `product` is a prop instead of the hard-coded string "oneevent".
     2. `useAuth()` — OneEvent's own hook, which THROWS outside `<AuthProvider>` — became the
        shell's `useOneId()`. The panel only ever wanted the signed-in user's id, and that is One
        ID's job anyway, so this both frees the component and puts it on the right source. */
export default function AskVaiaChat({ open, onClose, preferVoice = false, product = "oneevent" }: { open: boolean; onClose: () => void; preferVoice?: boolean; product?: string }) {
  const { avatar: vaiaAvatar, name: vaiaName } = useAiAssistant();
  const { userId } = useOneId();
  const navigate = useNavigate();
  const { lang } = useI18n();
  const intro = introFor(product);
  /* OneEvent's `useI18n` already collapses the shell's seven languages to "en" | "es" (see
     `oneevent/lib/i18n.tsx`), so Colombian Spanish arrives here as "es". */
  const es = lang === "es";
    ...
    try {
      const resp = await fetch(CHAT_URL, {
        ...
        body: JSON.stringify({
          ...
          userId,
          /* ⚠️ `product` WAS NEVER SENT, so this panel has been getting the OneSocial prompt.
             `vaia-chat` reads `product` from the body to choose its system prompt and has carried
             a detailed OneEvent one for weeks — applicants, payout stages, check-in, the Rolodex.
             Nobody ever passed the field, so VAIA answered event hosts as a career assistant,
             talking about endorsements and job applications. Two correct halves of one feature
      ...
      if (!resp.ok) {
        ...
        throw new Error(errData.error || (es ? "No se pudo conectar" : "Failed to connect"));
      }
  ...
  }, [input, isLoading, messages, userId, product, mode, speakLang]);
  ...
  useEffect(() => {
    ...
    if (mode === "voice" && messages.length === 0) {
      const greet = es
        ? "Hola, soy VAIA. Hábleme — toque el micrófono y pregúnteme lo que quiera. ¿Prefiere escribir? Toque el teclado."
        : "Hey, I'm VAIA. Talk to me — tap the mic and ask me anything. Prefer to type? Tap the keyboard.";
      setSpeaking(true);
  ...
  return createPortal(
    <>
      ...
      <div className="ow-vaia-scope fixed inset-0 z-[998] bg-black/60 backdrop-blur-sm" onClick={() => { window.dispatchEvent(new Event("vaia-stop-all-audio")); onClose(); }} />
      ...
      <motion.div
        ...
        className="ow-vaia-scope fixed inset-0 z-[999] flex items-end justify-center p-2 pointer-events-none sm:items-center sm:p-4"
      ...
      >
            ...
            >
              ...
              >
                <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-primary/40 shadow-lg"
                  ...
                  <img decoding="async" src={vaiaAvatar} alt="VAIA" className="w-full h-full object-cover" />
                </div>
                <motion.div
              ...
              >
                {es ? intro.role.es : intro.role.en}
              </motion.p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            >
              <img decoding="async" src={vaiaAvatar} alt={vaiaName} className="w-full h-full object-cover" />
            </div>
            <div>
              <p className="text-sm font-bold text-foreground leading-tight">Ask {vaiaName}</p>
              <p className="text-[10px] text-muted-foreground leading-tight">{es ? intro.role.es : intro.role.en}</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button
              ...
              title={mode === "voice" ? (es ? "Cambiar a escribir" : "Switch to typing") : (es ? "Cambiar a voz" : "Switch to voice")}
              type="button"
            >
          ...
          {messages.length === 0 && (
            <div className="text-center py-6">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed px-2">
                {es ? intro.hello.es : intro.hello.en}
              </p>
              <div className="flex flex-col gap-2 items-center mt-5">
                {(es ? intro.starters.es : intro.starters.en).map(q => (
                  <button
                    key={q}
                    onClick={() => setInput(q)}
            ...
            >
              <div className="w-full rounded-2xl border border-border/60 bg-muted/40 p-3">
                <div className="flex items-center justify-between gap-3 pb-2">
                  ...
                  <span className="text-[11px] text-primary">{es ? "Revise antes de enviar" : "Review before send"}</span>
                </div>

                <div ref={transcriptScrollRef} className="max-h-[24svh] min-h-[120px] overflow-y-auto rounded-xl border border-border/50 bg-background/50 px-3 py-2.5">
                  {voice.listening ? (
                    voice.transcript ? (
                    ...
                    <textarea
                      ...
                      placeholder={es ? "Revise y edite su transcripción antes de usarla." : "Review and edit your transcript before using it."}
                    />
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 w-full">
                    ...
                    {voice.listening && voice.visualizerBars.length > 0 ? (
                      ...
                      <div className="flex items-center justify-center gap-2 w-full">
                        <div className="w-2 h-2 rounded-full bg-destructive animate-pulse" />
                          <span className="text-[11px] text-muted-foreground">{es ? "Escuchando… hable ahora" : "Listening… speak now"}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {voice.listening ? (
                  <button
                    ...
                    title={es ? "Dejar de escuchar" : "Stop listening"}
                    type="button"
                  >
                    <Check size={16} />
                  </button>
                  ...
                  <button
                    ...
                    className="flex-shrink-0 w-9 h-9 flex items-center justify-center rounded-full border-2 border-primary bg-primary text-primary-foreground transition-colors hover:opacity-90"
                    title={es ? "Usar transcripción" : "Use transcript"}
                    type="button"
                  >
                    <Send size={16} />
                  </button>
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 border-t border-border/60 flex-shrink-0">
          {mode === "voice" && !isVoiceReviewOpen ? (
            <div className="flex w-full flex-col items-center gap-1.5 py-1">
              <button
                ...
                style={{ width: 54, height: 54, background: "linear-gradient(135deg, hsl(var(--primary)), hsl(var(--primary) / 0.72))", boxShadow: speaking ? "0 0 0 4px hsl(var(--primary) / 0.18), 0 0 24px hsl(var(--primary) / 0.35)" : "0 0 18px hsl(var(--primary) / 0.25)" }}
                title={es ? "Toque para hablar" : "Tap to talk"}
                type="button"
              >
                <Mic size={22} className="text-white" />
              </button>
              <span className="text-[11px] text-muted-foreground text-center px-2">
                {speaking ? (es ? "VAIA está hablando…" : "VAIA is speaking…")
                  : isLoading ? (es ? "Pensando…" : "Thinking…")
                  : (es ? "Toque para hablar — o el teclado para escribir" : "Tap to talk — or the keyboard icon to type")}
              </span>
            </div>
          ) : !isVoiceReviewOpen ? (
            <input
              ...
              placeholder={voiceHintActive ? (es ? "Toque el micrófono para hablar…" : "Tap mic to speak…") : (es ? "Pregúntele lo que quiera a VAIA…" : "Ask VAIA anything…")}
          ...
          {mode === "text" && !isVoiceReviewOpen && !input.trim() && (
            <button
              ...
              title={es ? "Entrada por voz" : "Voice input"}
              type="button"
            >
              <Mic size={18} />
            </button>
