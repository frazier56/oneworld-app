...
import {
  ...
  I18nProvider,
  registerVaiaPanel,
  PRODUCT_PATH, safeAuthReturn, useOneId, type AppKey,
  ...
  return (
    <Router basename={BASENAME}>
      ...
      <Routes>
        ...
        {/* 22 Sep — Lee: *"the admin panel… green background. It should be a blue background… not
            tied to OneJob."* Same config, One World hue: admin belongs to the whole ecosystem. */}
        <Route path="/admin" element={
          <AppShell config={ONE_WORLD_CONFIG} skipTerms skipSetup>
            <AdminScreen />
          </AppShell>
        } />
...
/* ── THE FRONT DOOR (Lee, 23 Sep 2026) ────────────────────────────────────────────────────────
   *"You can't take everybody straight to the feed because we don't know who they are."*
   Signed in → the feed. Signed out → the One World splash with Create account and Sign in, which
   is also exactly the frame the opener film (v5.2) dissolves into — the film plays over it for
   every signed-out visitor (see Opener.tsx) and hands over to it with nothing to navigate.
   While the session is still being read, nothing is drawn rather than the wrong screen. */
function FrontDoor() {
  /* One AppShell around both: `useOneId` only answers inside the provider AppShell mounts. */
  return <AppShell config={ONE_WORLD_CONFIG} bare skipTerms><FrontDoorScreen /></AppShell>;
}
function FrontDoorScreen() {
  const { loading, userId } = useOneId();
  if (loading) return <ScreenFallback />;
  return userId ? <Suspense fallback={<ScreenFallback />}><WorldFeed /></Suspense> : <OneWorldEntry />;
}
