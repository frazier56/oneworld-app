...
import { ScreenHeading, FeedMedia, FeedVideoPlayer, IconPlay, coverOf, feedLead, feedSlides, orderedPhotos, W } from "@oneworld/shell";
...
interface EventData {
  ...
  /* MEDIA (OneEvent 30, 20 Sep 2026) — the OneHome shape; see lib/listingMedia.ts in the shell. */
  photos?: string[] | null;
  videos?: string[] | null;
...
export default function EventDetail() {
  ...
  /* The full-screen player with sound, opened by tapping a video hero or a video tile. */
  ...
  /* ── MEDIA (OneEvent 30) ──────────────────────────────────────────────────────────────────
     `photos` / `videos` / `cover_photo` / `feed_preview` are the OneHome shape. An event saved
     before the columns existed has only `cover_image_url`, which becomes its one photo. The
     STILL cover (share cards, calendar, WhatsApp) is `coverOf`; what the HERO shows is the feed
     lead — a photo, or a video playing muted and looping, poster = the cover, tap for sound. */
  const media = {
    photos: (event.photos && event.photos.length) ? event.photos : (event.cover_image_url ? [event.cover_image_url] : []),
    videos: event.videos ?? [],
    cover_photo: event.cover_photo ?? null,
    feed_preview: event.feed_preview ?? null,
  };
  const heroCover = coverOf(media) ?? event.cover_image_url;
  const heroLead = feedLead(media);
  const heroSlides = feedSlides(media);
  /* Every photo beyond the cover, then every video, then the older attachment media. */
  const galleryItems: { url: string; type: "image" | "video" }[] = [
    ...orderedPhotos(media).slice(1).map(url => ({ url, type: "image" as const })),
    ...media.videos.map(url => ({ url, type: "video" as const })),
    ...mediaItems.filter(m => !media.photos.includes(m.url) && !media.videos.includes(m.url)),
  ];
  ...
  const allPhotos: { url: string; type: "image" | "video" }[] = [
    ...(heroCover ? [{ url: heroCover, type: "image" as const }] : []),
    ...galleryItems,
  ];
  const mediaOffset = heroCover ? 1 : 0;
    ...
    <div>
      ...
      <div>
        ...
        {(() => {
          ...
          return (
            <div className={`relative rounded-2xl overflow-hidden mb-6 bg-black/40 mx-auto ${(heroCover || heroLead) ? "cursor-pointer" : ""}`} style={{ aspectRatio: heroAspect, maxHeight: "min(72vh, 560px)", maxWidth: 560 }}
              onClick={() => { if (heroLead?.kind === "video") setPlaying(0); else if (heroCover) setLightboxIndex(0); }}
              title={heroLead?.kind === "video" ? W(lang, "Tap to watch with sound", "Toque para oír") : heroCover ? W(lang, "Tap to view full size", "Toque para ver en grande") : undefined}>
              {heroLead?.kind === "video" ? (
                <>
                  {/* THE COVER CAN BE A VIDEO (Lee, 20 Sep 2026): the host's feed lead plays here,
                      muted and looping while on screen, over the still cover as its poster. Tap
                      opens the full-screen player with sound. The still cover is what share
                      cards and the calendar carry — a video cannot go there. */}
                  {heroCover && (
                    <img decoding="async" src={heroCover} alt="" aria-hidden="true" className="absolute inset-0 w-full h-full object-cover scale-110 blur-2xl opacity-50" />
                  )}
                  <FeedMedia media={heroLead} eager poster={heroCover} className="relative z-[1] w-full h-full object-cover" />
                  <span className="pointer-events-none absolute inset-0 z-[2] grid place-items-center">
                    {/* The same play mark as the Discover card — one glyph for "this is a video". */}
                ...
                <>
                  ...
                  <img decoding="async"                     src={heroCover}
                    alt={event.title}
...
>
        ...
        <div className="space-y-6 pb-16">
            ...
            {/* Full-screen player with sound — over every slide (lead, videos, photos), thumb
                swipe, Back only: this is the event's own page, so there is nowhere to "view". */}
            {playing !== null && heroSlides.length> 0 && (
              <FeedVideoPlayer slides={heroSlides} start={playing} poster={heroCover} title={event.title}
                lang={lang} onClose={() => setPlaying(null)} />
            )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {galleryItems.map((item, i) => (
                    <div
                      ...
                      onClick={() => {
                        /* A video tile opens the player with sound at that video; a photo tile the lightbox. */
                        const at = item.type === "video" ? heroSlides.findIndex(s => s.url === item.url) : -1;
                        if (at>= 0) setPlaying(at); else setLightboxIndex(i + mediaOffset);
                      }}
>
                      {item.type === "video" ? (
                        ...
                        <img decoding="async" src={item.url} alt={`Media ${i + 1}`} className="w-full h-full object-cover" />
                      )}
                    </div>
                  ))}
                </div>
...
>
              ...
              <div className="flex items-center gap-3 pr-14">
                {host?.photo_url ? (
                  <img decoding="async"                     src={host.photo_url}
                    alt={host.full_name}
