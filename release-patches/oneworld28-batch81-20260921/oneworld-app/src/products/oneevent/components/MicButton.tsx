...
export default function MicButton({ onTranscript }: { onTranscript: (text: string) => void }) {
  ...
  return (
    <span className="relative inline-flex flex-col items-center">
      <button type="button" onClick={toggle} aria-label="Voice input" className={`relative grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl border transition ${rec ? "border-red-400 bg-red-500/10" : "ow-edge"}`}>
        {rec ? (
