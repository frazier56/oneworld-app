...
interface Registration {
  ...
  /** Tickets on this registration. One QR, `quantity` admissions. */
...
interface Props {
  ...
  /** EV-11: after every ticket on a registration has been admitted, a further scan is a
     re-entry (welcome back) when this is on, and a refusal when it is off. */
  allowReentry?: boolean;
}

type ScanResult =
  | { type: "success"; name: string; email: string; admitted: number; of: number }
  | { type: "already"; name: string; email: string; time: string; of: number }
  | { type: "reentry"; name: string; email: string; of: number }
...
export default function EventCheckInTab({ eventId, registrations, onRefresh, pendingApps, requiresApproval, allowReentry = false }: Props) {
  ...
  const showResult = useCallback((r: ScanResult) => {
    ...
    const kind = r.type === "success" || r.type === "fd" ? "fresh"
      : r.type === "error" ? "bad" : "already";   // re-entry sounds like "already": two beeps, amber
    feedback(kind);
  }, [feedback]);
  ...
  /* ── THE TICKET RULE (EV-11, Lee 20 Sep 2026) ─────────────────────────────────────────────
     One registration carries `quantity` tickets and ONE QR code. Each ticket admits one person,
     once: the code may be scanned `quantity` times and each scan counts one admission. After
     that, a further scan is a RE-ENTRY if the host allowed it (amber, "welcome back", nothing
     counted) or a refusal if not (amber, "all N admitted").

     ⚠️ TWO DOORS, ONE TICKET. The admission is written as a CONDITIONAL update — "count is still
     what I read" — so two scanners cannot both admit the last ticket. If the write touches no
     row, somebody else got there first; re-read and answer from the fresh count. */
  const doCheckIn = useCallback(async (reg: Registration): Promise<ScanResult> => {
    const name = reg.profile?.full_name ?? "Attendee";
    const email = reg.profile?.email ?? "";
    if (isPendingApproval(reg)) return { type: "pending", name };
    if (reg.status === "cancelled") return { type: "error", message: m("This ticket was cancelled") };
    const of = Math.max(1, Number(reg.quantity ?? 1));
    const admitted = Math.max(0, Number(reg.checked_in_count ?? 0));
    if (admitted >= of) {
      if (allowReentry) return { type: "reentry", name, email, of };
      return { type: "already", name, email, time: reg.checked_in_at ?? "", of };
    }
    const { data, error } = await supabase
      .from("event_registrations")
      .update({ status: "checked-in", checked_in_at: new Date().toISOString(), checked_in_count: admitted + 1 })
      .eq("id", reg.id)
      .eq("checked_in_count", admitted)
      .select("id, checked_in_count, checked_in_at");
    if (error) return { type: "error", message: m("Check-in failed") };
    onRefresh();
    if (!data || data.length === 0) {
      /* Lost the race: another scanner admitted on this code a moment ago. Read the truth. */
      const { data: fresh } = await supabase.from("event_registrations").select("checked_in_count, checked_in_at").eq("id", reg.id).single();
      const n = Math.max(0, Number(fresh?.checked_in_count ?? admitted + 1));
      if (n >= of) return allowReentry ? { type: "reentry", name, email, of } : { type: "already", name, email, time: fresh?.checked_in_at ?? "", of };
      return { type: "success", name, email, admitted: n, of };
    }
    return { type: "success", name, email, admitted: admitted + 1, of };
  }, [isPendingApproval, onRefresh, m, allowReentry]);
  ...
  const resultCard = scanResult ? (
    <div className={cn(
      ...
      scanResult.type === "already" || scanResult.type === "fd-already" || scanResult.type === "reentry" ? "border-amber-500/40" :
      ...
      {scanResult.type === "success" && (
        <>
          ...
          {scanResult.of > 1 && (
            <p className="text-xs text-muted-foreground mt-0.5">{scanResult.admitted} / {scanResult.of} {m("admitted on this ticket")}</p>
          )}
        </>
      )}
      {scanResult.type === "reentry" && (
        <>
          <AlertCircle className="w-9 h-9 mx-auto mb-1.5 text-amber-600 dark:text-amber-400" />
          <p className="text-sm font-semibold text-foreground">{scanResult.name}</p>
          {scanResult.email && <p className="text-xs text-muted-foreground break-all">{scanResult.email}</p>}
          <p className="text-xs font-semibold text-amber-700 dark:text-amber-300 mt-1">{m("Re-entry — welcome back")} · {new Date().toLocaleTimeString()}</p>
          {scanResult.of > 1 && (
            <p className="text-xs text-muted-foreground mt-0.5">{scanResult.of} / {scanResult.of} {m("admitted on this ticket")}</p>
          )}
        </>
      )}
      {scanResult.type === "already" && (
        <>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{scanResult.of > 1 ? `${scanResult.of} / ${scanResult.of} ${m("admitted on this ticket")} · ` : ""}{m("No re-entry for this event")}</p>
        </>
      )}
      {scanResult.type === "fd" && (
        <>
...
>
                  <div className="w-10 h-10 rounded-full bg-muted flex items-center justify-center overflow-hidden shrink-0">
                    {reg.profile?.photo_url ? (
                      <img decoding="async" src={reg.profile.photo_url} alt="" className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="flex items-center gap-2 text-xs text-muted-foreground">
                      <span>TICKET-{reg.id.slice(0, 3).toUpperCase()}</span>
                      {/* A multi-ticket registration shows how many of its admissions are used. On the
                          second line, so the name keeps its room beside the time. */}
                      {Math.max(1, Number(reg.quantity ?? 1)) > 1 && (
                        <span className="inline-flex items-center rounded-full bg-primary/10 border border-primary/30 px-2 py-0.5 text-[10px] font-semibold tabular-nums text-primary">
                          {Math.max(0, Number(reg.checked_in_count ?? 0))} / {Math.max(1, Number(reg.quantity ?? 1))}
                        </span>
                      )}
                    </p>
                  </div>
