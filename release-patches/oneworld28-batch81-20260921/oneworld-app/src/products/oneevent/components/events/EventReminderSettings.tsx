...
export default function EventReminderSettings({ eventId, hostId, event }: EventReminderSettingsProps) {
  ...
  const actualLocation = event?.location?.trim() || "";
  const previewLocation = actualLocation || t("reminders.location_tbd", "Location TBD");
  const directionsUrl = actualLocation
    ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(actualLocation)}`
    : null;
...
>
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent closeLabel={t("reminders.close", "Close")} className="max-h-[90svh] w-[calc(100%-2rem)] max-w-md overflow-y-auto rounded-3xl p-0">
          <DialogHeader className="border-b border-border px-5 pb-4 pr-12 pt-5 text-left">
            ...
            <DialogDescription>
              {actualLocation
                ? t("reminders.preview_description", "Each attendee sees their own name. The links open the map and event page.")
                : t("reminders.preview_description_no_location", "Each attendee sees their own name. The event link opens the event page.")}
            </DialogDescription>
          </DialogHeader>
