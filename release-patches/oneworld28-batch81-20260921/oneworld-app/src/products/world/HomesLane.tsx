...
import {
  supabase, useAsync, useAsyncResult, productHref, feedLead, coverOf, mediaRowOf,
...
export function useHomesLane(lang: string, query: string, enabled: boolean) {
  ...
  const listingsQ = useAsyncResult(async () => {
  ...
  /* `listings` keeps its old shape for every reader below; the failure rides alongside it. */
  const listings = listingsQ.data;

  /* ⚠️ THE SALE HALF MUST REPORT FAILURE TOO, OR THE LANE HANGS. — 22 Sep 2026
     When these reads started throwing on a PostgREST error, this one stayed on `useAsync`,
     which swallows a rejection into `undefined`. `ready` below ANDs both halves, so a failed
     sale read — with rentals perfectly healthy — left the lane shimmering forever while
     `failed` stayed false, which is precisely the defect the error handling was added to kill.
     And `retry` only re-ran the rental half: pressing Try again cleared the error screen and
     left a permanent shimmer behind it. Both halves are failure-aware now and `retry` below
     re-runs both. */
  const salesQ = useAsyncResult(async () => {
    const { data, error } = await supabase
  ...
  /* ⚠️ THIS ONE MUST NOT THROW. It is decoration — the agent behind a listing — and the lane's `ready`
       does not wait on it. Throwing here would turn a missing name into a lane that never
       loads, which is a far worse trade than a card with no face on it. The primary read is
  ...
  const cards: LaneCard[] = useMemo(() => {
    const rent: { at: number; card: LaneCard }[] = kind === "sale" ? [] : shown.map(p => ({
      ...
      card: {
        ...
        /* ── R17 NOTE 12 · THE BUTTON MUST SAY WHAT IT DOES ─────────────────────────────
           Lee: *"'Request a booking' is the wrong button and goes to the wrong place. It opens
           the listing, not a booking. The button should say 'View listing' and open the
           listing. Booking happens from the listing page, after they have seen the details."*

           The destination was always right — one swipe card cannot carry enough of a home for
           anybody to commit money to it, and every other surface in OneHome sends people to the
           listing first. It was the LABEL that lied, and a button whose words promise one thing
           and whose tap does another is the kind of defect that costs trust rather than time.
           The real booking button lives on the listing page, where the dates, the price
           breakdown and the house rules are on screen beside it. */
        cta: W(lang, "View listing", "Ver el anuncio"),
    ...
    const sale: { at: number; card: LaneCard }[] = kind === "rent" ? [] : salesShown.map(p => ({
      ...
      card: {
        ...
        sub: locationShort(p, lang) || null,
        /* Same rule as the rental half above (R17 note 12): this opens the listing, so it says
           so. "Request a showing" is a real action and it lives on the sale listing's own page,
           below the photographs and the history — not on a card the buyer has looked at for
           two seconds. */
        cta: W(lang, "View listing", "Ver el anuncio"),
        href: productHref("onesale", `/s/${p.id}`),
        engagement: {
  ...
  /* ⚠️ `view` AND `layout` WERE IN HERE AND THEY SENT PEOPLE BACK TO SLIDE ONE. — 22 Sep 2026
     `resetKey` means "the reader NARROWED the set, so the old scroll position is meaningless".
     Opening the map flips `view` to "map" and closing it flips it back, so a reader on slide 14
     who looked at the map and pressed Back returned to slide 1 — the exact failure the R17 note
     in WorldFeed says was fixed. Switching between the list, grid and big layouts did the same.
     Neither of those changes WHICH items are in the lane; only the search, the filters and the
     sort do. Those are what stay. */
  const resetKey = `${query}|${filters.homeKind}|${activeCount(filters)}|${filters.neighbourhood}|${sort}`;

  return {
    ready: listings !== undefined && sales !== undefined,
    /* Either half failing is the lane failing, and one press re-runs both. */
    /* ⚠️ `failed` MEANS "NOTHING TO SHOW", NOT "SOMETHING WENT WRONG". — 22 Sep 2026
       ...
       scroll, not a wipe. */
    failed: (!!listingsQ.error || !!salesQ.error) && cards.length === 0,
    pageFailed: (!!listingsQ.error || !!salesQ.error) && cards.length > 0,
    retry: () => { listingsQ.retry(); salesQ.retry(); },
