...
import {
  ...
  useChromeHidden, showChrome, hideChrome, NavIcon, type AppKey, useOneId,
} from "@oneworld/shell";
...
/* The round glass control in the header and down the right rail — the prototype's `.ico`.
   ...
   see you can press. */
/* `ow-tap` paints the shell's invisible 44 x 44 hit box behind a 38px circle: the picture
   stays the size the header needs, the thumb target reaches the floor tokens.css calls
   "44 x 44, EVERYWHERE". The bell beside it already had 44 via NotificationBell; the menu
   did not, so two controls that look identical had different hit areas. — 22 Sep 2026 */
const ICO = "ow-tap grid h-[38px] w-[38px] place-items-center rounded-full border border-white/30 bg-black/40 text-white backdrop-blur-md";
/**
...
/**
 * THE FOOTER PILL, CARVED OPEN AROUND THE MARK — R30, Lee's option B1.
 * ============================================================================================
 * Lee: *"carve out the glass pill in the middle... like a crescent... so the logo stands by
 * itself and it's not overlaid on top of the pill, because you cut the pill in half."*
 *
 * So the bar is two pieces of glass with the mark floating free between them, and this draws
 * them. Three things had to be true and each one cost a round to find:
 *
 * ── 1 · THE GLASS CANNOT LIVE ON THE BAR ────────────────────────────────────────────────────
 * A mask clips an element's whole subtree, so masking the bar itself erased the five controls
 * and the mark along with the glass. The glass is therefore a layer UNDERNEATH them, and the
 * bar keeps nothing but its layout.
 *
 * ── 2 · THE CARVE IS NOT CENTRED ON THE BOX. IT IS CENTRED ON THE MARK. ─────────────────────
 * Lee, on the symmetric version: *"the right side is the right distance, it's the left side
 * that's not, because you're not factoring in how the logo is. That whole black side on the
 * left, that's really the other part of the circle. You're just looking at the actual pixels."*
 *
 * He is right, and it is measurable. `mark-oneworld.png` is 420 x 512; its ink spans x=6..405,
 * but its CENTRE OF MASS is at x=233 — twenty-three pixels right of the image's own centre.
 * The lit crescent and the speckles are all on the right; the left is the unlit sweep, dark and
 * low-contrast, which reads as empty while still being part of the mark. A carve centred on the
 * box therefore sits right of where the eye puts the middle and pinches the left.
 *
 * The fix keeps the right edge exactly where he approved it and opens only the left:
 * each side gets its OWN circle, same radius, the left one slid out by `OPEN`.
 * ⚠️ NOT one bigger circle — that was tried. Growing the radius to open the left also flattens
 * the arc (at R=56 through a 64px bar it is nearly straight) and it stops reading as a carve at
 * all. Same radius, different centre.
 *
 * ── 3 · THE CORNER FILLET HAS TO BE TINY ───────────────────────────────────────────────────
 * Lee: *"it's very sharp, like you took a razor blade. Round it off just a little."* The fillet
 * is tangent to the OUTSIDE of the carve, so every pixel of corner radius pushes the junction
 * sideways and widens the mouth: at 4px the opening grew 8px a side and the cut washed out into
 * two rounded rectangles leaning apart. 2.5px knocks the point off and leaves the carve.
 *
 * The path is rebuilt on resize because the bar's width comes from `inset-x-4` on a frame whose
 * width varies — stretching one SVG to fit would stretch the carve with it.
 */
const CARVE_R = 46;      // the bite, matching the radius Lee approved on the right
const CARVE_OPEN = 6;    // B1 — how much further the LEFT side opens. Right side never moves.
const CARVE_FILLET = 2.5;

function carvePath(W: number, H: number): string {
  const cap = H / 2, cy = H / 2, R = CARVE_R, r = CARVE_FILLET;
  const cxR = W / 2, cxL = W / 2 - CARVE_OPEN;
  /* The fillet centre sits on y=r (tangent to the top edge) at distance R+r from the carve's
     centre (tangent to the carve); T is where the two curves actually meet. */
  const end = (side: -1 | 1, cx: number) => {
    const dxF = Math.sqrt((R + r) * (R + r) - (r - cy) * (r - cy));
    const fx = cx + side * dxF;
    const len = R + r, ux = (fx - cx) / len, uy = (r - cy) / len;
    return { fx, tx: cx + R * ux, ty: cy + R * uy };
  };
  const L = end(-1, cxL), Rt = end(1, cxR);
  const left = [
    `M 0 ${cap}`, `A ${cap} ${cap} 0 0 1 ${cap} 0`, `L ${L.fx} 0`,
    `A ${r} ${r} 0 0 1 ${L.tx} ${L.ty}`,
    `A ${R} ${R} 0 0 0 ${L.tx} ${H - L.ty}`,
    `A ${r} ${r} 0 0 1 ${L.fx} ${H}`,
    `L ${cap} ${H}`, `A ${cap} ${cap} 0 0 1 0 ${cap}`, "Z",
  ].join(" ");
  const right = [
    `M ${W} ${cap}`, `A ${cap} ${cap} 0 0 0 ${W - cap} 0`, `L ${Rt.fx} 0`,
    `A ${r} ${r} 0 0 0 ${Rt.tx} ${Rt.ty}`,
    `A ${R} ${R} 0 0 1 ${Rt.tx} ${H - Rt.ty}`,
    `A ${r} ${r} 0 0 0 ${Rt.fx} ${H}`,
    `L ${W - cap} ${H}`, `A ${cap} ${cap} 0 0 0 ${W} ${cap}`, "Z",
  ].join(" ");
  return `${left} ${right}`;
}

/**
 * OFF-SCREEN CHROME MUST LEAVE THE KEYBOARD ORDER, NOT JUST THE SCREEN READER. — 22 Sep 2026
 * ============================================================================================
 * The feed had `aria-hidden` on three bars that stay in the layout when hidden: the header, the
 * search pane and the footer. `aria-hidden` on something still focusable is the textbook
 * accessibility violation — assistive tech is told the control is not there while Tab still
 * lands on it, so a keyboard reader tabs through ten-plus invisible controls and hears nothing.
 *
 * `inert` is the attribute that actually does both: it removes the subtree from the focus order
 * AND from the accessibility tree. `BottomTabs` in the shell already solved this the same way;
 * the feed's own three copies of the same bar had not adopted it.
 */
function useInert(ref: React.RefObject<HTMLElement>, off: boolean) {
  useLayoutEffect(() => {
...
function CarvedGlass() {
  /* ⚠️ IT MEASURES ITSELF, NOT THE BAR — and that is not a style choice, it is the only thing
     that works. React attaches refs bottom-up during commit, so a child's layout effect runs
     BEFORE its parent's ref is attached: reading `barRef.current` from in here found `null` on
     the first pass, nothing ever rendered, and no error was thrown to say so. This layer is
     `absolute inset-0` of the bar, so its own box IS the bar's box. */
  ...
  useLayoutEffect(() => {
    ...
    const read = () => {
      ...
      setBox(prev => (prev && Math.abs(prev.w - r.width) < 0.5 && Math.abs(prev.h - r.height) < 0.5
  ...
  const layers = (() => {
    if (!box || box.w < 120) return null;
    const d = carvePath(box.w, box.h);
    const svg = (inner: string) =>
      `url("data:image/svg+xml;utf8,${encodeURIComponent(
        `<svg xmlns="http://www.w3.org/2000/svg" width="${box.w}" height="${box.h}" viewBox="0 0 ${box.w} ${box.h}">${inner}</svg>`)}")`;
    const shape = svg(`<path d="${d}" fill="#fff"/>`);
    /* The hairline follows the carve too, or the glass reads as torn where it was cut. */
    const edge = svg(`<path d="${d}" fill="none" stroke="rgba(255,255,255,.18)" stroke-width="1.1"/>`);
    return (
      <>
        <div data-ow="footer-glass"
          ...
          style={{ background: "rgba(255,255,255,.12)",
                   ...
                   maskImage: shape, WebkitMaskImage: shape,
                   maskSize: "100% 100%", WebkitMaskSize: "100% 100%",
                   maskRepeat: "no-repeat", WebkitMaskRepeat: "no-repeat" }} />
        <div className="absolute inset-0"
          style={{ backgroundImage: edge, backgroundSize: "100% 100%", backgroundRepeat: "no-repeat" }} />
      </>
    );
  })();

  return <div ref={self} aria-hidden className="pointer-events-none absolute inset-0">{layers}</div>;
}

export default function WorldFeed() {
  /* Kept out of the focus order whenever they are off-screen — see `useInert`. */
  const headerRef = useRef<HTMLElement>(null);
  const paneRef = useRef<HTMLDivElement>(null);
  const footerRef = useRef<HTMLElement>(null);
  const { lang } = useI18n();
  const { userId: signedInId } = useOneId();
  ...
  const byLane: Record<LaneKey, {
    ...
    /* Set by every lane once its primary read can fail out loud — see useAsyncResult.
       `failed` is "this lane has nothing to show"; `pageFailed` is "the NEXT page did not
       arrive, but what you are reading is fine". They are different screens. */
    failed?: boolean; pageFailed?: boolean; retry?: () => void;
  ...
  /* ⚠️ THIS RAN BEFORE THE SLIDES EXISTED, AND BACK LANDED ON A BLACK SCREEN. — 22 Sep 2026
     On mount every lane's `ready` is still false, so the column holds one shimmer box, its
     `scrollHeight` is one screen, and `scrollTo({top: startIdx * clientHeight})` clamped to 0.
     But `idxs` WAS seeded to N, so the windowing renders only N-1..N+1 as real slides and
     paints slide 0 as a plain black box — which is exactly what the reader saw after tapping
     View listing on slide 14 and pressing Back. The one journey this whole address-driven
     design exists to get right was the one it got wrong.

     It now waits for the lane that is actually being restored into. `restored` latches so a
     later page-in never yanks a reader who has since scrolled. */
  const restored = useRef(false);
  const startLaneReady = byLane[LANES[startLane].key].ready;
  useEffect(() => {
    if (restored.current) return;
    const t = track.current;
    if (t) t.scrollTo({ left: startLane * t.clientWidth, behavior: "auto" });
    if (startIdx === 0) { restored.current = true; return; }
    if (!startLaneReady) return;               // the slides are not on the page yet
    const col = columns.current[startLane];
    if (!col || col.scrollHeight <= col.clientHeight) return;
    col.scrollTo({ top: startIdx * col.clientHeight, behavior: "auto" });
  ...
  /* ⚠️ ONE PER LANE, NOT ONE FOR THE SCREEN. — 22 Sep 2026
     This was a single ref shared by all four columns. Scroll Homes to slide 10, swipe across to
     Events — a fresh column at scrollTop 0 — and the first swipe up computes its delta against
     Homes' position: a large NEGATIVE number, so the chrome came BACK on a gesture that should
     have hidden it. It corrected itself on the next scroll event, which is why it read as the
     header being flaky rather than as a bug with a cause. Same shape as `askedAt` beside it. */
  const lastTop = useRef<Record<string, number>>({});
  const askedAt = useRef<Record<string, number>>({});
  /* When a page fails we let the next scroll retry it, but not on every scroll event — a lane
     whose next page is genuinely unreachable would otherwise hammer the database for as long as
     the reader keeps moving. One attempt every four seconds. */
  const lastPageRetry = useRef<Record<string, number>>({});
  const onColumnScroll = (key: LaneKey) => (e: React.UIEvent<HTMLDivElement>) => {
    ...
    const wantsMore = !d0.atEnd && n >= d0.cards.length - 3;
    if (wantsMore && d0.pageFailed) {
      /* ⚠️ WITHOUT THIS, ONE FAILED PAGE STOPS PAGING FOREVER. `take` has already moved, so
         `more()` would change nothing, and the guard above remembers this card count as
         already-asked — so the lane would simply never load again, silently, for the rest of
         the session. `retry()` re-runs the request that failed, at the size it already asked
         for. Throttled, because a lane that genuinely cannot reach the next page must not turn
         a scrolling thumb into a request loop. */
      const now = Date.now();
      if (now - (lastPageRetry.current[key] ?? 0) > 4000) {
        lastPageRetry.current[key] = now;
        d0.retry?.();
      }
    } else if (wantsMore && askedAt.current[key] !== d0.cards.length) {
      askedAt.current[key] = d0.cards.length;
      d0.more();
    }

    /* ⚠️ THE FIRST SCROLL EVENT IN A LANE ONLY ESTABLISHES WHERE THE LANE WAS.
       The old single ref started at 0, which was right for the first column and wrong for the
       other three — see the note beside `lastTop`. Defaulting a new lane to the element's
       CURRENT position instead would make its first event a zero delta and lose it, and a
       deep-link restore (`?i=14`, which scrolls programmatically and fires an event) would
       otherwise read as a huge downward swipe and hide the chrome on arrival. So the first
       event records the origin and does nothing; from the second on it behaves as before. */
    const prev = lastTop.current[key];
    if (prev === undefined) { lastTop.current[key] = el.scrollTop; return; }
    const d = el.scrollTop - prev;
    /* 6px of deadband: momentum scrolling emits jittering deltas and a bar that chatters is the
       flicker Lee has reported three times. ⚠️ The origin moves only when we ACT on it — update
       it on every event and a slow drag of five pixels a frame never accumulates past the
       deadband, so the chrome would never move at all. */
    if (Math.abs(d) < 6) return;
    lastTop.current[key] = el.scrollTop;
  ...
  /* ⚠️ THE SHEET PUTS THE CHROME BACK THE WAY IT FOUND IT. — 22 Sep 2026
     This used to be `if (comments) hideChrome(); else showChrome();`, which had two faults, one
     of them visible on every single mount:

       1. `else showChrome()` fires on MOUNT, because the effect runs once with `comments`
          already false. Arriving at the feed therefore forced the header and tab bar back on,
          overriding whatever the rest of the app had just decided.
       2. Closing the sheet forced them on too — even for somebody who had deliberately scrolled
          the chrome away before tapping a comment. They swipe it off, read a comment, close, and
          the bars are back. The sheet is not entitled to undo a gesture it did not cause.

     So: remember what the chrome was doing when the sheet opened, and restore exactly that. A
     ref rather than a dep, because reading the value must not re-run this effect and re-hide the
     chrome under the open sheet. `null` means "the sheet has not been opened", which is what
     keeps the mount pass from touching anything. */
  const chromeWasHidden = useRef<boolean | null>(null);
  const chromeHiddenNow = useRef(false);
  chromeHiddenNow.current = chromeHidden;
  useEffect(() => {
    if (comments) { chromeWasHidden.current = chromeHiddenNow.current; hideChrome(); return; }
    if (chromeWasHidden.current === null) return;      // never opened — leave the chrome alone
    if (!chromeWasHidden.current) showChrome();        // it was on before; put it back
    chromeWasHidden.current = null;
  }, [comments]);
  /* And if the feed unmounts with the sheet open — the phone's back button, a deep link, a
     sign-out — the chrome must not stay hidden for the screen that comes next. */
  useEffect(() => () => { if (chromeWasHidden.current !== null) showChrome(); }, []);

  /* ⚠️ THE RAIL IS UNMOUNTED WHILE THE SHEET IS OPEN, SO THE SHEET CANNOT PUT FOCUS BACK.
     `railVisible` is `&& !comments`, so tapping Comment removes the button that was focused in
     the same render the sheet mounts — `document.activeElement` is <body> by the time the
     sheet's own effect runs, and on close a keyboard user was left at the top of the document.
     The rail mounts a NEW button when the sheet closes, so the element has to be found again
     rather than remembered. One frame after the close, because the rail has to be back first.
     — 22 Sep 2026 */
  const sheetWasOpen = useRef(false);
  useEffect(() => {
    if (comments) { sheetWasOpen.current = true; return; }
    if (!sheetWasOpen.current) return;
    sheetWasOpen.current = false;
    const id = window.requestAnimationFrame(() => {
      const btn = document.querySelector<HTMLElement>('[data-ow="rail"] [data-ow="comments"]')
        ?? document.querySelector<HTMLElement>('[data-ow="comments"]');
  ...
  /* ⚠️ THE CARD IS A DEPENDENCY, AND IT USED TO BE MISSING. — 22 Sep 2026
     This effect read the card out of `byLane` but listed only `[lane.key, idx, comments]`. On
     first paint a lane's cards array is EMPTY — the reads have not landed yet — so `current` is
     undefined, the effect returns early, and when the data arrives nothing in that list has
     changed, so it never runs again. The first card you ever see carried no comment count, on
     every lane, every session; swiping to card 2 and back was the only way to get one, which is
     precisely why this looked intermittent rather than broken.

     Depending on the card's identity rather than the object fixes it without re-firing on every
     unrelated render of the lane: the query is keyed by exactly the two values it sends. */
  const countKey = current?.engagement ? `${current.engagement.source}:${current.id}` : "";
  /* The card the badge is currently showing a number FOR. Re-counting after the sheet closes is
     the point — you just posted — but blanking the badge while you open it is not: `comments` is
     a dependency, so an unconditional reset made the number vanish every time the sheet opened.
     Only a different card clears it. */
  const countFor = useRef("");
  /* ⚠️ THE RE-COUNT BELONGS TO THE CLOSE, NOT TO THE OPEN. `comments` as a raw dependency fired
     the effect twice per visit to the sheet — once on the way in, which is pure cost, and once
     on the way out, which is the useful half because you may have just posted. This ticks only
     on the way out, so opening a sheet costs nothing and closing one refreshes the badge. */
  const [closedTick, setClosedTick] = useState(0);
  const wasOpen = useRef(false);
  useEffect(() => {
    if (comments) { wasOpen.current = true; return; }
    if (!wasOpen.current) return;
    wasOpen.current = false;
    setClosedTick(n => n + 1);
  }, [comments]);
  useEffect(() => {
    if (!countKey) { countFor.current = ""; setCommentCount(undefined); return; }
    const [source, ...rest] = countKey.split(":");
    const id = rest.join(":");
    let live = true;
    if (countFor.current !== countKey) { countFor.current = countKey; setCommentCount(undefined); }
    supabase.from("media_comments")
      .select("id", { count: "exact", head: true })
      .eq("media_item_id", id).eq("media_source", source)
      /* `.catch` because everything else in this batch grew one: a head-count that rejects was
         an unhandled rejection in the console and a badge that stayed blank with no reason. */
      .then(({ count }) => { if (live) setCommentCount(count ?? 0); },
            () => { if (live) setCommentCount(undefined); });
    return () => { live = false; };
  }, [countKey, closedTick]);
  ...
  /* The three bars that stay in the layout when they leave the screen. */
  useInert(headerRef, chromeHidden || pane);
  useInert(paneRef, !pane);
  useInert(footerRef, chromeHidden);
  ...
  return (
    ...
    <div className="fixed inset-0 z-0 flex justify-center bg-black">
      ...
      <header ref={headerRef} aria-hidden={chromeHidden || pane || undefined}
        ...
        <div className="pointer-events-auto flex items-center justify-between gap-2.5 px-4 pb-1.5 pt-[11px]">
          ...
          <div className="flex items-center gap-2">
            {/* 22 Sep — Lee: *"my notification bell… it's just not there… can't even touch it."*
                The bell draws nothing when nobody is signed in, which left this frosted circle
                empty. No member, no circle. */}
            {signedInId && (
        ...
        {LANES.map((l, i) => {
          ...
          return (
            <div key={l.key} ref={el => { columns.current[i] = el; }}
              ...
              {data.failed ? (
                /* ⚠️ THE LANE USED TO SHIMMER FOREVER, OR CLAIM THE WORLD WAS EMPTY. — 22 Sep 2026
                   A failed read resolved to an empty array, so the reader was told "Be the first
                   one here" when the truth was that the request broke; and when the whole hook
                   rejected, `ready` never turned true and the shimmer ran until the tab was
                   closed. A person can act on "that did not load, try again". They cannot act on
                   either of the other two, and one of them is a lie about their own city. */
                <div className="grid h-full w-full place-items-center px-8 text-center">
                  <div>
                    <p className="text-[16px] font-extrabold">
                      {W(lang, "That didn't load.", "Eso no cargó.")}
                    </p>
                    <p className="mt-1.5 text-[13.5px] leading-snug opacity-65">
                      {W(lang, `We couldn't reach ${l.en} just now. Your connection or ours.`,
                               `No pudimos conectar con ${l.es} ahora mismo. Tu conexión o la nuestra.`)}
                    </p>
                    <button type="button" onClick={() => data.retry?.()}
                      className="ow-tap mt-4 rounded-full border border-white/30 bg-white/10 px-5 py-2.5
                        text-[14px] font-extrabold backdrop-blur-md">
      ...
      {railVisible && (
        /* ⚠️ THE RAIL DOES NOT HIDE. It belongs to the item, like the information block, and it
           ...
           hide on a swipe up. */
        <div data-ow="rail"
          /* THE GAP IS 16 PX. Lee picked it off a photographed set — six spacings from 20 px
             down to 6 px, shot from one build with only this number changed — after 36 and then
             20 both read as too tall to him. Six controls at a fixed 48 px is 288 px that cannot
             move, so the gap is the only dial and it carries 80 px of the 368. For the record:
             36 → 468 px, 55% of an 844-tall screen; 20 → 388, 46%; 16 → 368, 44%, top edge 42%
             of the way down.
             ⚠️ This number is duplicated in ListingEngagement's vertical stack, because like,
             comment and share are three children of THAT element, not of this one. Both or
             neither, or the column spaces itself unevenly. */
          /* ── NO CIRCLES (R26) ───────────────────────────────────────────────────────────
          ...
          className="absolute bottom-[calc(120px+env(safe-area-inset-bottom))] right-4 z-20 flex flex-col items-center gap-4">
          ...
          {/* The same 48 the shell's three use, so the column is one rhythm (R27). */}
          <button type="button" className="ow-tap grid h-[48px] w-[48px] place-items-center text-white"
          ...
          <button type="button" className="ow-tap grid h-[48px] w-[48px] place-items-center text-white" onClick={toggleSound} aria-pressed={sound}
      ...
      {/* ── THE SEARCH PANE — the lane's OWN controls, on frosted glass over the video. ─────
          ...
          half a control. */}
      <div ref={paneRef} aria-hidden={!pane}
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-ink/[0.08] pt-3 dark:border-white/10">
          ...
          {/* ⚠️ A PAGE THAT DID NOT ARRIVE HAS TO SAY SO SOMEWHERE. — 22 Sep 2026
              When the next page fails, the cards already on screen stay — the reader is not
              thrown out of the lane for a page they had not reached. But silence there would be
              a feed that quietly stops growing, which is the same failure as a control that
              half works. One line, and the next swipe near the end retries by itself. */}
          {byLane[lane.key].pageFailed && (
            <p role="status" aria-live="polite" className="text-[12.5px] font-bold text-amber-500">
              {W(lang, "Couldn't load more — retrying", "No se pudo cargar más — reintentando")}
            </p>
          )}
        </div>
      ...
      <nav ref={footerRef} aria-hidden={chromeHidden || undefined}
        ...
        className={`absolute inset-x-4 bottom-[max(22px,env(safe-area-inset-bottom))] z-40 flex h-16 items-center
          justify-around transition-all duration-300
          ${chromeHidden ? "translate-y-[150%] opacity-0" : "translate-y-0 opacity-100"}`}>
        {/* THE GLASS IS A LAYER, NOT A BACKGROUND ON THE BAR — see `CarvedGlass`. */}
        <CarvedGlass />
        ...
        <button type="button"
          ...
          className="ow-tap grid h-[42px] w-[42px] place-items-center rounded-full text-white opacity-75">
          {/* ── THE FEED GLYPH, NOT A HOUSE (R30) ──────────────────────────────────────
              Lee: *"I just don't like that home icon. It doesn't seem to be as modern as the
              others."* The cause was structural, not taste: the house was TWO paths — a roof
              line and a three-sided box — meeting at a hard corner with the eaves hanging past
              the walls, while every other glyph in this bar is one continuous stroke with round
              joins. It was the only thing down here with a visible seam.

              Two stacked cards. It is also the more honest picture: this button does not go to
              a home screen, it returns you to the TOP OF THE LANE you are standing in, with the
              map closed and the list layout restored. */}
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
            strokeLinecap="round" strokeLinejoin="round" aria-hidden>
            <rect x="3.2" y="4.2" width="17.6" height="8.2" rx="2.4" /><rect x="3.2" y="15" width="17.6" height="5.4" rx="2.2" />
          </svg>
        </button>
        <button type="button" onClick={() => setPane(p => !p)} aria-pressed={pane}
          aria-label={W(lang, "Search and filters", "Búsqueda y filtros")}
          className="ow-tap grid h-[42px] w-[42px] place-items-center rounded-full text-white opacity-60">
