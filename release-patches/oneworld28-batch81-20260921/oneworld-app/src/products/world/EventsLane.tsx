...
import { supabase, useAsync, useAsyncResult, productHref, feedLead, coverOf, mediaRowOf, W, type MapPin } from "@oneworld/shell";
import {
...
export function useEventsLane(lang: string, query: string, enabled: boolean) {
  ...
  const rowsQ = useAsyncResult(async () => {
  ...
  /* `rows` keeps its old shape for every reader below; the failure rides alongside it. */
  const rows = rowsQ.data;

  /* ⚠️ THIS ONE MUST NOT THROW. It is decoration — the host behind an event — and the lane's `ready`
  ...
  const cards: LaneCard[] = useMemo(() => shown.map(e => {
    ...
    return {
      ...
      /* R18 note 1 — the same five controls as every other lane. `event` is a new value in the
         `media_source` / `item_type` discriminators, which are open text by design. */
      engagement: { source: "event" as const, savesAs: "event" as const },
    };
  }), [shown, hosts, lang]);
  ...
  /* R22 — the feed's map is the shell's real Google map now, so these are `MapPin`s.
     ⚠️ `exact: false` ON EVERY EVENT, AND DELIBERATELY. `geo_precision` is a rental column;
     `events` has no equivalent, so there is no way to know whether a host meant this address to
     be public. An approximate circle is the safe direction — a pin on a guess is not. When
     events gain the column, this becomes the same expression the rental half uses. */
  const pins: MapPin[] = useMemo(() => shown
    .filter(e => typeof (e as any).latitude === "number" && typeof (e as any).longitude === "number")
    .map(e => ({
      id: e.id, lat: (e as any).latitude as number, lng: (e as any).longitude as number,
      exact: false,
      priceLabel: ticketLine(e, lang) ?? "",
      title: e.title,
      photo: coverOf(e as any) ?? e.cover_image_url,
      facts: whenAndSpots(e, lang),
      href: productHref("oneevent", `/e/${e.id}`),
    })), [shown, lang]);
  ...
  return {
    ...
    /* ⚠️ `failed` MEANS "NOTHING TO SHOW", NOT "SOMETHING WENT WRONG". — 22 Sep 2026
       ...
       scroll, not a wipe. */
    failed: !!rowsQ.error && cards.length === 0,
    pageFailed: !!rowsQ.error && cards.length > 0,
    retry: rowsQ.retry,
    total: rows?.length ?? 0,
