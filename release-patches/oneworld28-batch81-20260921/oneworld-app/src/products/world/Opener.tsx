...
import { W, useI18n, sharedSessionStorage, SHARED_STORAGE_KEY } from "@oneworld/shell";
 ...
 * Film on a fresh open → fade into the LIVE signed-out home screen with no cut and no white
...
/** Served flat out of `public/` by PUB30 — in the batch-62 manifest, 13,226,549 bytes. */
const FILM = "opener/one-world-opener-v5-2.mp4";
/** Versioned, so recutting the film to v6 shows it to everybody once more by construction. */
const KEY = "ow.opener.v52.seen";
...
function signedInHint(): boolean {
  try { return !!sharedSessionStorage.getItem(SHARED_STORAGE_KEY); } catch { return false; }
}
...
function decideOnce(replayAlways: boolean, pathname: string): boolean {
  ...
  try {
    ...
    /* Lee, 22 Sep 2026: signed OUT, the film plays EVERY time (we don't know who you are, so you
       get the front door). Signed IN, straight to the feed — no film. The sandbox (replayAlways)
       keeps playing it for review. The session check is synchronous on purpose: the shared
       sign-in cookie is readable before the SDK finishes restoring the session. */
    else if (!replayAlways && signedInHint()) ok = false;
...
export default function Opener({ replayAlways }: {
  ...
  const { lang } = useI18n();
  /* useNavigate is no longer used here — the film hands over to the root screen underneath. */
  ...
  /* ── THE SCREEN THAT IS WAITING UNDERNEATH ────────────────────────────────────────────────
     Nothing. The film hands over to whatever the root already renders, signed in or out, and as
     of 22 September 2026 the root IS the feed on production as well as the sandbox.

     ⚠️ THIS USED TO NAVIGATE TO `/yourworld` AND THAT IS NOW WRONG. Lee, after signing in to
     production and landing on Your World again: *"everyone who signs in lands on the new feed
     page… You should be able to get to [Your World] through the hamburger icon. But that's it."*
     The navigate was one of three ways a person still reached that screen without asking for it;
     the other two were the root route and the sign-in return trip, both closed in the same batch.

     (The history, so it is not re-litigated: this was `/join` first, which was worse — the film
     ended on the Create-your-One-ID FORM, *"as if the person had already chosen to sign up"*, and
     that form has no back control. Then `/yourworld`, to match the splash v5.2 dissolves into.
     Now nowhere, because the screen underneath is the right one on its own.)

     ⚠️ KNOWN COST, FOR ONE WORLD REIMAGINED: v5.2's last second was cut to dissolve into the
     SPLASH, so the final frame no longer matches what is revealed behind it. The film is right
     and the destination is right; it is the crossfade between them that is now mismatched. That
     is a re-cut, not a code change, and it belongs to the lane that owns the film. */
  useEffect(() => { /* nothing to do — the root screen underneath is the destination */ }, [armed]);
  ...
  return (
    ...
    >
      ...
      <video
        ...
        src={`${(import.meta as any).env?.BASE_URL ?? "/"}${FILM}`}
        playsInline
        preload="auto"
        autoPlay
        onEnded={finish}
      ...
      <button
        ...
        data-ow="opener-skip"
        onClick={finish}
        className="ow-tap absolute z-[1] inline-flex h-[34px] items-center rounded-full border border-white/25 bg-black/35 px-4 text-[13px] font-extrabold text-white backdrop-blur-md"
        style={{
