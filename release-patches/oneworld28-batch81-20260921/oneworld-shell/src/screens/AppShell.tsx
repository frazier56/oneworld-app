...
import { getVaiaPanel } from "../lib/vaiaPanel";
...
import { WORLD_FEED_HOME } from "../lib/authReturn";
...
function BackToTop() {
  ...
  return (
    <button ref={btn} onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
      ...
      /* ⚠️ `bottom-24` WAS A GUESSED NUMBER AND IT LANDED ON TOP OF A BUTTON. 96px cleared the
         footer tabs and nothing else, so the moment a listing grew a pinned contact bar this
         arrow sat over its left-hand half. Every bar that pins itself to the bottom now publishes
         its own measured height, so this stacks above all of them and above none that are absent. */
      className={`ow-tap fixed bottom-[calc(var(--ow-tabs,84px)+var(--ow-bookbar,0px)+var(--ow-contactbar,0px)+12px)] left-3 z-40 grid h-11 w-11 place-items-center rounded-full shadow-lg
...
function Chrome({
  ...
  /* ⚠️ `useEffect` HERE WAS ONE FRAME OF THE NEW SCREEN AT THE OLD SCROLL OFFSET. — 16 Sep 2026
     A passive effect runs AFTER paint. So landing on a tab from 2000px down painted the incoming
     screen scrolled into its middle, then snapped it to the top on the next frame. That snap is
     one of the things Lee has been calling flicker: *"if you start clicking around going from the
     feed to the centre footer button, it's flickering bad."* `useLayoutEffect` runs BEFORE the
     browser paints, so the first frame the person sees is already at the top. */
  ...
  /* Read at render, not stored in state: the app registers at module scope, before the first
     frame, so there is nothing to subscribe to and a subscription would only add a re-render. */
  const VaiaPanel = getVaiaPanel();

  return (
    <div className="min-h-screen">
      <TopBar config={config} rightSlot={rightSlot} />
      {/* ── VAIA'S PANEL IS SUPPLIED BY THE APP AND MOUNTED HERE ───────────────────────────
          The shell owns the pill and the `ow-vaia-open` event; the app owns the one real panel
          (it is built on lucide-react and framer-motion, both banned in this package) and hands
          it in through `registerVaiaPanel`. See `lib/vaiaPanel.ts` for why it is a registration
          rather than an import, and why it must be rendered HERE, inside these providers, rather
          than at the app root — she calls `useOneId()`, and above `<OneIdProvider>` that throws.

          Rendered once per AppShell, above every screen of every product, so the pill finally
          does something everywhere instead of on OneEvent alone. */}
      {VaiaPanel && <VaiaPanel />}
      <main className={`mx-auto px-4 pt-4 ${loc.pathname.startsWith("/admin") ? "max-w-7xl" : "max-w-lg"} ${hasTabs ? "pb-28" : "pb-10"}`}>
        ...
        <VaiaProvider value={vaiaApi}>
          ...
          {/* ⚠️ THIS USED TO SAY `key={loc.pathname}` AND THAT WAS THE WORST FLICKER IN THE APP.
              — 16 Sep 2026. Changing a `key` does not re-render a subtree, it DESTROYS it and
              builds a new one. So every single tab tap threw away the entire content column,
              including the <Suspense> boundary inside it — and a brand-new Suspense boundary can
              never take React's "keep the old content on screen" path, so it paints its fallback
              instantly, every time. Blank, then content. Tap around quickly and that is a strobe,
              which is exactly what Lee reported: the flicker is WORST when navigating, not when
              scrolling.

              The key was there for one narrow reason — clearing a caught error when you navigate
              away from a screen that threw. That is now done properly, by the boundary itself,
              from a prop it compares (`resetKey`). Same guarantee, without demolishing the tree
              on every tap. */}
...
function MaybeSetup({ skip, children }: { skip?: boolean; children: React.ReactNode }) {
  ...
  if (requiredProfile === "required") {
    const next = `${location.pathname}${location.search}${location.hash}` || WORLD_FEED_HOME;
