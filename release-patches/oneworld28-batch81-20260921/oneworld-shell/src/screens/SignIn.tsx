...
import { safeAuthReturn, signUpHref, WORLD_FEED_HOME, toBrowserPath } from "../lib/authReturn";
...
/* The default landing place is the world feed, not Your World (Lee, 21 Sep 2026 — the
   note is on `WORLD_FEED_HOME` in lib/authReturn). A `?next=` passed by a caller still
   wins: a guest who came from an event checkout goes back to the checkout. */
export default function SignIn({ next: requestedNext = WORLD_FEED_HOME }: { next?: string }) {
  ...
  /* ⚠️ "UNAVAILABLE" IS NOT "FAILED". — 22 Sep 2026
     This used to be `captchaStatus !== "ok"`, which disabled Send-code AND the password
     Sign-in button permanently whenever Turnstile's script was blocked — a privacy
     extension, a corporate proxy, a country that cannot reach Cloudflare. That member
     could not sign in by email at all, and the only advice on screen was "reload the page",
     which does nothing about a blocked script.

     `Turnstile.tsx` says this in its own header: *"the screen stays usable: the server
     decides what to do about a missing token. Silently trapping a real person on a sign-up
     screen because their ad blocker ate a script is a worse outcome than the bot we were
     trying to stop."* The component reported `unavailable` correctly; this line ignored it.

     So the button waits only while the challenge is still RECOVERABLE — loading, expired, or
     errored, all three of which render Turnstile's own "Restart security check" button right
     above this one, so the person always has the next move on screen. `unavailable` is the one
     state with no way back: the script never loaded, restarting cannot load it, and there is
     nothing the person can do. That case goes through with a null token and the server rules
     on it — which is where that decision belonged.

     ⚠️ The first version of this fix relaxed the gate to `=== "loading"` alone, which let an
     EXPIRED or ERRORED challenge post `captchaToken: null` while the widget was sitting right
     there offering a restart. That is a hole, not a rescue: it throws away a token the person
     could have had for one tap, and once the project-wide server switch goes on it turns into a
     failed request with no explanation. Blocked-and-unrecoverable passes; blocked-and-fixable
     waits. — 22 Sep 2026 */
  const captchaBusy = captchaStatus === "loading" || captchaStatus === "expired" || captchaStatus === "error";
  ...
  const redirectTo = `${window.location.origin}${toBrowserPath(next)}`;
  ...
  const signInWithPassword = async () => {
    ...
    /* ⚠️ `unavailable` IS NOT `failed` — the same rule as `captchaBusy` above, which this line
       used to contradict. `SignUp.tsx` already carried the escape; this screen did not, so a
       member whose browser blocks Cloudflare got a live Sign-in button that answered "Complete
       the browser check first" every single time, next to a widget that says the check could not
       start. A control that always errors is worse than a disabled one. When the challenge could
       not load at all, the request goes up with a null token and the server decides. Every other
       state is either `ok` (a token exists) or already held by `captchaBusy`. — 22 Sep 2026 */
    if (!captcha && captchaStatus !== "unavailable") {
      setErr(W(lang, "Complete the browser check first.", "Completa primero la verificación del navegador."));
      return;
    }
    setErr(null); setNoAccount(false); setBusy("email");
    const { error } = await supabase.auth.signInWithPassword({
      /* `?? undefined`, not `?? ""`. The `unavailable` escape above means this can now legally
         be null, and GoTrue's option is `string | undefined`: undefined OMITS the field, which
         is the honest "no token" the server gets to rule on. An empty string would be a token
         claim of zero length and fails verification differently. */
      email: email.trim(), password, options: { captchaToken: captcha ?? undefined },
    });
    setBusy(null);
    remintCaptcha();
    if (error) {
    ...
    window.location.replace(toBrowserPath(next));
  };
  ...
  const verify = async (explicit?: string) => {
    ...
    /* ⚠️ THIS MUST NOT BE ABLE TO STOP THE REDIRECT. `recordOneIdNotice` throws on RPC
       failure by design, and an uncaught throw here skipped `window.location.replace` while
       the token was ALREADY in `spentTokens` and `busy` was already cleared — so a member
       whose code was correct, and who was genuinely signed in, sat on a screen that never
       moved, showed no error, and ignored a second press of Verify. `SignUp` has always
       wrapped the identical call; sign-in did not.

       The notice is evidence we keep for GDPR Art. 13 and Colombia's Ley 1581. Losing one
       row of that evidence is a problem for us. Stranding a signed-in member on a dead screen
       is a problem for them, and theirs wins. */
    try {
      await recordOneIdNotice(lang, { ecosystem_account: true });
    } catch { /* logged server-side; never block a completed sign-in */ }
    window.location.replace(toBrowserPath(next));
  };
