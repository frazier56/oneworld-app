...
import { safeAuthReturn, WORLD_FEED_HOME, toBrowserPath } from "../lib/authReturn";
...
export default function SignUp({ next: requestedNext = WORLD_FEED_HOME }: { next?: string }) {
  ...
  const firstIncompleteStep = async (): Promise<Step | null> => {
    ...
    if (!status.is_required || status.is_complete) { window.location.replace(toBrowserPath(next)); return null; }
  ...
  const startOverAtSignIn = async () => {
    ...
    try {
      ...
      window.location.replace(toBrowserPath("/signin?next=" + encodeURIComponent(next)));
    } catch (error) {
      setErr(say(error));
    } finally {
      setBusy(false);
  ...
  const oauth = async (provider: "google" | "apple") => {
    ...
    const { error } = await oauthSignIn(provider, `${window.location.origin}${toBrowserPath(next)}`);
  ...
  const verifyEmailCode = async (explicit?: string) => {
    ...
    try {
      ...
      else window.location.assign(toBrowserPath(next));
    ...
    <form ref={formRef} onSubmit={submitStep} noValidate className="ow-onboarding mx-auto w-full max-w-sm space-y-4">
      ...
      {step === "email" && (
        <>
          ...
          {captchaStatus !== "ok" && (
            <p className="text-center text-[12px] font-medium opacity-60">
              {captchaStatus === "loading"
                ? W(lang, "Checking your browser before we send a code.",
                          "Comprobando tu navegador antes de enviar el código.")
                : W(lang, "We could not run the bot check here. You can still continue.",
                          "No pudimos ejecutar la verificación aquí. Puedes continuar igual.")}
            </p>
          )}
          ...
          {/* ⚠️ A BLOCKED CAPTCHA MUST NOT BLOCK THE ACCOUNT. — 22 Sep 2026
              `disabled={!emailOk || !captcha}` meant that anyone whose browser could not load
              Turnstile — a privacy extension, a corporate proxy, a country that cannot reach
              Cloudflare — could not create an account at all, and the message above told them
              to reload the page, which does nothing about a blocked script. `Turnstile.tsx`
              reports `unavailable` for exactly this and says in its own header that the screen
              must stay usable and the SERVER decides about a missing token. It does. */}
          <Primary onClick={sendEmailCode}
            disabled={!emailOk || (!captcha && captchaStatus !== "unavailable")}>{t("continue")}</Primary>

          {/* ── THE WAY OUT OF THE FIRST STEP (R18, 21 September 2026) ─────────────────────
              `BackLink` renders nothing here — this is the first step of the wizard, so there
              is nothing behind it to go back to — which left somebody who already HAS an
              account with no control on the screen but Continue. That became visible the moment
              the opener film started handing people to this form; it is a real dead end for
              every returning member who lands here by any route.

              A text link, not a second button: the page has one action and this is not it.
              `next` rides along exactly as it does on the resume path above, so a ticket buyer
              who came here from a checkout still returns to the ticket after signing in. */}
          <p className="pt-1 text-center text-[13.5px] leading-relaxed opacity-75">
            {W(lang, "Already have an account? ", "¿Ya tienes cuenta? ")}
            <a href={"/signin?next=" + encodeURIComponent(next)} className="font-bold underline">
              {W(lang, "Sign in", "Iniciar sesión")}
            </a>
          </p>
        </>
      )}
