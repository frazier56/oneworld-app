 ...
 */
/* PUB30, 23 Sep 2026: the feed now IS the front door on production, so this is "/" — the change
   this comment predicted. It was "/sandbox", and SignIn/SignUp finish with a FULL page load
   (`window.location.replace`), so on production a signed-in member was reloaded onto /sandbox/ —
   which GitHub Pages serves from the SANDBOX build, not production's. Invisible only while the two
   builds happened to match. */
export const WORLD_FEED_HOME = "/";

/** A router path → the address to hand to `window.location`. Router paths are relative to the
 *  mount; a full page load is not. Inside the sandbox mount (`/sandbox/…`) the prefix is put back,
 *  so a sign-in on the sandbox stays on the sandbox and one on production stays on production. */
export function toBrowserPath(next: string): string {
  try {
    const here = window.location.pathname;
    const inSandbox = here === "/sandbox" || here.startsWith("/sandbox/");
    if (inSandbox && !(next === "/sandbox" || next.startsWith("/sandbox/"))) return "/sandbox" + (next.startsWith("/") ? next : "/" + next);
  } catch { /* no window: leave it */ }
  return next;
}
