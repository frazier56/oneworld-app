...
export default function Drawer({
  config, open, onClose, extraRows,
}: {
  config: AppConfig; open: boolean; onClose: () => void;
  /**
   * ── ROWS A SCREEN ADDS TO ITS OWN MENU (World Feed 30, 20 September 2026) ────────────────
   * Rendered directly under the product's own rows, above the Apps and Services groups.
   * Every existing caller omits it and gets exactly today's drawer, row for row.
   *
   * It exists because the world feed has menu items that belong to the FEED and to no product:
   * "Show the intro again" now, "Your lanes" next. Putting those in a product's config would
   * give them to a product that does not have a feed; building a second drawer beside this one
   * would be a second drawer.
   */
  ...
  const authNext = safeAuthReturn(location.pathname === "/" ? WORLD_FEED_HOME : location.pathname + location.search + location.hash);
  ...
  /* ── IT LOOKED LIKE A MENU AND BEHAVED LIKE NOTHING. — 22 Sep 2026 ─────────────────────
     A portal overlay with a click scrim, rendered as a bare `<nav>`: no dialog role, no
     `aria-modal`, no Escape handler anywhere in the file, no initial focus, no trap, no
     restore. So Escape did not close it; a keyboard reader opened the menu and the next Tab
     left it for the page underneath; a screen-reader user was never told a menu had opened;
     and Sign out — the one control people most need to find in here — was reachable only by
     accident. Every other overlay in the family does this properly. */
  const panel = useRef<HTMLElement>(null);
  const restoreTo = useRef<HTMLElement | null>(null);
  /* ⚠️ `onClose` IS NOT A DEPENDENCY, IT IS A REF. — 22 Sep 2026
     All four call sites pass an inline arrow — `onClose={() => setOpen(false)}` — so `onClose`
     is a brand-new function on every render of the parent. With it in the dep list the effect
     below tore down and re-ran on every one of those renders, and its teardown calls
     `restoreTo.current?.focus()`. The result was focus being yanked back to the hamburger
     button mid-interaction, repeatedly, while the menu stayed open: unusable with a keyboard
     and bewildering with a screen reader. The handler has to be current without the effect
     being allowed to notice that it changed. */
  const onCloseRef = useRef(onClose);
  onCloseRef.current = onClose;
  useEffect(() => {
    /* ⚠️ THIS HOOK LIVES ABOVE `if (!open) return null` AND GUARDS ON `open` ITSELF.
       It was written below that early return first, which is a conditional hook — React threw
       #310, "rendered more hooks than during the previous render", and the whole feed went
       white the moment somebody opened the menu. Caught by the harness; it would have been a
       blank screen in Lee's hand otherwise. */
    if (!open) return;
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70]">
      ...
      <nav ref={panel} tabIndex={-1} role="dialog" aria-modal="true"
        aria-label={W(lang, "Menu", "Menú")}
        className="absolute right-3 top-3 flex max-h-[calc(100vh-24px)] outline-none w-[218px] max-w-[76vw] flex-col overflow-y-auto overscroll-contain rounded-3xl border border-white/40 p-3 shadow-2xl dark:border-white/10"
        style={{
 ...
 className="ow-edge ow-tap grid h-7 w-7 place-items-center rounded-full border text-base "
