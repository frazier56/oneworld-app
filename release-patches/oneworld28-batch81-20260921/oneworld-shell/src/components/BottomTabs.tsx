 ...
 */
/* Declared in `lib/authReturn` now, because the auth screens land on it too — see the note
   there. Re-exported so every existing importer of this module keeps working. */
export { WORLD_FEED_HOME } from "../lib/authReturn";
...
export default function BottomTabs({
  ...
  /**
   * ── SLOT ONE GOES TO THE MASTER FEED (R23) ───────────────────────────────────────────────
   * Lee: *"on the far left footer, whatever icon it is, that takes you back to the main feed.
   * We are effectively replacing all of the feeds from all of the apps with our master feed.
   * You don't need these individual feeds any more — we've replaced it."*
   *
   * So the leftmost slot is Home in the only sense that is left: home is the master feed. Each
   * product's own `tabs[0]` is still declared and still routable by URL; it just is not the
   * thing this bar points at any more.
   *
   * ⚠️ R22 PUT THE MARK IN THE CENTRE AND THAT WAS WRONG. The centre is the raised slot and it
   * carries the one action a product exists for — list a place, create an event, start a job.
   * Taking it for navigation cost the member the button they use most. The mark belongs in the
   * middle of the WORLD FEED's own footer, which is a different bar, and it is there.
   */
  const WorldSlot = () => (
    <NavLink key="ow-world" to={WORLD_FEED_HOME} end
      aria-label={lang === "en" ? "World feed" : "Feed del mundo"}
      className={cls}>
      {/* Two stacked cards, not a house (R30, Lee's pick of five). This slot has always gone
          to the WORLD FEED and has always been labelled "World feed" — so a house was the wrong
          picture for it in every product, and the feed glyph is the right one in every product.
          Matched to the world feed's own footer; the two must never disagree. */}
