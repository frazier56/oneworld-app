 ...
 */
export default function FeedMedia({ media, className, eager = false, poster, sound = false }: {
  media: FeedPreview; className: string; eager?: boolean; poster?: string | null;
  /**
   * ── SOUND, OFF BY DEFAULT (World Feed 30, 20 September 2026) ─────────────────────────────
   * Every existing caller is a card in a scrolling feed, where a clip that speaks unasked is
   * the reason people scroll past. So the default is exactly today's behaviour — muted — and
   * omitting the prop changes nothing anywhere.
   *
   * The world feed is the one surface with a sound control on it (Lee: off by default, and
   * once it is on it stays on). It passes the member's choice down; nothing else does.
   */
  sound?: boolean;
}) {
  /* ⚠️ A <video> SWALLOWS THE TAP. Lee, 16 Sep 2026: *"I have to tap it about three or four or
     five times... you should just touch the video anywhere."* He is describing the difference
     between the photo slides and the video slide, and it is real: a media element is an
     interactive control in its own right, so a touch that lands on it is consumed there and
     never reaches the <Link> wrapped around it. Tapping a PHOTO worked first time; tapping the
     VIDEO needed you to find a stray pixel of the link around its edge.

     `pointer-events: none` makes the media decorative, so every touch anywhere on the slide
     goes to the link that wraps it. This is the feed only — the media here is never the control,
     the whole slide is. The full-screen player has its own real controls and is untouched. */
  const inert = className + " pointer-events-none";
  if (media.kind === "video") return <FeedVideo url={media.url} className={inert} poster={poster} sound={sound} />;
  return (
    <img src={thumbFor(media.url)} alt="" loading={eager ? "eager" : "lazy"} decoding="async"
...
function FeedVideo({ url, className, poster, sound = false }: { url: string; className: string; poster?: string | null; sound?: boolean }) {
  ...
  useEffect(() => {
    ...
    /* Set as a PROPERTY too, not only the JSX attribute: some WebKit builds refuse autoplay
       ...
       ⚠️ AUTOPLAY IS ONLY GRANTED TO A MUTED ELEMENT. So the first play() is always made while
       muted, and the sound is turned on immediately afterwards. Asking for sound BEFORE the
       element has ever played is how a video silently refuses to start at all. */
    el.muted = true;
    if (typeof IntersectionObserver === "undefined") return;
    const io = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) void el.play().then(() => { el.muted = !sound; }).catch(() => undefined);
