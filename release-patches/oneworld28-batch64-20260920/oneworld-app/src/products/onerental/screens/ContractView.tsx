...
import {
  buildSnapshot, sha256, stamp, ADDENDUMS, rentalError, locationLine,
...
export default function ContractView() {
  ...
  const prop = useAsync(async () => {
    ...
    const [{ data }, addressResult] = await Promise.all([
      supabase.from("rental_properties")
        .select("id, title, city, region, country, neighbourhood").eq("id", c.property_id).maybeSingle(),
  ...
  /* An open extension offer, if there is one. Read by both sides — the host's manage screen
     shows it as "waiting on your tenant", and the tenant gets the buttons. */
  const offer = useAsync(async () => {
    if (!c) return null;
    const { data } = await supabase.from("rental_contract_amendments")
      .select("id, old_ends_on, new_ends_on, reason")
      .eq("contract_id", c.id).eq("kind", "extension").eq("state", "offered")
      .order("created_at", { ascending: false }).limit(1).maybeSingle();
    return data as any;
  }, [c?.id, tick], !!c);

  async function respondExtension(accept: boolean) {
    if (!offer) return;
    setBusy(true); setErr(null);
    const { error } = await supabase.rpc("rental_respond_extension", { p_amendment: offer.id, p_accept: accept });
    setBusy(false);
  ...
  async function accept() {
    ...
    const snapshot = buildSnapshot({
      /* ⛔ The three invented monthly sentences used to be appended to the document text here.
         A contract document must not carry a clause nobody agreed to, and it certainly must not
         carry one this codebase made up. The host's real terms already reach the contract through
         `ContractScreen`'s draft, from `lease_notice_days` and its two neighbours. */
      documentText: body,
      ...
      property: `${prop?.title ?? ""} — ${locationLine(prop ?? {}, lang)}`,
    ...
    <div className="space-y-4">
      ...
      {prop?.address_line && (
        <a
          ...
          href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([formatStoredAddress(prop.address_line ?? ""), prop.city].filter(Boolean).join(", "))}`}
          target="_blank"
...
>
      ...
      {/* ── THE TWO DOORS ONTO A RUNNING STAY ─────────────────────────────────────────────
          The host gets the controls for changing it. The tenant gets the one thing a host can
          ask of them but cannot do to them: staying longer. Both live here because this screen
          is the one both parties already open to see where they stand. */}
      {iAmAgent && c.status === "active" && (
        <Link to={productHref("onerental", `/c/${c.id}/manage`)}
          className="btn-ghost ow-tap flex w-full justify-center px-4 py-3 text-[14px] font-black">
          {W(lang, "Manage this stay", "Gestionar esta estadía")}
        </Link>
      )}

      {iAmTenant && offer && (
        <section className="card border-amber-500/40 bg-amber-500/[0.07]">
          <h2 className="text-[13px] font-black uppercase tracking-wide opacity-60">
            {W(lang, "Your host has offered an extension", "Tu anfitrión ofreció una extensión")}
          </h2>
          <p className="mt-2 text-[14px] leading-snug">
            {W(lang, `Stay until ${offer.new_ends_on} instead of ${offer.old_ends_on}.`,
                     `Quedarte hasta el ${offer.new_ends_on} en lugar del ${offer.old_ends_on}.`)}
          </p>
          {offer.reason && <p className="mt-1 text-[12.5px] opacity-70">{offer.reason}</p>}
          <p className="mt-2 text-[12.5px] opacity-65">
            {W(lang, "Nothing changes unless you accept. Accepting means the rent keeps running for the extra time.",
                     "Nada cambia si no aceptas. Aceptar significa que el arriendo sigue corriendo ese tiempo extra.")}
          </p>
          <div className="mt-3 grid gap-2">
            <button type="button" className="btn-primary ow-tap w-full px-4 py-3 disabled:opacity-45" disabled={busy}
              onClick={() => void respondExtension(true)}>
              {W(lang, "Accept the extension", "Aceptar la extensión")}
            </button>
            <button type="button" className="btn-ghost ow-tap w-full px-4 py-3 disabled:opacity-45" disabled={busy}
              onClick={() => void respondExtension(false)}>
              {W(lang, "No thanks", "No, gracias")}
            </button>
          </div>
        </section>
      )}
