 ...
 */
/** New schedules use USD; never copy a native foreign amount into that field. */
export function scheduleRentUsd(property: { price?: unknown; currency?: string | null }, rate?: number | null): string | null {
  if (property.price == null || property.price === "") return null;
  const amount = Number(property.price);
  if (!Number.isFinite(amount) || amount <= 0) return null;
  const source = (property.currency || "USD").toUpperCase();
  if (source === "USD") return String(amount);
  if (source !== "COP" || rate == null || !Number.isFinite(rate) || rate <= 0) return null;
  const converted = amount / rate;
  return Number.isFinite(converted) && converted > 0 ? converted.toFixed(2) : null;
}
...
export default function PaymentSetup() {
  ...
  // A different property/schedule starts a different draft, including async loading state.
  return <PaymentSetupForm key={JSON.stringify([propertyId, scheduleId])} propertyId={propertyId} scheduleId={scheduleId} />;
}

export function PaymentSetupForm({ propertyId, scheduleId }: { propertyId?: string; scheduleId: string | null }) {
  const { lang } = useI18n();
  ...
  useEffect(() => {
    if (!property || scheduleId) return;
    setStartsOn(prev => prev || property.available_from || "");
  }, [property, scheduleId]);

  useEffect(() => {
    if (!property || scheduleId || rentEdited.current || rentSeeded.current) return;
    const initial = scheduleRentUsd(property, trm?.rate);
    if (initial == null) return;
    rentSeeded.current = true;
    setRent(initial);
  }, [property, scheduleId, trm?.rate]);
  ...
  useEffect(() => {
    if (!existing || existingLoaded) return;
  ...
  const validRate = trm != null && Number.isFinite(trm.rate) && trm.rate > 0;
  const unsupportedSchedule = !!existing && (existing.currency || "USD").toUpperCase() !== "USD";
  ...
  async function save(send: boolean) {
    if (!userId || !propertyId || (scheduleId && (!existingLoaded || unsupportedSchedule))) return;
  ...
  if (scheduleId && (!existingLoaded || unsupportedSchedule)) {
    return <div className="space-y-3 pb-28">
      ...
      <p role="status">{unsupportedSchedule
        ? W(lang, "This schedule uses a different currency and cannot be edited in this USD form. Its saved amounts have not been changed.", "Este plan usa otra moneda y no se puede editar en este formulario en USD. Sus montos guardados no han cambiado.")
        : existing === null
          ? W(lang, "This payment schedule is unavailable. Go back and reopen the correct schedule.", "Este plan de pagos no está disponible. Vuelva y abra el plan correcto.")
          : W(lang, "Loading your saved payment schedule…", "Cargando su plan de pagos guardado…")}</p>
    </div>;
  }
    ...
    <div className="space-y-1 pb-28">
      ...
      <FormSection title={W(lang, "The rent", "El canon")} required
        ...
        <Field label={W(lang, "Monthly rent (USD)", "Canon mensual (USD)")}
          hint={!scheduleId && property && !rentEdited.current && !rentSeeded.current && scheduleRentUsd(property, trm?.rate) == null
            ? W(lang, "The listing price could not be converted. Enter the monthly rent in USD, or wait for the exchange rate.", "No se pudo convertir el precio del inmueble. Ingrese el canon mensual en USD o espere la tasa de cambio.")
            : validRate && rentNum > 0
            ? W(lang, `≈ ${fmtCop(rentNum, trm.rate)} at today's official rate`,
                      `≈ ${fmtCop(rentNum, trm.rate)} a la tasa oficial de hoy`)
            : undefined}>
          <MoneyInput value={rent} onChange={v => { rentEdited.current = true; setRent(v); }} currency="USD" placeholder="1900" />
        </Field>
