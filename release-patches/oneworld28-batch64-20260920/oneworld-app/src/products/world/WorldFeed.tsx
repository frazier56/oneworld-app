...
export default function WorldFeed() {
  ...
  useEffect(() => {
    if (mapOpen && homes.view !== "map") homes.setView("map");
    else if (!mapOpen && homes.view === "map") homes.setView("list");
    if (!mapOpen) pushedMap.current = false;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mapOpen]);
  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden bg-black text-white">
            ...
            <div key={l.key} ref={el => { columns.current[i] = el; }}
              ...
              {!data.ready ? (
                ...
                <div className="min-h-full w-full overflow-y-auto bg-paper px-3 pb-28
                  ...
                  {/* ── THE WAY OUT (R17 note 2) ──────────────────────────────────────────
                      The map used to be the one screen in this feed with no exit: the rail is
                      hidden here (a map is not an item, so save, comment and share have nothing
                      to act on), and the button that opened it went with the rail. Back now
                      works — see the history note above — but a gesture is not a control, and a
                      screen a person can reach has to show them the way off it.

                      Labelled, not a bare chevron. "Back" on its own asks "back to what?" when
                      you have been swiping sideways through four lanes. */}
                  <button type="button" onClick={() => chooseView("list")}
                    className="ow-tap sticky top-0 z-[2] -mt-1 mb-3 inline-flex h-[38px] items-center gap-1.5
                      rounded-full border ow-edge bg-paper/90 px-3.5 text-[13px] font-extrabold
                      text-ink backdrop-blur-md dark:bg-ink/85 dark:text-paper">
      ...
      <div aria-hidden={!pane}
        ...
        className={`absolute inset-x-0 bottom-0 top-0 z-30 overflow-y-auto bg-paper/[0.85] px-4 pb-28
          pt-[calc(env(safe-area-inset-top)+50px)] text-ink backdrop-blur-2xl backdrop-saturate-150
          transition-transform duration-300 dark:bg-ink/[0.85] dark:text-paper
      ...
      <nav aria-hidden={chromeHidden || undefined}
        ...
        <button type="button"
          onClick={() => { setPane(false); chooseView("list"); homes.setLayout("big"); }}
        ...
        {/* GO INTO THIS LANE'S APP. It wears that app's own centre icon rather than a plus, so
            the button says which app it opens before you press it — and because both the icon
            and the destination come from the product's config, the feed's centre button and the
            app's centre button are the same button by construction. */}
