...
import { CONFIGS, MessagesScreen, ThreadScreen, ProfileScreen, PublicWorld, SettingsScreen, lazyScreen, ScreenFallback} from "@oneworld/shell";
...
export function EventProviders() {
  return (
    <AuthProvider>
      ...
      {/* VAIA moved to the APP ROOT on 14 Sep 2026 (see `App.tsx`). She was mounted here, inside
          OneEvent's own provider tree, which is precisely why seven other products tapped the pill
          and got nothing. Two mounts would mean two listeners and two panels on one tap, so this
          one goes. Nothing about OneEvent's VAIA changes — it is the same component. */}
    </AuthProvider>
  );
}
...
export const oneEventChildRoutes = (
  <Route element={<EventProviders />}>
    ...
    {/* The shell's PlanUpsell links every product to `/plans`; OneEvent's screen is `pricing`. */}
