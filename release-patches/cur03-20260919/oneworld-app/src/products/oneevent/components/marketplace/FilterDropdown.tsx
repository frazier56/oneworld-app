...
export function FilterDropdown({ label, options, value, onChange }: { label: string; options: Opt[]; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <>
 <button type="button" onClick={() => setOpen(true)} className={`ow-edge shrink-0 rounded-full border px-3.5 py-2 text-sm font-semibold ${active ? "border-teal bg-teal/10 text-teal" : " "}`}>
      ...
      {open && createPortal(
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/40 p-6" onClick={() => setOpen(false)}>
          <div className="glass-modal max-h-[60svh] w-full max-w-[320px] space-y-1 overflow-y-auto rounded-3xl p-3 shadow-2xl" onClick={e => e.stopPropagation()}>
