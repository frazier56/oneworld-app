...
export default function Alerts() {
  ...
  return (
    <div className="space-y-3">
      <ScreenHeading right={unread> 0 && user?.id ? ( <button onClick={async () => { setRows(prev => (prev || []).map(x => ({ ...x, read_at: x.read_at || new Date().toISOString() }))); await markAllNotificationsRead(user.id); }} className="rounded-full border ow-edge px-3 py-1.5 text-xs font-semibold"> {t("markAllRead")} </button> ) : null}>{t("alertsTitle")}</ScreenHeading>
