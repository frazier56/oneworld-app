...
const COPY = {
  ...
  /* ⚠️ A SHORTER FORM, BECAUSE IT NOW LIVES IN A NARROW COLUMN. "30 nights minimum" is one
     character too wide for a two-column grid on a phone and wrapped to "30 nights / minimum" —
     a second line that is not a statement. "30-night minimum" is the same fact in one line.
     Takes the bare number, not a counted phrase, because the hyphenated unit is singular. */
  minStayShort:  { en: "{d}-night minimum", es: "mínimo {d} noches", de: "Mindestens {d} Nächte", ru: "минимум {d} ночей", zh: "最少 {d} 晚", pt: "mínimo de {d} noites" },
  ...
  /* The count is the point — see the note on the button in PropertyDetail. */
  moreDetails:   { en: "{n} more details", es: "{n} detalles más", de: "{n} weitere Angaben", ru: "ещё {n} деталей", zh: "另有 {n} 项详情", pt: "mais {n} detalhes" },
  /* ⚠️ "1 MORE DETAILS". The plural template is right for every count but one, and one is a
     count a real listing reaches — the sale listing in the harness hit it on the first render.
     A separate key rather than a plural rule: these are six languages whose plural rules do not
     agree (Russian has three forms), and the only count that needs special handling here is 1. */
  oneMoreDetail: { en: "1 more detail", es: "1 detalle más", de: "1 weitere Angabe", ru: "ещё 1 деталь", zh: "另有 1 项详情", pt: "mais 1 detalhe" },
  ...
  /* ⚠️ THE MARK, NOT A DISCLAIMER. The host's own words, put through a machine. Short, quiet and
     unmissable — anyone deciding on a place from a translated house rule deserves to know the
     host never wrote that sentence. */
  autoTranslated:{ en: "Translated automatically", es: "Traducido automáticamente", de: "Automatisch übersetzt", ru: "Переведено автоматически", zh: "自动翻译", pt: "Traduzido automaticamente" },
  showOriginal:  { en: "Show original", es: "Ver original", de: "Original anzeigen", ru: "Показать оригинал", zh: "显示原文", pt: "Ver original" },
  showTranslated:{ en: "Show translation", es: "Ver traducción", de: "Übersetzung anzeigen", ru: "Показать перевод", zh: "显示译文", pt: "Ver tradução" },
  ...
  /* ⚠️ THIS SENTENCE WAS WRONG THE MOMENT MESSAGING STOPPED BEING PAUSED. It still told the
     reader their messages were on hold while the Send message button sat live above it. */
  oneAtATime:    { en: "You already have a request open, so showings and bookings are on hold. You can still message the host.", es: "Ya tiene una solicitud abierta, así que las visitas y reservas están en espera. Aún puede escribirle al anfitrión.", de: "Sie haben bereits eine offene Anfrage, daher pausieren Besichtigungen und Buchungen. Nachrichten sind weiter möglich.", ru: "У вас уже есть открытая заявка, поэтому показы и бронирования на паузе. Написать хозяину по-прежнему можно.", zh: "您已有一个进行中的申请，因此看房与预订暂停。您仍然可以给房东留言。", pt: "Você já tem uma solicitação aberta, então visitas e reservas ficam em espera. Você ainda pode falar com o anfitrião." },
  ...
  /* ⛔ `timeNotSet` IS GONE. There is no such state any more: a listing that says nothing gets
     the house default, three in the afternoon and eleven in the morning. Telling a guest the time
     was "not stated" left them with a question to go and ask, on a screen whose whole job is to
     answer it. */
  standardTime:  { en: "Standard time", es: "Hora estándar", de: "Standardzeit", ru: "Стандартное время", zh: "标准时间", pt: "Horário padrão" },
  ...
  /* ── THE REQUEST PANEL (batch 41) ─────────────────────────────────────────────────────────
     Lee, 17 September 2026: *"you literally duplicate it… that needs to be consolidated down"*
     and *"it really needs to be in a pop-up box… it doesn't take up all this damn room on the
     screen unnecessarily."* One pill on the listing, everything else behind a tap.

     And inside it: *"these should be sometimes steps, sometimes information."* What has already
     happened is not a step. What the two of you have still to do is a numbered step. A note about
     where a copy lives is neither — it is a footnote. */
  yourRequest:   { en: "Your request", es: "Su solicitud", de: "Ihre Anfrage", ru: "Ваша заявка", zh: "您的申请", pt: "Sua solicitação" },
  tapToView:     { en: "Tap to view", es: "Toque para ver", de: "Zum Ansehen tippen", ru: "Нажмите, чтобы открыть", zh: "点击查看", pt: "Toque para ver" },
  okGotIt:       { en: "OK", es: "OK", de: "OK", ru: "OK", zh: "好", pt: "OK" },
  alreadyDone:   { en: "Done", es: "Hecho", de: "Erledigt", ru: "Готово", zh: "已完成", pt: "Feito" },
  sentAndWaiting:{ en: "Request sent. The host can see it.", es: "Solicitud enviada. El anfitrión ya la ve.", de: "Anfrage gesendet. Der Gastgeber sieht sie.", ru: "Заявка отправлена. Хозяин её видит.", zh: "申请已发送，房东可以看到。", pt: "Solicitação enviada. O anfitrião já pode vê-la." },
  whatHappensNow:{ en: "What happens now", es: "Qué sigue", de: "Wie es weitergeht", ru: "Что дальше", zh: "接下来会怎样", pt: "O que acontece agora" },
  stepHostAnswers:{ en: "The host answers by {when}. If they do not, the request lapses and you are not charged.", es: "El anfitrión responde antes del {when}. Si no lo hace, la solicitud caduca y no se le cobra nada.", de: "Der Gastgeber antwortet bis {when}. Andernfalls verfällt die Anfrage und Ihnen wird nichts berechnet.", ru: "Хозяин отвечает до {when}. Если нет, заявка аннулируется и с вас ничего не спишут.", zh: "房东须在 {when} 前答复。若未答复，申请失效，不会向您收费。", pt: "O anfitrião responde até {when}. Se não responder, a solicitação caduca e você não é cobrado." },
  stepHostAccepted:{ en: "The host accepted.", es: "El anfitrión aceptó.", de: "Der Gastgeber hat angenommen.", ru: "Хозяин принял заявку.", zh: "房东已接受。", pt: "O anfitrião aceitou." },
  stepPayNow:    { en: "Send the rent by {rail} and confirm it here by {when}. Your dates are held until then. Nothing is taken automatically.", es: "Envíe el canon por {rail} y confírmelo aquí antes del {when}. Sus fechas están reservadas hasta entonces. Nada se cobra automáticamente.", de: "Senden Sie die Miete per {rail} und bestätigen Sie es hier bis {when}. Ihre Termine sind bis dahin reserviert. Es wird nichts automatisch abgebucht.", ru: "Отправьте оплату через {rail} и подтвердите здесь до {when}. До этого момента даты за вами. Автоматически ничего не списывается.", zh: "请在 {when} 前通过 {rail} 支付租金并在此确认。在此之前为您保留日期，系统不会自动扣款。", pt: "Envie o aluguel por {rail} e confirme aqui até {when}. Suas datas ficam reservadas até lá. Nada é cobrado automaticamente." },
  stepPayCard:   { en: "Authorize your card by {when}. It is held, not charged, until the host confirms.", es: "Autorice su tarjeta antes del {when}. Queda retenida, no cobrada, hasta que el anfitrión confirme.", de: "Autorisieren Sie Ihre Karte bis {when}. Sie wird nur reserviert, nicht belastet, bis der Gastgeber bestätigt.", ru: "Авторизуйте карту до {when}. Средства блокируются, но не списываются, пока хозяин не подтвердит.", zh: "请在 {when} 前授权您的银行卡。在房东确认前只作预授权，不会扣款。", pt: "Autorize seu cartão até {when}. Fica retido, não cobrado, até o anfitrião confirmar." },
  stepDontPayYet:{ en: "Do not send any money yet. Wait until the host accepts.", es: "No envíe dinero todavía. Espere a que el anfitrión acepte.", de: "Senden Sie noch kein Geld. Warten Sie, bis der Gastgeber zusagt.", ru: "Пока не отправляйте деньги. Дождитесь согласия хозяина.", zh: "请先不要付款，等房东接受后再付。", pt: "Ainda não envie dinheiro. Espere o anfitrião aceitar." },
  stepYouSent:   { en: "You confirmed sending it on {when}. The host confirms receipt next.", es: "Usted confirmó el envío el {when}. Falta que el anfitrión confirme la recepción.", de: "Sie haben den Versand am {when} bestätigt. Als Nächstes bestätigt der Gastgeber den Eingang.", ru: "Вы подтвердили отправку {when}. Далее хозяин подтверждает получение.", zh: "您已于 {when} 确认付款，接下来由房东确认收到。", pt: "Você confirmou o envio em {when}. Falta o anfitrião confirmar o recebimento." },
  stepPaymentIn: { en: "The host confirmed your payment.", es: "El anfitrión confirmó su pago.", de: "Der Gastgeber hat Ihre Zahlung bestätigt.", ru: "Хозяин подтвердил ваш платёж.", zh: "房东已确认您的付款。", pt: "O anfitrião confirmou seu pagamento." },
  stepSendId:    { en: "Add your passport or ID. Final approval waits on it.", es: "Agregue su pasaporte o documento. La aprobación final depende de eso.", de: "Fügen Sie Ihren Pass oder Ausweis hinzu. Die endgültige Zusage hängt davon ab.", ru: "Добавьте паспорт или удостоверение. Окончательное одобрение зависит от этого.", zh: "请上传护照或身份证件，最终批准以此为准。", pt: "Adicione seu passaporte ou documento. A aprovação final depende disso." },
  stepConfirmed: { en: "Your stay is confirmed.", es: "Su estadía está confirmada.", de: "Ihr Aufenthalt ist bestätigt.", ru: "Ваше проживание подтверждено.", zh: "您的入住已确认。", pt: "Sua estadia está confirmada." },
  stepLapsed:    { en: "This request lapsed. Do not send payment. Ask the host to start a new one.", es: "Esta solicitud caducó. No envíe el pago. Pida al anfitrión iniciar una nueva.", de: "Diese Anfrage ist verfallen. Senden Sie keine Zahlung. Bitten Sie den Gastgeber um eine neue.", ru: "Заявка аннулирована. Не отправляйте оплату. Попросите хозяина создать новую.", zh: "此申请已失效，请勿付款。请让房东重新发起。", pt: "Esta solicitação caducou. Não envie pagamento. Peça ao anfitrião para iniciar outra." },
  cleaningFee:   { en: "Cleaning, at checkout", es: "Aseo, a la salida", de: "Reinigung, beim Auszug", ru: "Уборка, при выезде", zh: "清洁费，退房时", pt: "Limpeza, na saída" },
  cleaningToHost:{ en: "Charged once when you move out, not now. If you extend your stay it carries over.", es: "Se cobra una sola vez al mudarse, no ahora. Si prorroga la estadía, se traslada.", de: "Wird einmalig beim Auszug berechnet, nicht jetzt. Bei Verlängerung wird sie übertragen.", ru: "Списывается один раз при выезде, а не сейчас. При продлении переносится.", zh: "仅在退房时收取一次，现在不收。若续住则顺延。", pt: "Cobrada uma única vez na saída, não agora. Se prorrogar, é transferida." },
  houseRules:    { en: "House rules", es: "Reglas de la casa", de: "Hausregeln", ru: "Правила дома", zh: "住宿规则", pt: "Regras da casa" },
  /* ⚠️ SHORT ON PURPOSE — these two sit side by side in the pinned bar at the foot of every
     listing, and "Request a showing" / "Besichtigung anfragen" cannot share a 390px row with
     anything. The full wording still appears on the buttons in the page body, where each one has
     the whole width to itself. */
  barMessage:    { en: "Message", es: "Mensaje", de: "Nachricht", ru: "Написать", zh: "留言", pt: "Mensagem" },
  barShowing:    { en: "Showing", es: "Visita", de: "Termin", ru: "Показ", zh: "看房", pt: "Visita" },
  /* ── VIEWING BEFORE BOOKING (17 September 2026) ────────────────────────────────────────
     The warning is shown BEFORE the request is sent, because afterwards it is only an excuse. */
  seeFirstTitle: { en: "See it first?", es: "¿Verlo primero?", de: "Zuerst besichtigen?", ru: "Сначала посмотреть?", zh: "先看房吗？", pt: "Ver primeiro?" },
  seeFirstBody:  { en: "You can still ask for a viewing while the host is deciding. Once they accept and hold your dates, viewings close — from that point cancelling means money moving back and forth.", es: "Puede pedir una visita mientras el anfitrión decide. Cuando acepte y reserve sus fechas, las visitas se cierran — a partir de ahí cancelar implica devoluciones de dinero.", de: "Sie können eine Besichtigung anfragen, solange der Gastgeber entscheidet. Sobald er zusagt und Ihre Termine reserviert, sind Besichtigungen geschlossen — ab da bedeutet eine Absage Geld hin und zurück.", ru: "Пока хозяин решает, вы можете попросить показ. После согласия и брони дат показы закрываются — дальше отмена означает возврат денег.", zh: "房东考虑期间您仍可预约看房。一旦房东接受并锁定日期，看房即关闭——之后取消就会涉及退款。", pt: "Você ainda pode pedir uma visita enquanto o anfitrião decide. Quando ele aceitar e reservar suas datas, as visitas se encerram — a partir daí cancelar significa dinheiro indo e voltando." },
  seeFirstGo:    { en: "Request a viewing first", es: "Pedir una visita primero", de: "Zuerst Besichtigung anfragen", ru: "Сначала запросить показ", zh: "先预约看房", pt: "Pedir uma visita primeiro" },
  seeFirstSkip:  { en: "Request the booking", es: "Solicitar la reserva", de: "Buchung anfragen", ru: "Запросить бронирование", zh: "直接申请预订", pt: "Solicitar a reserva" },
  showingClosed: { en: "The host is holding your dates, so viewings are closed on this place.", es: "El anfitrión tiene reservadas sus fechas, así que las visitas están cerradas en este inmueble.", de: "Der Gastgeber hält Ihre Termine, daher sind Besichtigungen für diese Unterkunft geschlossen.", ru: "Хозяин удерживает ваши даты, поэтому показы по этому объекту закрыты.", zh: "房东已为您锁定日期，因此该房源不再安排看房。", pt: "O anfitrião está segurando suas datas, então as visitas estão encerradas neste imóvel." },
  bookingOpen:   { en: "You already have a booking request open on this place.", es: "Ya tiene una solicitud de reserva abierta en este inmueble.", de: "Sie haben für diese Unterkunft bereits eine offene Buchungsanfrage.", ru: "У вас уже есть открытая заявка на бронирование по этому объекту.", zh: "您在此房源已有一个进行中的预订申请。", pt: "Você já tem uma solicitação de reserva aberta neste imóvel." },
