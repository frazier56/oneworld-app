...
export default function ListHub() {
  ...
  const counts = useAsync(async () => {
    const [saved, mine, drafts, latestDraft] = await Promise.all([
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── ⚠️ REORGANISED INTO THINGS THAT ARE ACTUALLY THE SAME KIND OF THING ───────────────
          Lee, 20 September 2026: *"you got all this stuff scattered all over the place and they
          really are subsets of each other… I would say you get three options: my profile, my
          properties for sale, my properties for rent. And then you got calendar."*

          The old grouping was "Your properties", "Manage your search" and "More tools", and those
          headings were about what WE think a row is for rather than what a person owns. Renting
          out a place and renting one were split across two sections; the drafts and the showings
          that belong to your rentals sat in different groups from the rentals; and the two
          profiles — host and renter — were nowhere near each other even though they are the same
          noun with two hats.

          Now it reads the way somebody would say it out loud: this is me, these are the places I
          rent out, these are the places I'm selling, and then the two things that belong to
          neither — the calendar, and the places I saved while looking. */}

      <Section label={W(lang, "My profile", "Mi perfil")}>
        ...
        {/* ⚠️ THE TWO PROFILES ARE NOT THE SAME THING, and Lee asked outright whether they were.
            ...
            now sit together under one heading instead of in two unrelated sections. */}
        <Opt d={P.card}
          title={W(lang, "Renter profile", "Perfil de arrendatario")}
          sub={renterPct === null
      ...
      <Section label={W(lang, "My properties for rent", "Mis inmuebles en arriendo")}>
        <Opt d={P.home} primary
          title={W(lang, "My rental properties", "Mis inmuebles en arriendo")}
          sub={!counts || (counts.mine === 0 && counts.drafts === 0)
      ...
      <Section label={W(lang, "My properties for sale", "Mis inmuebles en venta")}>
        <Opt d={P.home}
      ...
      <Section label={W(lang, "Everything else", "Todo lo demás")}>
        <Opt d={P.cal}
        ...
        {/* Saved places belong to neither portfolio: they are somewhere you are LOOKING at, on
            both sides of the product at once, which is why the count says so. */}
        <Opt d={P.heart}
...
/* ── ⚠️ THE COUNT CHIP IS GONE. THE COUNT LIVES IN THE GREY LINE. (Lee, 14 Sep 2026) ────────
   *"My rental properties, when you click the centre footer button, is wrapped. Just remove that
   [count]. You could put the count below in the grey text."*

   The chip was `shrink-0`, so it took its width first and the TITLE — the only flexible thing in
   the row — absorbed the loss and wrapped to two lines. "My rental properties" became "My rental
   / properties". The chip also said "10" beside a grey line already reading "10 live · 10 in
   draft": the same number twice, one of them unlabelled.

   So the count is not shortened or shrunk, it is MOVED, into the line that can say what the
   number counts. Removed from the component itself rather than from the two call sites Lee
   happened to see, per his standing rule that a fix asked for once belongs on every screen. */
/* ── ⚠️ WHAT EATS A TILE'S WIDTH — Lee, 14 Sep 2026, second pass ─────────────────────────────
   *"The arrows take up space, causing the text to wrap unnecessarily… it's implied that you click
   on these things. And then you got these icons on the left side — I think the icons were cool,
   but they do take up a lot of space."*

   A row here is: icon + gap + TITLE + gap + chevron. Everything except the title is `shrink-0`,
   so every fixed pixel is taken out of the title's budget and the title is the only thing that can
   pay — by wrapping. Measured at 390px: the chevron and its gap were 28px and the icon block was
   60px, so 88 of 358 usable pixels went to two things that carry no information.

   The chevron is gone. The whole row is a button, it depresses on touch, and nothing else on the
   screen looks like it — an arrow saying "this is tappable" is the redundancy Lee keeps striking.
   The icon stays, at 40px rather than 48, because it is the thing that makes the list scannable
   and he likes it; 20px of that 88 comes back without losing it.

   Removed from the COMPONENT, so it holds on every tile on both hubs, not the two he pointed at. */
function Opt({ d, title, sub, primary, onClick }: {
  d: string; title: string; sub: string; primary?: boolean; onClick: () => void;
}) {
  return (
