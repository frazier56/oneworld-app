...
/* ── "optional" AND "OPT" WERE ENGLISH IN EVERY LANGUAGE ────────────────────────────────────
   Lee, 14 September 2026: *"When you push another flag, that means a hundred percent of that
   page needs to be translated."* Two badges in this file were bare English literals rather than
   copy, so every optional field on every form in every product said "optional" in English to a
   German, Russian or Chinese reader. A hard-coded word is invisible to a translation pass that
   only looks at the strings somebody remembered to wrap. */
const LESS: Localized = { en: "Less", es: "Menos", co: "Menos", de: "Weniger", pt: "Menos", ru: "Меньше", zh: "减少" };
const MORE: Localized = { en: "More", es: "Más", co: "Más", de: "Mehr", pt: "Mais", ru: "Больше", zh: "增加" };
const VALUE: Localized = { en: "Value", es: "Valor", co: "Valor", de: "Wert", pt: "Valor", ru: "Значение", zh: "数值" };
const OPTIONAL: Localized = {
  en: "optional", es: "opcional", co: "opcional",
  de: "optional", pt: "opcional", ru: "необязательно", zh: "选填",
};
const OPTIONAL_SHORT: Localized = {
  en: "OPT", es: "OPC", co: "OPC",
  de: "OPT", pt: "OPC", ru: "НЕОБ.", zh: "选填",
};

function OptionalBadge({ short, className }: { short?: boolean; className?: string }) {
  const { lang } = useI18n();
  return <span className={className}>{WA(lang, short ? OPTIONAL_SHORT : OPTIONAL)}</span>;
}
      ...
      <div className="mb-3.5 flex items-start gap-2.5">
        <div className="min-w-0 flex-1">
          {/* v92 · the hint is a ⓘ, not a line of prose. Lee, 17 Aug: *"anytime you have a
              ...
              ── AND IT SITS BESIDE THE TITLE, NOT UNDER IT (Lee, 12 Sep 2026) ──────────────
              *"It has an info button there... but that info button should be right to the
              right of videos."* It was on its own line below the heading, which gave a
              one-word section a two-line header and left a dot floating with nothing to
              belong to. Inline in the heading it reads as part of the title, which is what it
              is — and the section gets its height back. */}
          <h3 className="text-lg font-extrabold leading-tight tracking-tight">
            {title}{required && <span className="ml-1 text-red-500">*</span>}
            {hint && <InfoDot title={title} body={hint} lang={lang} className="ml-1.5 align-middle" />}
          </h3>
        </div>
    ...
    <div className="flex h-full flex-col">
      ...
      <span className="label flex min-w-0 flex-col items-start gap-0.5">
        ...
        <span className="flex min-w-0 max-w-full items-center gap-0.5">
          ...
          {hint && <InfoDot title={label} body={hint} lang={lang} />}
        </span>
        {optional && <OptionalBadge className="text-[10.5px] font-semibold uppercase tracking-wide opacity-40" />}
      </span>
    ...
    return (
      <div className={`grid gap-2 ${cols === 3 ? "grid-cols-3" : "grid-cols-2"}`}>
        {options.map(o => {
          ...
          return (
            /* ── THE TICK BOX IS GONE (Lee, 12 Sep 2026) ─────────────────────────────────
               *"Furnished and Penthouse, they're just not in the pill correctly. Just take the
               little checkbox thing off, and that's gonna leave room for them... and Open view is
               on two rows unnecessarily."*

               He is right, and the arithmetic is the argument. A 16px box plus its 8px gap is 24
               of a 3-column chip's ~100px on a 390px phone — a quarter of the width spent saying
               something the chip already says three other ways: brand border, brand tint, brand
               text. "Open view" needed those 24 pixels and wrapped without them.

               Nothing is lost for a screen reader either: `aria-pressed` is what actually
               announces the state, and it was carrying that job the whole time. */
            <button key={o.value} type="button" aria-pressed={on}
              onClick={() => onChange(on ? values.filter(v => v !== o.value) : [...values, o.value])}
              className={`ow-tap flex min-h-[44px] items-center justify-center rounded-xl border px-2 py-2 text-center text-[12.5px] font-bold leading-snug transition ${
                on
    ...
    <div className="flex flex-wrap gap-2">
      {options.map(o => {
        ...
        return (
          <button key={o.value} type="button" aria-pressed={on}
            onClick={() => onChange(on ? values.filter(v => v !== o.value) : [...values, o.value])}
            /* Same removal as the column layout above, for the same reason and so the two
               shapes of this one control do not read as two different controls. */
            className={`ow-tap inline-flex items-center rounded-full border px-3.5 py-1.5 text-[12.5px] font-bold transition ${
              on
                ? "border-brand bg-brand/10 text-brand-deep dark:text-brand-light"
                : "border-ink/12 opacity-70 hover:opacity-100 dark:border-white/15"}`}>
            {o.label}
...
export function Stepper({
  ...
  return (
    <div className="flex h-12 items-stretch overflow-hidden rounded-xl border ow-edge">
      <button type="button" aria-label={WA(lang, LESS)} disabled={!empty && value! <= min}
      ...
      <div className="flex min-w-0 flex-1 items-center justify-center gap-1 overflow-hidden whitespace-nowrap bg-ink/[0.025] px-2 dark:bg-white/[0.04]">
        {editable ? (
          <input
            type="number"
            inputMode="decimal"
            aria-label={inputLabel ?? WA(lang, VALUE)}
            min={min}
            max={max}
            step={step}
            value={empty ? "" : value ?? ""}
      ...
      <button type="button" aria-label={WA(lang, MORE)} disabled={!empty && value! >= max}
        style={{ touchAction: "none" }}
        onPointerDown={e => begin(1, e)}
        onPointerUp={() => end(1)}
        onPointerCancel={stop}
  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-ink/[0.09] bg-ink/[0.012] p-2.5 dark:border-white/12 dark:bg-white/[0.02]">
      <div className="mb-1.5 flex min-w-0 flex-1 flex-col items-start gap-0.5">
        ...
        {optional && (
          <OptionalBadge short className="shrink-0 text-[9.5px] font-black uppercase tracking-wider opacity-35" />
        )}
      </div>
      <Stepper {...stepper} />
...
export function Toggle({
 ...
 className="ow-edge ow-tap flex w-full items-start gap-3 rounded-xl border p-3 text-left transition hover:bg-brand/[0.04] ">
  ...
  return (
    <div className="mt-5 space-y-2.5 pb-6">
        ...
        <div className="flex items-stretch gap-2.5">
          {draft && (
            <button type="button" onClick={draft.onClick}
              className="btn-ghost min-w-0 flex-1 truncate whitespace-nowrap text-[12.5px]">
              {draft.label}
            </button>
          )}
          {cancel && (
      ...
      <button type="button" onClick={primary.onClick} disabled={primary.busy || primary.disabled}
        /* ── THE LABEL SHRINKS BEFORE IT IS CUT (14 September 2026) ─────────────────────────
           Lee: *"Make sure text that's supposed to fit inside a bubble actually fits. Try not to
           wrap it."* Measured at 390px: "Weiter: Verfügbarkeit und Besichtigungen" needed 377px
           in a 358px button and "Сохранить черновик" needed 189px in a 174px one — so German and
           Russian readers were losing the end of the only button that moves the form forward.
           ⚠️ A `vw` clamp does NOT solve this, and measuring proved it: at 390px, `3.8vw` is
           14.8px, so the floor never engages on the phone where the problem is. The secondary
           row takes a fixed smaller size, and the one German step title that still did not fit
           was shortened in the dictionary rather than shrinking every language to suit it. */
        className="btn-primary w-full truncate whitespace-nowrap py-3.5 text-[14px] font-bold disabled:opacity-60">
