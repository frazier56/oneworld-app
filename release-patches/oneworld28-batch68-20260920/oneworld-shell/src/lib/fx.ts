...
export function fromUsd(usd: number, to: string, trmCop?: number | null): number | null {
  ...
  const official = to === "COP" && trmCop != null && Number.isFinite(trmCop) && trmCop > 0;
  const rate = official ? trmCop : fxNow()?.perUsd?.[to];
...
export function drawPair(usd: number, viewer: string, trmCop?: number | null) {
  ...
  return {
    ...
    copSource: trmCop != null && Number.isFinite(trmCop) && trmCop > 0 ? ("trm" as const) : (fxNow() ? ("feed" as const) : null),
  };
}
