 ...
 */
/** Keep the rendered denomination alongside the price, including FX fallback. */
export function drawPriceDisplay(usdAmount: number, ccy: ViewerCcy, rate?: number | null): { text: string; currency: ViewerCcy } {
  ...
  if (ccy === "COP" && rate != null && Number.isFinite(rate) && rate > 0 && Number.isFinite(n * rate)) {
    return { currency: "COP", text: new Intl.NumberFormat("es-CO", {
  ...
  if (ccy && ccy !== "USD") {
    ...
    if (converted != null) return { text: fmtCcy(converted, ccy), currency: ccy };
  }

  return { currency: "USD", text: new Intl.NumberFormat("en-US", {
    style: "currency", currency: "USD", maximumFractionDigits: 0,
  }).format(n) };
}

/** Existing string API preserves every caller's formatting. */
export function drawPrice(usdAmount: number, ccy: ViewerCcy, rate?: number | null): string {
  return drawPriceDisplay(usdAmount, ccy, rate).text;
}
