...
export default function CalendarScreen() {
  ...
  return (
    <div className="space-y-4 pb-28">
      ...
      {/* ── ⚠️ THE MONTH WAS DOING THE PAGE TITLE'S JOB, AND DOING IT BADLY ─────────────────
          This screen had no heading of its own. `ScreenHeading` was given the month name, so the
          VAIA pill and avatar — which every ScreenHeading carries on its right — landed in the
          middle of a row that also held both month arrows. At 390px the result was "September …"
          truncated to an ellipsis, flanked by two large white pills pinned to the screen edges:
          the widest, loudest things on the page were two arrows.

          Split in two. The page says what page it is, once, like every other screen. The month
          navigator is a control row under it, at control weight, with the month centred between
          two arrows that are sized like arrows.

          `min-w-0 flex-1` on the month label is still load-bearing — a flex item defaults to
          min-width:auto, so without it "Septiembre De 2026" pushes the row sideways at 390px,
          which shots-rental.mjs caught as a horizontal scroll in both themes. */}
      <ScreenHeading>{W(lang, "Calendar", "Calendario")}</ScreenHeading>

      <div className="card flex items-center gap-2 p-2">
        <button type="button" className="ow-tap ow-edge grid h-10 w-10 shrink-0 place-items-center rounded-full border text-[17px] font-black"
          aria-label={W(lang, "Previous month", "Mes anterior")}
          onClick={() => setMonth(m => new Date(m.getFullYear(), m.getMonth() - 1, 1))}>‹</button>
        <p className="min-w-0 flex-1 truncate text-center text-[15px] font-black capitalize">{monthName}</p>
        <button type="button" className="ow-tap ow-edge grid h-10 w-10 shrink-0 place-items-center rounded-full border text-[17px] font-black"
          aria-label={W(lang, "Next month", "Mes siguiente")}
          onClick={() => setMonth(m => new Date(m.getFullYear(), m.getMonth() + 1, 1))}>›</button>
      </div>
      ...
      {!!owned?.length && <section className="card space-y-4 p-4" aria-labelledby="host-blackout-heading">
        ...
        {/* ⚠️ IT WAS HALF WIDTH, PINNED RIGHT, AND "Block these dates" WRAPPED ONTO TWO LINES
            inside it — the two defects Lee names most often, in one control. `col-start-2` put
            the page's primary action in the right-hand half of a two-column grid whose left half
            was empty unless you happened to be editing. Full width when it is alone; an even
            pair only when there is genuinely a second action beside it. */}
        <div className={editingId ? "grid grid-cols-2 gap-3" : ""}>
          ...
          <button type="button" className="btn-primary w-full whitespace-nowrap" onClick={() => void saveBlackout()} disabled={busy || loadFailed || !propertyId}>
      ...
      {/* "Nothing booked yet" floated on its own with nothing saying what was not booked. A
          section needs a name before it can be empty. */}
      <div className="mt-4 space-y-2">
        <h2 className="px-1 text-[11px] font-bold uppercase tracking-[0.08em] opacity-40">
          {W(lang, "Booked stays", "Estadías reservadas")}
        </h2>
        {(rows ?? []).length === 0 && (
          <p className="card p-5 text-center text-[12.5px] opacity-55">
