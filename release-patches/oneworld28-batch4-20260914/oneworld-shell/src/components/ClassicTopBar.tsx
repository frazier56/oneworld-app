...
export function localisedWordmark(config: AppConfig, lang: Lang): AppConfig["wordmark"] {
  ...
  if (home) {
    return {
      ...
      /* Both OneHome sections wear the rental mark in the chrome — one app, one face. */
      markSrc: config.wordmark.brand === "Home" ? "/mark-onerental.png" : config.wordmark.markSrc,
    };
  }
      ...
      <div className="mx-auto flex h-14 max-w-lg items-center justify-between px-4">
        <div className="flex min-w-0 items-center gap-0.5 overflow-hidden">
          ...
          <Link to={home} aria-label={config.wordmark.ink + config.wordmark.brand}>
            <Wordmark wordmark={localisedWordmark(config, lang)} h={38} />
          </Link>
        </div>
