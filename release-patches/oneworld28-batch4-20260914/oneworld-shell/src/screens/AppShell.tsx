...
import AskVaia from "../components/ai/AskVaia";
...
function Chrome({
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* ── THE PILL NOW OPENS SOMETHING (14 September 2026) ────────────────────────────────
          `VaiaPill` has always dispatched `ow-vaia-open`, and until tonight the only listener in
          the codebase lived inside OneEvent — so on every other product tapping VAIA did nothing
          at all, silently. Mounted here, once, above every screen, because the pill is shell
          chrome and a shell control whose behaviour is owned by one product is how eight products
          get eight different assistants.

          OneEvent is skipped: it mounts its own voice-first panel with live transcription, and
          two listeners would open two VAIAs on the same tap. When that panel moves into the shell
          this exception goes with it. */}
      {config.key !== "oneevent" && <AskVaia product={config.key} />}
