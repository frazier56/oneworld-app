...
type SaleAgent = { agent_id: string; full_name: string | null; photo_url: string | null; score: number | null; total_listings: number; active_listings: number };
 ...
 * (`sale_agents`) rather than N queries — a directory that fires a query per row is a
 * directory that gets slower every week it succeeds.
 *
 * `sale_agents` counts PUBLIC listings only, so an agent cannot inflate their number with
...
export default function Agents() {
  ...
  const agents = useAsync(async () => {
    const { data, error } = await supabase.rpc("sale_agents", { q: q.trim() || null, lim: 60 });
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="mt-3 text-[12px] leading-relaxed opacity-55">
        {W(lang,
          "Every agent here carries a OneScore built from work they have actually completed — across selling, renting, jobs and events. It is the part a WhatsApp group cannot give you.",
          "Cada agente aquí tiene un OneScore construido con trabajo realmente completado — en ventas, arriendos, trabajos y eventos. Es lo que un grupo de WhatsApp no le puede dar.")}
      </p>
        ...
        {agents === undefined ? (
          ...
          <div className="card p-8 text-center">
            ...
            <p className="mt-1 text-[12.5px] opacity-55">
              {W(lang, "An agent appears here as soon as they publish their first property.",
          ...
          agents.map(a => (
            <Link
              key={a.agent_id}
              to={productHref("onesale", `/p/${a.agent_id}`)}
