        ...
        <Link to={href} className="pointer-events-auto block">
          <div className="flex items-stretch justify-between gap-3">
            <div className="flex min-w-0 flex-col justify-end">
              {/* ⚠️ THE TITLE IS GONE FROM THE CARD. Lee, 16 Sep 2026: *"When you're on the feed
                  you don't need the title of the property. You just need the attributes — what
                  type of property it is, bedroom, bathroom count, and size."*

                  He is right, and a host's title is the least reliable line on the card: it is
                  free text, it is often in one language, and it is frequently marketing rather
                  than fact. "House · 3 bed · 4 bath · 220 m²" is the same length, tells somebody
                  whether to stop scrolling, and is true of every listing because we derive it.
                  The title is still the heading of the listing page, where there is room for it
                  and where somebody has already decided to look. */}
              {facts && <p className="text-[13.5px] font-bold leading-snug">{facts}</p>}
            </div>
          ...
          {/* ⚠️ REVIEWS LEFT, HEART RIGHT, ONE ROW. Lee, 16 Sep 2026: *"You got this heart sitting
              out there kind of randomly... it can go on the same row where you have property
              reviews and host reviews. You got that on the left and then you can put the heart on
              the far right."*

              That is the right home for it and not only because it tidies the corner: the heart
              and the review counts are the same KIND of thing — what other people think of this
              place, and what you think of it — so they belong on one line. It also means nothing
              floats over the photograph any more, which is what was eating taps.

              ⚠️ This row is INSIDE the <Link>, so the heart must stop the tap reaching it or
              liking a place would also open it. `onClick` with stopPropagation and
              preventDefault on the wrapper is the whole guard. */}
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="shrink-0" onClick={e => { e.preventDefault(); e.stopPropagation(); }}>
              <ListingEngagement
                ...
                allowShare={false}
                hideComments
                /* ⚠️ Keep `onComments` even with no comment button drawn: inside
                   ListingEngagement it is what makes the count query SKIP media_comments. Drop it
                   and every card in the feed runs a count for a button nobody sees. */
                onComments={() => {}}
                lang={lang} compact />
            </div>
          </div>
