    ...
    <div className="relative" ref={box}>
      ...
      <button type="button" onClick={() => setOpen(o => !o)}
        ...
        <span className="relative grid h-4 w-6 place-items-center overflow-hidden rounded-[3px] bg-ink/10 text-[9px] font-bold leading-none dark:bg-white/15">
          {lang.toUpperCase()}
          <img decoding="async" src={flagSrc(current.cc)} alt=""
        ...
        <div role="listbox" translate="no"
          ...
          {countries.map(({ code, cc, country }) => (
            <button key={country} type="button" onClick={() => pickCountry(code)}
              ...
              <img decoding="async" src={flagSrc(cc)} alt="" className="h-3.5 w-5 shrink-0 rounded-[2px] object-cover" />
            ...
            <div className="max-h-56 overflow-y-auto">
              {CURRENCIES.map(c => (
                <button key={c.code} type="button"
                  ...
                  <img decoding="async" src={`https://flagcdn.com/w40/${c.flag}.png`} alt="" className="h-3 w-4 shrink-0 rounded-[2px] object-cover" />
