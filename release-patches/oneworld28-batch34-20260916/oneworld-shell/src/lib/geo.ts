...
export async function geocode(query: string): Promise<GeoPoint | null> {
  ...
  try {
    /* ── ⚠️ THE TIMEOUT WAS ON THE WRONG HALF, AND SAVE COULD HANG FOREVER ──────────────────
       The note above says geocoding must never be able to block publishing. It could. The guard
       below covers the geocoder CALLBACK, and it only starts once `loadGoogleMaps()` has already
       resolved — but that promise settles on `script.onerror` and on Google's own ready callback,
       and on nothing else. A script that downloads and then never invokes its callback — an
       invalid key, a quota that ran out overnight, a corporate proxy returning a stub, an ad
       blocker rewriting the response — leaves it pending for the life of the page.

       `save()` awaits this before it writes anything. So on that failure the host taps Publish,
       the button goes to "…", and it stays there: no row, no error, nothing to retry, and the
       listing they just spent ten minutes on is still only in the form.

       Found by driving the form in a browser with Google unreachable and counting the database
       writes: zero, with no error anywhere. The race covers the LOAD as well as the lookup, so
       the worst case is a listing with no map pin — which the comment above already calls a
       completely normal outcome. */
    const g: any = await Promise.race([
      loadGoogleMaps(),
      new Promise<null>(resolve => window.setTimeout(() => resolve(null), GEOCODE_TIMEOUT_MS)),
    ]);
