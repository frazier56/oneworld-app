  ...
  return (
    /* ── ONE PHOTOGRAPH, NOTHING BESIDE IT ────────────────────────────────────────────────────
       ...
       *"From the main feed, I should be able to scroll sideways to see the listing pictures. I
       ...
       what is on screen. */
    <div>
    {playing && <FeedVideoPlayer url={playing} poster={coverOf(l)} title={l.title} href={href}
    ...
    <article className="ow-feed-card relative overflow-hidden rounded-2xl border border-ink/[0.08] bg-ink/5 dark:border-white/10 dark:bg-white/5">
      {slides.length > 0 ? (
        <div
          ...
          {slides.map((m, i) => (
            ...
            m.kind === "video" ? (
              <button key={m.url} type="button" onClick={() => setPlaying(m.url)}
                {...tapAnywhere(() => setPlaying(m.url))}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 px-3 pb-2 pt-16 text-white"
        ...
        {/* ── v90 · U36 · THE HEART, ALONE ────────────────────────────────────────────────
            ...
            ⚠️ THIS WAS `absolute right-2 top-[58%]` AND IT SAT ON TOP OF THE PRICE. Measured at
            390px: the control covered the last forty pixels of "≈ COP 4.200.000". A percentage
            of the CARD and a text block that grows upward from the card's BOTTOM are unrelated
            coordinates, so the overlap arrived the moment a listing quoted a second currency —
            and the note here said it was "clear of the scrim text on the left", which was true
            and beside the point, because the price is on the RIGHT.

            It is IN FLOW now, right-aligned at the top of the scrim block. Still Lee's "right
            side, middle towards the bottom", and no longer able to land anywhere else however
            many lines the right column carries. Never give this an `absolute` with a percentage
            again. */}
