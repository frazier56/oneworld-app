...
import { coverOf, mediaRowOf, orderedPhotos } from "../lib/media";
...
export default function PropertyDetail() {
  ...
  async function publishNow() {
    ...
    setPublishing(false);
    if (error) { setPublishError(rentalError(error, lang)); return; }
  ...
  const liveRequest = useAsync(async () => {
    ...
    const { data } = await supabase.from("rental_booking_requests")
      .select("id, starts_on, ends_on, nights, guest_total, currency, state, expires_at, check_in_time, check_out_time")
  ...
  const actionState: "open" | "requested" = liveRequest ? "requested" : "open";
  ...
  return (
    <div className="space-y-4">
      ...
      <h1 className="text-[22px] font-black leading-tight tracking-tight">{p.title}</h1>
      ...
      <div className="mt-4 flex items-center gap-x-3 gap-y-1">
        {(p as any).listing_no && (
      ...
      <div className="mt-3 flex flex-wrap items-baseline gap-x-2 gap-y-1">
        ...
        {p.display_currency === "COP" && p.display_fx_rate && (
          <span className="text-[12.5px] opacity-55">
            ≈ {cop(p.price, p.display_fx_rate)} · {D(lang, "chargedInUsd")}
          </span>
        )}
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {p.max_guests != null && (
          <Fact>{counted(lang, p.max_guests, GUESTS)}</Fact>
        )}
        {p.bedrooms != null && <Fact>{counted(lang, p.bedrooms, BEDROOMS)}</Fact>}
        {p.bathrooms != null && <Fact>{counted(lang, p.bathrooms, BATHROOMS)}</Fact>}
        {p.area_m2 != null && <Fact>{p.area_m2} m²</Fact>}
        <Fact>{p.furnished ? D(lang, "furnished") : D(lang, "unfurnished")}</Fact>
        {/* ⛔ A WHOLE SECTION BECAME ONE BULLET. 17 September 2026.
            "Month-to-month stay" was a titled panel on the public listing carrying three invented
            terms and a renewal dashboard. On a listing page the fact worth knowing is just this:
            it is let by the month. Everything else belongs to somebody who already lives there. */}
        {p.price_unit === "month" && <Fact>{D(lang, "rentedMonthly")}</Fact>}
        {p.available_from && (
          <Fact><IconCalendar size={11} /> {D(lang, "from")} {p.available_from}</Fact>
        )}
        {p.min_term_days > 1 && (
          <Fact>{D(lang, "minNights", { n: counted(lang, p.min_term_days, NIGHTS) })}</Fact>
        )}
      </div>
      ...
      {Array.isArray((p as any).house_rules) && (p as any).house_rules.length > 0 && (
        <section className="card mt-3 p-4">
          ...
          <ol className="mt-2 space-y-1.5">
            {((p as any).house_rules as string[]).map((rule, i) => (
      ...
      {/* ⚠️ THE DESCRIPTION HAD NO HEADING AND LEE COULD NOT TELL WHAT IT WAS. 16 Sep 2026:
          *"Right now it's just some text on the page. We don't even know what it is. It should be
          a subsection, just like it says The space and inside and outside."*

          Right: every other block of this page announces itself, so an unlabelled paragraph reads
          as something that escaped from somewhere rather than as the owner's own words. Same
          heading treatment as the amenity groups, so it joins the same list rather than starting
          a second visual language. */}
      {p.description && (
        <section className="mt-5">
          <h3 className="text-[11.5px] font-black uppercase tracking-wide opacity-45">{D(lang, "descTitle")}</h3>
        <p className="mt-2 whitespace-pre-line text-[14.5px] leading-relaxed">{p.description}</p>
        </section>
      )}
      ...
      {!mine && userId && p.status === "published" && (
        <section className="card mt-3 space-y-3 p-4">
          ...
          <ListingActions
            ...
            busy={booking}
            sending={sending}
            note={actionState === "requested" ? D(lang, "oneAtATime") : undefined}
            onMessage={() => void ask()}
            onShowing={() => setShowingOpen(true)}
            onBook={() => {
              setErr(null);
              if (p.price_unit === "month") { setMonthOpen(true); return; }
