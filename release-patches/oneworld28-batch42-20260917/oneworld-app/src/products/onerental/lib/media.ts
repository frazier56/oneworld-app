...
/* ⚠️ THIS USED TO CARRY ONE VIDEO AND LEE FOUND THE BUG. 16 Sep 2026: *"This listing should have
   two videos. I know it does, but when I tap the little arrow over in full screen, where's the
   second video at? It's not here."*

   It was not there because this function built the strip as ONE lead plus the photographs, and
   the lead was whichever single video the host had chosen as the preview. The second video was
   never in the list, so nothing downstream — the swipe strip, the counter, the full-screen
   player — could ever reach it. It only appeared on the listing page because that screen read
   `p.videos` directly.

   Now: every video, in order, then every photograph. One list, used by all three, so they cannot
   disagree about what exists. */
export function feedSlides(p: MediaRow): FeedPreview[] {
  const vids = (p.videos ?? []).map(url => ({ kind: "video" as const, url }));
  const lead = feedLead(p);
  /* The host's chosen preview still goes first when it is a photo — that is what they picked to
     represent the place. When it is a video it is already in `vids` and must not be doubled. */
  const leadPhoto = lead && lead.kind === "photo" ? [lead] : [];
  const seen = new Set([...vids, ...leadPhoto].map(x => x.url));
  const rest = orderedPhotos(p).filter(u => !seen.has(u)).map(url => ({ kind: "photo" as const, url }));
  return [...vids, ...leadPhoto, ...rest];
}
