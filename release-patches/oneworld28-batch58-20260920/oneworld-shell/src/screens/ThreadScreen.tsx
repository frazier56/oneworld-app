...
export default function ThreadScreen({ product, contextSlot }: { product: AppKey; contextSlot?: ReactNode }) {
 ...
 className="ow-edge ow-tap grid h-9 w-9 shrink-0 place-items-center rounded-full border bg-white/60 opacity-45 dark:bg-white/10">
                  <IconPhone />
                </button>
              </div>
              <div className="relative max-[420px]:hidden">
      ...
      {groupInfoOpen && head?.isGroup && (
        <div className="fixed inset-0 z-[80] bg-ink/45 p-3 backdrop-blur-sm" onClick={() => setGroupInfoOpen(false)}>
          <div onClick={e => e.stopPropagation()}
            className="relative mx-auto flex h-full max-h-[92svh] w-full max-w-md flex-col overflow-hidden rounded-[28px] border border-ink/10 bg-paper shadow-2xl dark:border-white/10 dark:bg-ink">
            <div className="border-b border-ink/[0.07] px-5 py-4 text-center dark:border-white/10">
              <button onClick={() => setGroupInfoOpen(false)}
 className="ow-edge ow-tap absolute right-5 top-5 grid h-9 w-9 place-items-center rounded-full border bg-white/70 dark:bg-white/10">
                <IconClose />
              </button>
 ...
 className="ow-edge ow-tap rounded-2xl border bg-white/70 px-2 py-2.5 dark:bg-white/10">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {groupMedia.slice(0, 12).map(item => (
                      <a key={item.id} href={item.url} target="_blank" rel="noreferrer"
                        ...
                        {item.type === "image" ? (
                          <img decoding="async" src={item.url} alt="" className="h-full w-full object-cover" />
                        ) : (
