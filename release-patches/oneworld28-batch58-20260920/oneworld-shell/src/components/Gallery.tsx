/* ⚠️ `decoding="async"` IS ON EVERY IMAGE IN THIS CODEBASE AS OF 16 SEP 2026, AND IT WAS ON
   NONE OF THEM BEFORE. Measured, not guessed: a scripted scroll of this page dropped frames with
   a worst frame of 92 milliseconds, while a text-only screen in the same run dropped none and
   never exceeded 17. Six frames in a row missed is not a stutter you have to look for.

   The default for an image is SYNCHRONOUS decode: when the bytes land, the browser turns that
   JPEG into pixels ON THE MAIN THREAD, and everything else — scrolling included — stops until it
   finishes. A full-width photograph is tens of milliseconds of that. A gallery is eighteen of
   them, arriving whenever the network feels like it, each one freezing the page for a moment.
   That is what Lee has been describing for four hours as flicker, and it is why it happens on
   image screens and not on text screens, at rest as well as while scrolling.

   `decoding="async"` moves that work off the main thread. It is one attribute, it is what every
   large media site sets, and we had it on zero of ninety-five images. */
...
/* ── A PHOTO BUTTON WITH NO NAME IS A BUTTON A BLIND PERSON CANNOT USE ──────────────────
   Max, 12 Sep 2026, from a LIVE audit of production: the collapsed gallery exposed four unnamed
   buttons and the expanded gallery five. Both loops wrap the image in a <button> and set
   `alt=""` on the image — correct, because the image is decorative once the button is named, and
   fatal because the button was never named. A screen reader announces "button" and nothing else,
   so the entire photo gallery is an undifferentiated row of unlabelled controls.

   The name goes on the BUTTON rather than the image: the accessible name of a control should say
   what the control does, and what these do is open one photo out of a set. `alt=""` stays.

   Seven languages, not two. `W` is the two-language shortcut and it is how German fell back to
   English on the photo menu (Max C-4); a string added today to a shared component should not
   repeat that. */
const PHOTO_LABEL: Localized = {
  en: "Open photo {n} of {total}",
  es: "Abrir foto {n} de {total}",
  co: "Abrir foto {n} de {total}",
  de: "Foto {n} von {total} öffnen",
  ru: "Открыть фото {n} из {total}",
  zh: "打开第 {n} 张照片，共 {total} 张",
  pt: "Abrir foto {n} de {total}",
};
const photoLabel = (lang: string, i: number, total: number): string =>
  WA(lang, PHOTO_LABEL).replace("{n}", String(i + 1)).replace("{total}", String(total));

export default function Gallery({
  photos, videos = [], lang, alt = "", footer, onOpenChange,
}: {
  photos: string[];
  /** ⚠️ ONE GALLERY, NOT TWO SECTIONS. Lee, 16 Sep 2026: *"Instead of having two categories at
   *  the top where we've got videos and then photos, let's just combine them. Videos show first,
   *  then photos. That way we don't have two sections for people to scroll into."*
   *  He is right, and the old shape actively hid the videos: a host's walkthrough sat in its own
   *  strip that you had to notice, above a gallery that looked like the whole gallery. Now the
   *  videos are simply the first slides. Swipe past them and you are in the photographs; there is
   *  nothing to discover and nothing to scroll INTO. A listing with no video is unchanged. */
  videos?: string[];
  lang: string;
  alt?: string;
  ...
  /* Videos first, then photographs, in one flat list. Everything below — the counter, the
     swipe strip, the grid and the fullscreen viewer — works off THIS, so there is exactly one
     ordering in the component and no way for two views to disagree about what slide three is. */
  const items = [
    ...videos.map(src => ({ src, video: true })),
    ...photos.map(src => ({ src, video: false })),
  ];
  /* ── A LISTING WITH NO MEDIA STILL HAS A GALLERY-SHAPED HOLE ────────────────────────────
     Lee, 17 September 2026, looking at a screenshot: *"there's an empty space there as if there
     should be a picture… you have the heart and the share button, but there's nothing to heart
     or share."*

     This used to `return null`, so on a listing with no media the page collapsed and the row of
     actions underneath sat against whatever happened to be above it — which reads as a broken
     layout rather than as an empty one. Holding the same 4:3 box the cover strip holds keeps
     the page the same shape in both states and says plainly what is missing.

     Worth saying: a published listing cannot reach this. The listing form refuses to publish
     without at least one photograph. This is the draft and the fixture case — but those are
     exactly the states a host sees while they are still working, so it should not look broken
     to them either. */
  if (items.length === 0) {
    return (
      <div>
        <div className="ow-edge flex aspect-[4/3] w-full flex-col items-center justify-center gap-1 rounded-2xl border border-dashed bg-ink/[0.03] dark:bg-white/[0.03]">
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="opacity-30" aria-hidden>
            <rect x="3" y="4" width="18" height="16" rx="2.5" />
            <circle cx="8.5" cy="9.5" r="1.6" />
            <path d="M3.5 17l4.8-4.8a2 2 0 0 1 2.8 0L16 17" />
          </svg>
          <span className="text-[12.5px] font-bold opacity-45">{W(lang, "No photos yet", "Aún no hay fotos")}</span>
        </div>
      ...
      {items.length > 1 && (
        <div className="mb-2 flex items-center justify-between gap-2">
          <span className="text-[12px] font-bold opacity-50">
            {videos.length > 0
              ? `${videos.length} ${W(lang, videos.length === 1 ? "video" : "videos", videos.length === 1 ? "video" : "videos")} · ${photos.length} ${W(lang, "photos", "fotos")}`
          ...
          <div className="flex items-center gap-1.5">
            ...
            {mode === "grid" && (
              <div className="flex overflow-hidden rounded-full border ow-edge">
 ...
 className="ow-edge ow-tap flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[12px] font-bold ">
              {mode === "cover" ? (
                <>
      ...
      {mode === "cover" && (
        <div className="relative overflow-hidden rounded-2xl border border-ink/[0.08] dark:border-white/10">
          <div
            ...
            {items.map((it, i) => (
              <button key={it.src} type="button" onClick={() => open(i)}
                aria-label={it.video ? W(lang, "Play video", "Reproducir video") : photoLabel(lang, i - videos.length, photos.length)}
                className="ow-tap relative block h-full w-full shrink-0 snap-center">
                {it.video ? (
                  <>
                    {/* Muted, looping, and playing only while it is the slide on screen — the
                        same rule the feed uses. A listing with five videos must not stream five
                        of them because somebody opened the page. */}
                    <DeckVideo src={it.src} />
                  ...
                  <img decoding="async" src={it.src} alt={i === 0 ? alt : ""} loading={i <= 1 ? "eager" : "lazy"}
      ...
      {mode === "grid" && (
        <div className="grid gap-1.5"
          ...
          {/* ⚠️ NOT A UNIFORM GRID ANY MORE. Lee, 16 Sep 2026: *"I think we should stagger the
              size of the pictures, kind of like how Airbnb does now, where they show some
              pictures larger, some pictures smaller."*

              Every fourth tile takes two columns and two rows. Two reasons it is a rhythm rather
              than a judgement: a fixed pattern cannot decide wrongly which of somebody's photos
              matters, and it stays stable as they add and remove photographs — a layout that
              reshuffles itself when you upload one more picture feels broken.

              Videos are always big. They are the thing a host went to the trouble of filming,
              and a walkthrough shrunk to a thumbnail is a walkthrough nobody plays.
              ⚠️ Only at 3 and 4 across. At 2 across a double-width tile IS the whole row, which
              is not a stagger, it is a list. */}
          {items.map((it, i) => {
            const big = cols > 2 && (it.video || i % 4 === 0);
            return (
              <button key={it.src} type="button" onClick={() => open(i)}
                aria-label={it.video ? W(lang, "Play video", "Reproducir video") : photoLabel(lang, i - videos.length, photos.length)}
                style={big ? { gridColumn: "span 2", gridRow: "span 2" } : undefined}
 className="ow-edge ow-tap relative overflow-hidden rounded-xl border border-ink/[0.08] ">
                {it.video
                  ? <DeckVideo src={it.src} />
                  : <img decoding="async" src={it.src} alt="" loading={i < 6 ? "eager" : "lazy"}
                      className="h-full w-full object-cover" style={{ aspectRatio: big ? "1 / 1" : "1 / 1" }} />}
                {it.video && (
                  <span className="pointer-events-none absolute inset-0 grid place-items-center">
                    <span className="grid h-11 w-11 place-items-center rounded-full bg-black/45 text-white"><PlayGlyph /></span>
                  </span>
                )}
              </button>
            );
      ...
      {viewer !== null && (
        <Viewer photos={items.map(i => i.src)} videoCount={videos.length} start={viewer} lang={lang} alt={alt}
...
function Viewer({
  photos, videoCount = 0, start, lang, alt, onClose, footer,
}: {
  photos: string[]; videoCount?: number; start: number; lang: string; alt: string;
        ...
        {photos.map((src, n) => (
          <div key={n} className="flex h-full w-full shrink-0 snap-center items-center justify-center px-2">
            ...
            <PinchZoom onZoomChange={setZoomed}>
              {n < videoCount
                ? <video src={src} controls playsInline loop autoPlay={n === i}
                    className="max-h-full max-w-full object-contain" />
                : <img decoding="async" src={src} alt={n === 0 ? alt : ""} loading={Math.abs(n - i) <= 2 ? "eager" : "lazy"}
                    className="max-h-full max-w-full object-contain" />}
            </PinchZoom>
          </div>
        ))}
...
/* A tile video: silent, looping, and playing only while it is actually on screen. */
function DeckVideo({ src }: { src: string }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) void el.play().catch(() => undefined); else el.pause();
    }, { threshold: 0.6 });
    io.observe(el);
    return () => { io.disconnect(); el.pause(); };
  }, [src]);
  return <video ref={ref} src={src} muted playsInline loop preload="metadata"
    className="h-full w-full object-cover" />;
}

/* Inline so the gallery keeps its no-icon-library rule. */
function PlayGlyph() {
