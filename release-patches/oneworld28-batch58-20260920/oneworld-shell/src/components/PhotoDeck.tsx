...
export default function PhotoDeck({
  ...
  async function add(files: FileList | null) {
    ...
    const one = async (i: number) => {
      /* Derivatives + the `-t` thumbnail + the public URL — one shared path, `uploadPhoto` in
         lib/mediaUpload.ts, so the VAIA-generated event flyer lands in storage exactly the way a
         chosen photograph does. Its comment carries the blank-canvas history from 12 Aug. */
      const r = await uploadPhoto(queue[i], uid, folder);
      if ("error" in r) { fatal = r.error; return; }
      out[i] = r.url;
    };
...
function Thumb({ src }: { src: string }) {
  ...
  return (
    <img decoding="async" src={url} alt="" loading="lazy" draggable={false} className="h-full w-full object-cover"
