 ...
 */
export default function FeedVideoPlayer({ slides, start, poster, title, href, lang, onClose, cta }: {
  ...
  /** Where the right-hand button goes. Omit it on the item's own page — there Back is the only
     way out that means anything, and it takes the full width. */
  poster?: string | null; title: string; href?: string; lang: string; onClose: () => void;
  /** The right-hand button's label. Defaults to "View listing"; OneEvent passes "View event". */
  cta?: string;
}) {
  ...
  /* ⚠️ PORTALLED TO <body>. `position: fixed` is measured against the nearest TRANSFORMED
     ancestor, not the viewport, and OneEvent's page column has one — so the "full-screen"
     player opened a few pixels short of the top with the app header showing through. Same
     class of bug PhotoDeck's sheet hit on 11 Aug; same fix. */
  return createPortal(
    <div className="fixed inset-0 z-[2000] bg-black" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <button type="button" onClick={onClose}
        className="ow-tap absolute z-10 right-3 top-3 grid h-11 w-11 place-items-center rounded-full bg-black/45 text-white"
        aria-label={W(lang, "Close", "Cerrar")}><IconX size={20} /></button>
      ...
      {at> 0 && (
        <button type="button" onClick={() => go(at - 1)}
          className="ow-tap absolute z-10 left-1 top-1/2 grid h-14 w-14 -translate-y-1/2 place-items-center text-white/70"
          aria-label={W(lang, "Previous", "Anterior")}><Chevron dir="left" size="lg" /></button>
      )}
      {at < slides.length - 1 && (
        <button type="button" onClick={() => go(at + 1)}
          className="ow-tap absolute z-10 right-1 top-1/2 grid h-14 w-14 -translate-y-1/2 place-items-center text-white/70"
          aria-label={W(lang, "Next", "Siguiente")}><Chevron dir="right" size="lg" /></button>
      )}
      ...
      {/* ⚠️ ABOVE THE SCRUBBER, NOT ON IT. On a video slide the browser draws its own control
          bar along the bottom edge (play, time, mute, full screen), and Back was sitting on top
          of the mute button — caught in the OneEvent 30 screenshots, 20 Sep 2026. A photo slide
          has no bar, so the pair drops back to the edge. */}
      <div className="absolute inset-x-0 bottom-0 flex items-center gap-3 p-4"
        style={{ paddingBottom: `calc(${current?.kind === "video" ? "4.5rem" : "1rem"} + env(safe-area-inset-bottom, 0px))` }}>
        <button type="button" onClick={onClose}
          ...
          <Chevron dir="left" size="sm" />{W(lang, "Back", "Atrás")}
        </button>
        {href && (
          <Link to={href}
            ...
            {cta ?? W(lang, "View listing", "Ver anuncio")}<Chevron dir="right" size="sm" />
          </Link>
        )}
      </div>
    </div>,
