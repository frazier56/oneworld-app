/* OneHome's own words in the languages beyond English and Spanish. Imported for the side effect
   of registering them — see `i18n/copy.ts`. It sits on the product's route entry so the dictionary
   is present before the first OneHome screen paints, and so no other product pays for it. */
import "./i18n/copy";
import "./i18n/copyRuZh";
import { lazy, Suspense } from "react";
import { Route, Outlet } from "react-router-dom";
import {
  CONFIGS, MessagesScreen, ProfileScreen, PublicWorld, SettingsScreen, PlansScreen, lazyScreen, ScreenFallback} from "@oneworld/shell";
...
const ManageStay = lazyScreen(() => import("./screens/ManageStay"));
...
export const oneRentalChildRoutes = (
  <Route element={<RentalProviders />}>
    ...
    {/* The host's controls for a stay that is already running — discount, cleaning fee,
        extension. Its own route rather than a panel on the contract, because the contract is
        the SAME screen for both parties by design and these three controls are not. */}
    <Route path="c/:id/manage" element={<ManageStay />} />
