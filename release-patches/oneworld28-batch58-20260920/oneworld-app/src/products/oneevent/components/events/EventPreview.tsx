...
export default function EventPreview({ formData, onBack, onPublish }: Props) {
  ...
  /* `coverImageUrl` is derived from the chosen cover photo (OneEvent 30); the decks upload as
     you choose, so there is no local File to preview any more. */
  const coverDisplayUrl = formData.coverImageUrl;
          ...
          return (
            <div className="relative rounded-2xl overflow-hidden mb-6 bg-black/40 mx-auto" style={{ aspectRatio: heroAspect, maxHeight: isFlyer ? "min(72vh, 560px)" : undefined }}>
              {hasCover ? (
                <>
                  {isFlyer && (
                    <img decoding="async" src={coverDisplayUrl} alt="" aria-hidden="true" className="absolute inset-0 w-full h-full object-cover scale-110 blur-2xl opacity-50" />
                  )}
                  <img decoding="async"                     src={coverDisplayUrl}
                    alt={ce("Event cover")}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {allMedia.map((a, i) => (
                    <div key={i} className="rounded-xl overflow-hidden aspect-video bg-secondary">
                      <img decoding="async" src={a.url} alt={a.name} className="w-full h-full object-cover" />
                    </div>
                  ))}
                </div>
              </div>
...
>
              ...
              <div className="flex items-center gap-3 pr-16">
                <div className="w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0 bg-primary/15 border border-white/10">
                  {hostPhotoUrl ? (
                    <img decoding="async" src={hostPhotoUrl} alt={shownDisplayName} className="w-full h-full object-cover" />
                  ) : (
