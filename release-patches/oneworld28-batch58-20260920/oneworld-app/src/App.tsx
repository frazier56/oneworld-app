...
/* ── THE SANDBOX MOUNT (PUB30, 20 September 2026) ───────────────────────────────────────────
   The same build answers at two addresses on the SAME origin: `/` is production and `/next/` is
   the next version, which Lee looks at before it is promoted. One origin because One ID is a
   same-origin session — a staging hostname would force a second sign-in and would therefore not
   mirror production at all, which is the whole point of having it.

   This is the ONLY source change the mount needs. `index.html` at both addresses is byte for
   byte the same file, so the basename cannot come from the build — it is read from the address
   the page was served at, once, at mount. Anything else (the deep-link hop, the amber band that
   says "this is not production") lives in that one HTML file and never enters the bundle.

   `undefined` rather than `"/"`: react-router treats an explicit "/" as a basename and a missing
   one differently in `useResolvedPath` for a bare root, and production must keep the behaviour it
   has had since August. */
const SANDBOX_BASE = "/next";
