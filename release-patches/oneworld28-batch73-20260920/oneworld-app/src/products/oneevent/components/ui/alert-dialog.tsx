...
export function AlertDialogCancel({ className, onClick, children, ...p }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  const { setOpen } = React.useContext(C);
 return <button className={cn("inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 font-semibold ", className)} onClick={(e) => { onClick?.(e); setOpen(false); }} {...p}>{children}</button>;
}
