...
export default function Charge() {
 ...
 <button type="button" onClick={cancel} disabled={busy} className="ow-edge ow-tap mt-2 w-full rounded-xl border py-3 text-sm font-bold">{t("cancelAttempt")}</button>
            </div>
          )}
          {st === "pending" && attempt.method !== "transfer" && (
            <div className="mt-4"><p className="text-[12.5px] opacity-70">{t("startTap")}</p>
 {!attempt.provider_ref && <button type="button" onClick={cancel} disabled={busy} className="ow-edge ow-tap mt-3 w-full rounded-xl border py-3 text-sm font-bold">{t("cancelAttempt")}</button>}
            </div>
          )}
  ...
  if (order) {
    ...
    return (
      <div className="px-4 pb-10">
        ...
        {err && <p role="alert" className="mt-3 text-[12.5px] font-bold text-rose-600 dark:text-rose-300">{err}</p>}
 <button type="button" onClick={reset} disabled={busy} className="ow-edge ow-tap mt-4 w-full rounded-xl border py-3 text-sm font-bold">{t("void")}</button>
      </div>
    );
  }
            ...
            {lines.map(l => (
              <li key={l.key} className="flex items-center justify-between gap-2 text-[13px]">
                ...
                <span className="flex items-center gap-1">
                  <button type="button" aria-label="−" onClick={() => setQty(l.key, l.qty - 1)} className="ow-tap h-8 w-8 rounded-full border ow-edge">−</button>
