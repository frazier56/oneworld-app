...
/* ⚠️ A SWITCHED-OFF CONTROL HAS TWO JOBS AND THEY PULL AGAINST EACH OTHER: it must stay
   READABLE — Lee, 17 September: *"the buttons are very faded, you can't see it"* — and it must
   not look TAPPABLE, or somebody presses it and nothing happens, which reads as a broken app.

   Fading it to 40% failed the first. Simply un-fading it failed the second: with the brand
   colour and a brand edge, an unavailable button looked exactly like an available one.

   So the two jobs are split across two properties. CONTRAST stays high, so it is legible. The
   ACTION CUE — the brand colour on the label, the icon and the edge — is what goes away, to a
   plain neutral. You can read what it is; you can see it is not for you right now; and the line
   underneath says why. */
function Row({ icon, label, onClick, disabled, tone }: {
  icon: React.ReactNode; label: string; onClick: () => void; disabled?: boolean;
  tone: "espresso" | "request";
}) {
  const base = "w-full disabled:cursor-not-allowed";
  const paint = tone === "espresso"
    ? "btn-primary disabled:opacity-55"
    : disabled
      ? "ow-tap w-full rounded-2xl border border-ink/15 bg-transparent px-4 py-3 text-[14px] font-black text-ink/40 dark:border-white/15 dark:text-white/40"
      : "ow-tap w-full rounded-2xl border border-brand/45 bg-brand/[0.10] px-4 py-3 text-[14px] font-black text-brand-deep dark:text-brand-light";
  return (
    <button type="button" onClick={onClick} disabled={disabled} className={`${paint} ${base}`}>
