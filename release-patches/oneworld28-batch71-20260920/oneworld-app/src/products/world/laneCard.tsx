 ...
 */
export function SlideEngagement({ card, lang, href, title, onComments }: {
 ...
 *
 * The heart already writes `saved_items` as a side effect when a card passes `savesAs`, which is
 * right for a listing card and wrong here: on a reel rail, like and save are two different
 * intentions and Instagram gives them two different controls. This is the second one — it writes
 * the same `saved_items` row, so a save made here is the save the Saved screen lists.
 */
export function RailSave({ card, lang }: { card: LaneCard | undefined; lang: string }) {
  ...
  const toggle = async () => {
    ...
    const q = next
      ? supabase.from("saved_items").upsert(
          { user_id: userId, item_type: kind, item_id: card.id },
          { onConflict: "user_id,item_type,item_id", ignoreDuplicates: true })
      : supabase.from("saved_items").delete()
          .eq("user_id", userId).eq("item_type", kind).eq("item_id", card.id);
    const { error } = await q;
  ...
  return (
    <button type="button" onClick={() => void toggle()} aria-pressed={on}
      className="ow-tap grid place-items-center text-white"
