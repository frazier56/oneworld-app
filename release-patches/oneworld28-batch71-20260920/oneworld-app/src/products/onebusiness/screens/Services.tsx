...
function ServiceDetail({ business, service, subscription, refresh }: { business: Business; service: Service; subscription?: Subscription; refresh: () => void }) {
 ...
 <button type="button" disabled={busy} onClick={() => void run(() => cancelRequest(subscription.id), "withdraw")} className="ow-edge ow-tap mt-3 rounded-xl border px-3 py-2 text-sm disabled:opacity-50">{t(busy ? "loading" : "cancelRequest")}</button>
        </>}
