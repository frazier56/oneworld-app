...
export default function ListingEngagement({
   ...
   */
  savesAs?: "rental_property" | "sale_property" | "event" | "job" | "profile";
  lang: string;
  compact?: boolean;
  /**
  ...
  const btn = `ow-tap flex min-h-[44px] rounded-full font-bold transition ${vertical ? "h-[38px] w-[38px] min-h-[38px] flex-col items-center justify-center gap-0 px-0 text-[10px] border border-white/30 bg-black/40 backdrop-blur-md" : "items-center gap-1.5 px-3 py-2"}`;

  return (
    <div>
      <div className={`flex gap-1 ${vertical ? "flex-col items-center" : "items-center"} ${size} ${onDark ? "text-white" : ""}`}>
