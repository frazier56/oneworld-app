...
export function RentalTiles({ publicView = false }: { publicView?: boolean }) {
      ...
      return (
        ...
        <div key={`m-${it.id}`} className="relative overflow-hidden rounded-xl border border-ink/[0.08] dark:border-white/10">
          {(it.thumbnail_url ?? it.media_url)
            ? <img decoding="async" src={it.thumbnail_url ?? it.media_url} alt="" loading="lazy"
 ...
 className="ow-edge ow-tap relative block overflow-hidden rounded-xl border border-ink/[0.08]">
        ...
        {coverOf(it)
          ? <img decoding="async" src={thumbFor(coverOf(it)!)} alt="" className="aspect-[4/5] w-full object-cover" loading="lazy"
  ...
  const chips = (
    ...
    <div className="relative z-[1] -mx-1 mb-2.5 flex gap-1.5 overflow-x-auto px-1 py-1"
      ...
      {TABS.map(t => {
        const on = t.id === tab;
        return (
          <button key={t.id} type="button" role="tab" aria-selected={on} onClick={() => setTab(t.id)} className={`ow-tap flex min-h-[40px] shrink-0 items-center rounded-xl px-3.5 text-[12.5px] font-bold transition ${ on ?"bg-ink text-paper dark:bg-white dark:text-ink" :"border ow-edge opacity-70"}`}>
            {t.label}
 ...
 className="ow-edge ow-tap mt-2.5 flex min-h-[44px] w-full items-center justify-center gap-1.5 rounded-xl border text-[13px] font-bold">
...
function MediaWallSheet({
 ...
 className="ow-edge ow-tap inline-flex min-h-[40px] items-center gap-1 rounded-full border py-1.5 pl-2 pr-3.5 text-sm font-semibold">
 ...
 className="ow-edge ow-tap mt-3 min-h-[44px] w-full rounded-xl border text-[13px] font-bold">
            {moreLabel}
          </button>
        )}
      </div>
