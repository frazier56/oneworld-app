...
export default function ListProperty({ onClose, editId }: { onClose?: () => void; editId?: string | null } = {}) {
  ...
  /* Twin forms, twin fix: the rent form seeded this with "Medellín" and so did this one, so a
     seller who skipped the field published a Medellín listing. Lee, 14 Sep 2026: *"Don't influence
     people."* Empty, with the city named in the placeholder rather than answered for them. */
  ...
  /* Same two readers as the rent form: Google returns "Medellín, Antioquia, Colombia" as a
     prediction's second line, so picking a building or an address answers the city and the
     department underneath it. Neither ever overwrites something already typed. */
  const regionFrom = (secondary?: string) => {
    const parts = (secondary ?? "").split(",").map(x => x.trim()).filter(Boolean);
    const noCountry = parts[parts.length - 1]?.toLowerCase() === "colombia" ? parts.slice(0, -1) : parts;
    return noCountry.length>= 1 ? noCountry[noCountry.length - 1] : "";
  };
  const fillPlaceBelow = (secondary?: string) => {
    const parts = (secondary ?? "").split(",").map(x => x.trim()).filter(Boolean);
    const noCountry = parts[parts.length - 1]?.toLowerCase() === "colombia" ? parts.slice(0, -1) : parts;
    if (noCountry.length>= 2 && !cityText.trim()) setCityText(noCountry[noCountry.length - 2]);
    if (noCountry.length>= 1 && !regionText.trim()) setRegionText(noCountry[noCountry.length - 1]);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <FormSection icon={I.where} title={W(lang, "Where it is", "Dónde queda")}>
        {/* ── THE SAME ORDER AS THE RENT FORM, FOR THE SAME REASON ─────────────────────────
            Lee, 12 Aug 2026: *"These are twin forms — literally copy the code and make them
            identical when they have the same things."* And 14 Sep 2026, on the rent form:
            *"Building name, then address, then neighbourhood, then city."* Applying it here
            without being asked twice, because it is the same screen wearing a different hue. */}
        ...
        <Field label={W(lang, "Address", "Dirección")}
          ...
          <PlacesInput variant="address" countries={["co"]} bias={cityLabel}
            ...
            onSelectParts={({ name, address: addr, full }) => { setAddressLine(name || full); fillPlaceBelow(addr); }}
            placeholder={W(lang, "e.g. Carrera 43A #7-50", "Ej.: Carrera 43A #7-50")} />
        </Field>
        {/* Google Places on every location field — Lee's global rule, 14 Sep 2026. Same control as
            the rent side and the filter sheet: searchable predictions plus the curated barrios as
            one-tap chips. A datalist is not autocomplete; on a phone it usually never opens. */}
        ...
        <Field label={W(lang, "City", "Ciudad")}
          ...
          <PlacesInput variant="city" countries={["co"]}
            ...
            onSelectParts={({ name, address: addr }) => { setCityText(name.split(",")[0].trim()); setRegionText(regionFrom(addr)); }}
