...
export default function HostSearch() {
 ...
 <button key={k} onClick={() => setSort(k)} className={`ow-edge rounded-full px-3.5 py-1.5 text-sm font-semibold transition ${sort === k ? "bg-teal text-white" : "border "}`}>{l}</button>
        ))}
      </div>
        ...
        <div className="space-y-3">
          {filtered.map(h => (
            <button key={h.id} onClick={() => nav(`/events/p/${h.id}`)} className="card flex w-full items-center gap-3 !rounded-3xl p-3 text-left transition active:scale-[.99]">
              {h.photo_url
                ? <img decoding="async" src={h.photo_url} alt="" className="h-14 w-14 shrink-0 rounded-2xl object-cover" />
