...
const CurFlag = ({ code }: { code: string }) => (
  <img decoding="async" src={`https://flagcdn.com/w40/${CUR_CC[code] ?? "us"}.png`} alt=""
...
export default function ContractForm({ recipientId, recipientName, initialRole, conversationId, initialDraftId, autoPreview, onClose, onSent }:
 ...
 <button onClick={() => setPreviewOpen(false)} className="ow-edge mb-3 flex h-10 items-center gap-1 rounded-full border pl-2 pr-3.5 text-sm font-bold">‹ {W(lang, "Back to edit", "Volver a editar")}</button>
    ...
    <div className="oj-create-page fixed inset-0 z-30 overflow-y-auto bg-paper dark:bg-ink">
      <div className="mx-auto min-h-full w-full max-w-lg px-4 pb-40 pt-20">
        ...
        <div className="mb-4 flex items-center justify-between gap-3 rounded-2xl bg-paper/70 px-2 py-2 backdrop-blur dark:bg-ink/60">
          <button onClick={onClose} className="flex h-10 items-center gap-1 rounded-full border ow-edge bg-paper/80 pl-2 pr-3.5 text-sm font-bold transition active:scale-[.98] dark:bg-white/10">
                    ...
                    <div className="mt-1 overflow-hidden rounded-xl border border-ink/10 dark:border-white/10">
                      {cpResults.map(r => (
                        <button type="button" key={r.id}
                          ...
                          {r.photo_url
                            ? <img decoding="async" src={r.photo_url} alt="" className="h-7 w-7 shrink-0 rounded-full object-cover" />
