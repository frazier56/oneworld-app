  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {MODES.map(m => (
          <button key={m.id} onClick={() => setMode(m.id)} className={`flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[13px] font-bold transition ${ mode === m.id ?"bg-teal text-white" :"border ow-edge"}`}>
            {m.icon}{m.label}
          </button>
        ))}
      </div>
      ...
      <div className="card space-y-3 p-4">
        <textarea value={caption} onChange={e => setCaption(e.target.value)} rows={mode ==="write" ? 6 : 3} placeholder={mode ==="live" ? W(lang,"What's your live about?","¿De qué trata tu directo?") : W(lang,"Say something…","Di algo…")} className="w-full resize-none rounded-xl border ow-edge bg-transparent px-3 py-2.5 text-[14px] outline-none placeholder:opacity-50" />
