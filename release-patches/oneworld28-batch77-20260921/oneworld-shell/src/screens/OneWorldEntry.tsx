...
/**
 * THE WAY OUT OF YOUR WORLD — Lee, 21 September 2026.
 * ============================================================================================
 * *"There should also be a back button on this page. If you're on this Your World page, there's
 * nowhere to go but forward. And you should also have a link that takes you to the feed —
 * somewhere near the top, I would assume."*
 *
 * Both, on one row above the language picker: back on the left where a back control belongs, the
 * feed on the right. Only this screen renders it — see `topSlot` in Splash.
 *
 * ── THE BACK BUTTON HAS TO COPE WITH HAVING NO BACK ─────────────────────────────────────────
 * This screen is now reached from the hamburger, so usually there IS somewhere behind it. But it
 * is also a real address somebody can open cold, and `navigate(-1)` from the first entry in a
 * session does nothing at all — a button that silently does nothing is worse than no button.
 * React Router marks that first entry with `key === "default"`, so when there is no history the
 * control goes to the feed instead, which is where this person was heading anyway.
 */
function EntryWayOut() {
  const navigate = useNavigate();
  const location = useLocation();
  const { lang } = useI18n();
  const canGoBack = location.key !== "default";
  return (
    <div className="mb-4 flex items-center justify-between gap-3 text-[13.5px] font-bold">
      <button type="button"
        onClick={() => (canGoBack ? navigate(-1) : navigate(WORLD_FEED_HOME))}
        className="ow-tap inline-flex items-center gap-1 rounded-full px-1 py-1.5 opacity-70">
      ...
      <Link to={WORLD_FEED_HOME}
        className="ow-tap inline-flex items-center gap-1.5 rounded-full border ow-edge px-3 py-1.5">
        {W(lang, "One World feed", "Feed de One World")}
