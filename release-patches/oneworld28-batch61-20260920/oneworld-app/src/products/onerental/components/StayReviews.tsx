...
export function StayReviewTasks({ lang }: { lang: string }) {
  ...
  return <section id="reviews" className="space-y-3"><h2 className="text-base font-bold">{es ? "Reseñas después de su estadía" : "Reviews after your stay"}</h2>
    {current?.error ? <div role="alert" className="card p-4"><p className="text-[13.5px] font-semibold">{es ? "No se pudieron cargar las estadías." : "Could not load completed stays."}</p><button type="button" className="btn-ghost mt-2" onClick={() => setTick(x => x + 1)}>{es ? "Reintentar" : "Try again"}</button></div>
