...
export default function Wordmark({
  ...
  return (
    ...
    <span className="inline-flex select-none items-end notranslate" translate="no" data-no-translate>
      <img decoding="async"         /* BASE_URL, because the app may not be served from the domain root. OneJob had this and
      ...
      <span className="flex min-w-0 flex-col justify-center text-left" style={{ lineHeight: 1, marginBottom: h * COLUMN_LIFT }}>
        ...
        <span className="overflow-hidden whitespace-nowrap font-semibold uppercase text-brand"
          style={{
            ...
            /* ⚠️ THE TAGLINE WAS BEING SLICED ALONG ITS BASELINE, IN EVERY PRODUCT.
               The column sets `lineHeight: 1` and this span sets `overflow-hidden` (which it
               needs, so a nowrap tagline can never widen the lockup). At line-height 1 the LINE
               BOX is shorter than the glyphs' ink, and overflow-hidden then cuts the bottom row
               of pixels off every capital — measured on the live header: font-size 5.51px,
               line-height 5.51px, clientHeight 6. `scrollHeight` reports no overflow, because ink
               overflow is not scrollable overflow, which is exactly why this survived a
               measurement pass and was only ever visible in a screenshot.
               1.35 gives the ink its room; the lockup has 36px of unused height to spend. */
            lineHeight: 1.35,
