  ...
  return createPortal(
    <div className="fixed inset-0 z-[90] grid place-items-end sm:place-items-center">
      ...
      <div
        ...
        className="relative max-h-[90svh] w-full max-w-lg overflow-y-auto overscroll-contain glass-modal rounded-t-3xl p-6 shadow-2xl sm:m-4 sm:rounded-3xl"
...
>
        <button onClick={onClose} aria-label="Close" className="absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-full border ow-edge">×</button>
        ...
        {items.length> 1 && (
          <div className="mt-5 flex items-center justify-between">
            <button disabled={index === 0} onClick={() => onIndex(index - 1)} aria-label="Previous" className="grid h-9 w-9 place-items-center rounded-full border ow-edge disabled:opacity-30">
            ...
            <button disabled={index === items.length - 1} onClick={() => onIndex(index + 1)} aria-label="Next" className="grid h-9 w-9 place-items-center rounded-full border ow-edge disabled:opacity-30">
