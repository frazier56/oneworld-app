...
/* Lee, 21 Sep 2026: go back to the previous cut. v7 (narrated, seven languages) is withdrawn
   from the opener; v5 is the film again. The v7 files stay in the repo, unreferenced, so this
   is one constant away from being switched back. */
/* v5.1 — v5 with the Skip pill removed from the picture; the app draws the only one. It ships
   under its OWN name because staging is add-only: the original v5 bytes are already in the repo
   at `opener/one-world-opener-v5.mp4`, so writing over that path is a hard stop, and it would
   also break rollback for every batch still pointing at the old file. */
const FILM = "opener/one-world-opener-v5-1.mp4";
const KEY = "ow.opener.v51.seen";
