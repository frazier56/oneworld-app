...
export default function OrderDetail() {
 ...
 <button type="button" disabled={busy} onClick={() => run(() => cancelAttempt(a.id))} className="ow-edge ow-tap flex-1 rounded-xl border text-[12.5px] font-bold">{t("cancelAttempt")}</button>
              </div>
            )}
          </li>
        ))}
 ...
 <button type="button" disabled={busy} onClick={() => run(() => voidOrder(order.id))} className="ow-edge ow-tap mt-4 w-full rounded-xl border py-3 text-sm font-bold">{t("void")}</button>
      )}
