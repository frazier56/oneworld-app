...
export default function MyRequestCard({ propertyId, userId, lang }: { propertyId: string; userId: string; lang: string }) {
  ...
  const req = useAsync(async () => {
    let query = supabase.from("rental_booking_requests")
      .select("id, starts_on, ends_on, nights, quoted_total, guest_total, currency, state, approval_stage, identity_status, payment_rail, payment_status, guest_fee_rate, payment_declared_at, payment_received_at, preapproval_expires_at, expires_at, check_in_time, check_out_time")
    ...
    <section aria-label={W(lang, "Your request", "Su solicitud")}
      ...
      <p className="mt-2 text-[12px] font-bold">
        {/* The sentence and the number come from the SAME subtraction. It used to say "no fee"
           when the RATE was zero while printing a fee derived from the TOTALS, so the two could
           disagree on one line about the same money. Same principle as the ranking explanation:
           an explanation computed from a different source than the thing it explains is the one
           artefact somebody reaches for when they are already confused.

           (`guest_fee_rate` is NOT NULL in the database and is in the select, so this is not the
           Number(null) === 0 trap — checked before changing it. This is about agreement.) */}
        {(() => {
      ...
      {/* ── ⚠️ WHAT YOU ACTUALLY ASKED FOR (17 September 2026) ────────────────────────────
          Lee: *"the only valuable piece is that when you submit a request, you know you asked for
          this start date, this end date and time."* The card carried dates and money and never a
          time, because until today a request had no check-in or check-out time to carry. It does
          now, and it is the SAME component the sheet used before this was sent, so the numbers
          cannot read one way before and another way after. */}
      <div className="mt-3">
        <StayFactsTable lang={lang} money={money}
          facts={{ starts_on: req.starts_on, ends_on: req.ends_on, nights: req.nights,
            check_in_time: req.check_in_time, check_out_time: req.check_out_time,
            guest_total: req.guest_total, currency: req.currency }} />
      </div>

      {/* ── ⚠️ TWO FACTS, TWO SENTENCES, THEN THE ADVICE ──────────────────────────────────
          "Pending approval" is about the HOST. "Payment not confirmed" is about the MONEY. Joined
          on one line they read as a single status and a tenant cannot tell which half is theirs
          to act on — which is exactly how somebody sends rent to a host who has not accepted. */}
      <p className="mt-3 text-[13px] leading-relaxed">{req.state === "accepted" ? D(lang, "acceptedByHost") : D(lang, "notAcceptedYet")}</p>
      <p className="text-[13px] leading-relaxed">{received ? D(lang, "paymentIsIn") : D(lang, "noPaymentYet")}</p>
      {req.state !== "accepted" && !received && !declared && !expired && (
        <p className="mt-1 text-[13px] font-black">{D(lang, "waitToPay")}</p>
      )}
      {expired
        ? <p className="mt-1 text-[12px] font-bold opacity-75">{D(lang, "requestLapsed")}</p>
        : req.state === "requested" && <p className="mt-1 text-[12px] opacity-65">{D(lang, "respondBy", { when: when(req.expires_at) })}</p>}
      <p className="mt-1 text-[11.5px] opacity-55">{D(lang, "requestInChat")}</p>
