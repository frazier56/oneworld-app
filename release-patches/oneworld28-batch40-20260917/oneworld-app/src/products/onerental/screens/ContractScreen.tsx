...
export default function ContractScreen() {
  ...
  const contractPath = `/rentals/r/${id}/contract${selectedRequest ? `?request=${selectedRequest}` : ""}`;
  ...
  const jumpToBlank = (key: string) => {
    if (HOST_PROFILE_KEYS.has(key)) { nav(hostProfilePath); return; }
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      ...
      setErr(W(lang,
        "That detail is not on this screen. Open the agreement below and fill it there.",
        "Ese dato no está en esta pantalla. Abra el contrato abajo y complételo allí."));
    }));
  };
  ...
  const tv = useMemo(() => ({
    ...
    ...f,
    direccion: f.direccion || formatStoredAddress(property?.address_line ?? "") || "",
  ...
  const onScreenBlanks = activeMissingKeys.filter(k => !HOST_PROFILE_KEYS.has(k));
  ...
  async function send() {
    ...
    if (activeMissingKeys.length) {
      ...
      const goingToProfile = HOST_PROFILE_KEYS.has(first);
      setErr(W(lang,
        goingToProfile
          ? `Complete the ${activeMissingKeys.length} required field${activeMissingKeys.length === 1 ? "" : "s"}. The first one lives on your host profile — opening it now, and Save brings you back here.`
          : `Complete the ${activeMissingKeys.length} required field${activeMissingKeys.length === 1 ? "" : "s"}. The first one is ready for you below.`,
        goingToProfile
          ? `Complete ${activeMissingKeys.length === 1 ? "el campo obligatorio" : `los ${activeMissingKeys.length} campos obligatorios`}. El primero está en su perfil de anfitrión — lo abrimos ahora, y al guardar volverá aquí.`
            ...
            <div className="rounded-xl border border-amber-400/40 bg-amber-400/[0.08] p-3">
              ...
              <p className="mt-1 text-[12px] leading-relaxed opacity-75">
                {hostProfileBlanks.length === 0
                  ? W(lang,
                      "They show as ____ below, and this cannot be sent until every one of them is filled.",
                      "Aparecen como ____ más abajo, y no se puede enviar hasta completarlos todos.")
                  : onScreenBlanks.length === 0
                    ? W(lang,
                        "They all come from your host profile — your legal name, your ID and your address for legal notices. Fill them there once and every agreement you write uses them.",
                        "Todos vienen de su perfil de anfitrión — su nombre legal, su documento y su dirección de notificaciones. Complételos allí una vez y todos sus contratos los usarán.")
                    : W(lang,
                        `${onScreenBlanks.length} ${onScreenBlanks.length === 1 ? "is" : "are"} filled on this screen. ${hostProfileBlanks.length} ${hostProfileBlanks.length === 1 ? "comes" : "come"} from your host profile — your legal name, your ID and your address for legal notices.`,
                        `${onScreenBlanks.length} se ${onScreenBlanks.length === 1 ? "completa" : "completan"} en esta pantalla. ${hostProfileBlanks.length} ${hostProfileBlanks.length === 1 ? "viene" : "vienen"} de su perfil de anfitrión — su nombre legal, su documento y su dirección de notificaciones.`)}
              </p>
              {onScreenBlanks.length > 0 && (
          ...
          <>
            ...
            {(editingLeaseDetails || !request) && <div className="space-y-3">
              ...
              <p className="rounded-xl border border-ink/10 bg-ink/[0.025] p-3 text-[11.5px] leading-relaxed opacity-70 dark:border-white/12 dark:bg-white/[0.04]">
                ...
                {f.direccion || formatStoredAddress(property?.address_line ?? "") || W(lang, "Complete this on the listing.", "Complétela en el anuncio.")}
              </p>
