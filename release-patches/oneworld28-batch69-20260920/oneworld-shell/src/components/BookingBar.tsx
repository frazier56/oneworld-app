...
export default function BookingBar({
  ...
  mode = "request", busy = false, earliestSelectableDate, existingStayNote, openSignal,
}: BookingBarProps & { openSignal?: number }) {
  const [open, setOpen] = useState(false);

  /* ⚠️ OPENED FROM OUTSIDE AS WELL AS FROM ITS OWN BUTTON (17 Sep 2026).
     The listing page now carries one action control — Message, Request a showing, Request a
     booking — and "Request a booking" has to reach THIS sheet, because a booking without dates
     is not a booking. A counter rather than a boolean: a boolean can only open the sheet once,
     and somebody who closes it and taps again expects it back. */
  useEffect(() => { if (openSignal) setOpen(true); }, [openSignal]);

  /* ── PUBLISH OUR OWN HEIGHT, so anything else pinned to the bottom can sit ABOVE us ───────
     `BottomTabs` already does exactly this with `--ow-tabs`, and this bar reads that variable to
     clear the footer. The listing's contact bar now has the same problem one level up: two
     sticky elements both anchored to `bottom: var(--ow-tabs)` land on top of each other. So this
     one measures itself the same way rather than anybody hard-coding a number that a translated
     label or a second line would immediately falsify. */
  const barRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    const el = barRef.current;
    if (!el) return;
    const publish = () => document.documentElement.style.setProperty("--ow-bookbar", el.offsetHeight + "px");
    publish();
    const ro = typeof ResizeObserver !== "undefined" ? new ResizeObserver(publish) : null;
    ro?.observe(el);
    return () => { ro?.disconnect(); document.documentElement.style.removeProperty("--ow-bookbar"); };
  }, []);
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/45"
          ...
          <div
            className="ow-booksheet max-h-[92svh] overflow-y-auto rounded-t-3xl bg-[var(--ow-paper,#fff)]
