...
export default function Staff() {
 ...
 {x.role !== "owner" && <button type="button" disabled={busy} onClick={() => run(() => setMemberActive(m.merchant!.id, x.user_id, !x.active))} className="ow-edge ow-tap rounded-full border px-3 py-1 text-[11.5px] font-bold">{x.active ? t("remove") : t("restore")}</button>}
          </li>
        ))}
      </ul>
    </div>
