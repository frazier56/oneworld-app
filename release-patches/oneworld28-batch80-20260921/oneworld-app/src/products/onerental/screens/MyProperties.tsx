...
/* PUB30 hand-merge, 22 Sep 2026. ONEHOME30 built this file on batch 57, where `coverOf`
   still lived in this product's own `lib/media`. OneEvent 30 has since moved the media
   helpers into `@oneworld/shell` and DELETED that local file, so the overlay's import
   pointed at nothing and would not have compiled. Every other screen in this product
   already imports it from the shell; so does this one. Nothing else in the overlay
   changed — the rest is ONEHOME30's work, byte for byte. */
...
export default function MyProperties() {
  ...
  return (
    ...
    <div className="space-y-4 pb-28">
      ...
      {/* ⚠️ THE TWO PAGES WERE CALLED DIFFERENT KINDS OF THING. Lee, 20 Sep 2026: *"Why do they
          have such different titles — my rental properties and listings for sale? It should be my
          properties for sale, my properties for rent."*

          One page said PROPERTIES and its twin said LISTINGS, so a host crossing between them by
          the toggle could not tell whether they were looking at the same kind of object under two
          names. These now match each other AND the hub panels that lead into them, which is the
          third place the same words have to agree. */}
      <ScreenHeading>
        {W(lang, "My properties for rent", "Mis inmuebles en arriendo")}
      </ScreenHeading>
      ...
      <div className="mt-4 space-y-3">
        ...
        {shown.map(p => {
          ...
          return (
            /* ⚠️ THIS WAS THE ONE SURFACE LEFT ON `.card` (Lee, 20 September 2026): *"the sales
               props screenshot looks better — slightly more rounded corners, and the panels have
               better contrast… we need to make both UIs look like the one on the sales side."*

               He is reading a real difference, not a mood. The sale listing card had already been
               moved to `.ow-panel` with a 24px radius; this one was still `.card`, whose light-mode
               fill is a gradient that fades out down the card and whose radius is 16px. Two pages
               one tap apart, one solid and rounder, the other softer and squarer. Same class, same
               radius, same padding as the sale card now. */
            <article key={p.id} className={`ow-panel relative overflow-visible rounded-3xl p-3 ${menuId === p.id ? "z-40" : "z-0"}`}>
              {/* `?owner=1` — this is the surface that grants the owner's view of a listing. The feed
                  deliberately does not. See PropertyDetail. */}
              <Link to={productHref("onerental", `/r/${p.id}?owner=1`)} className="ow-tap flex gap-3 pr-10">
                {coverOf(p)
