...
export default function Requests() {
  ...
  return (
    <div className="space-y-4 pb-28">
      {/* ⚠️ THERE WAS NO WAY BACK. Lee, 20 Sep 2026: *"you should also have a back button, I would
          think, just like you have on the other page where it says back to property tools. Once
          you go to your showings and requests, you should say back to my rental properties."*

          A host reaches this page from the portfolio — from the amber row at the top of it — so
          that is where back goes, and it says so by name. Same shape and same words as the link
          on `MyProperties`, because two back links that look different are two back links a person
          has to read. */}
      <Link to={productHref('onerental', '/properties')}
        className="ow-tap inline-flex items-center gap-1 text-[13px] font-bold opacity-65">
        <span aria-hidden>‹</span>{W(lang, 'Back to my properties for rent', 'Volver a mis inmuebles en arriendo')}
      </Link>
