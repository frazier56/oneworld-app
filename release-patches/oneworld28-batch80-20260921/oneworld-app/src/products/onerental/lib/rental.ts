...
export type Property = {
  ...
  country: string | null;
  city: string | null;
  /* The department / state / province, EXACTLY as the host selected or typed it — never a code.
     Added 15 Sep 2026; every listing written before then has null here, which is why every
     reader treats it as one more optional segment rather than a required one. */
  region: string | null;
  ...
  furnished: boolean;
  /* The optional lease clauses this property normally uses — the DEFAULT a new agreement starts
     from (Lee, 14 Sep 2026). The set that was actually signed lives on the contract, so editing a
     listing can never change a lease already signed. */
  addendum_keys: string[] | null;
  ...
  /* ── WHAT TIME THE DOOR OPENS AND WHAT TIME YOU MUST BE OUT (17 Sep 2026) ───────────────
     Null means the host has not stated one. Nothing invents a default: a guest who reads a time
     we made up has been promised something nobody agreed to. */
  /** One rule per element, each at most 100 characters, rendered as a numbered list. The count
      the host may store is set by their plan and enforced by the database, not the form. */
  house_rules: string[];
  /** Flat host cleaning charge for one stay. Null = the host charges none. */
...
/**
 * THE FEED CARD'S COLUMNS — the narrow read, named once.
 * ============================================================================================
 * This block was written inline in `screens/Feed.tsx` (11 Aug 2026) when the Discover feed was
 * cut down from the full `PROPERTY_COLUMNS` because it was fetching roughly three quarters of
 * its bytes for fields the card never draws. It is lifted out here, unchanged, for one reason:
 * the world feed at `/sandbox` reads the same rows, and a SECOND copy of a column list is a
 * second thing to forget to update. One list, two readers, no drift.
 *
 * It is the eleven fields the card renders, plus the four the filter sheet reads.
 */
export const FEED_CARD_COLUMNS =
...
export const PROPERTY_COLUMNS =
  ...
  "price, price_unit, host_pays_guest_fee, currency, display_currency, addendum_keys, " +
  "display_fx_rate, deposit_required, deposit_amount, deposit_currency, country, city, region, neighbourhood, " +
  ...
  "check_in_time, check_out_time, cleaning_fee, house_rules, " +
...
import { drawPriceDisplay, productHref, type ViewerCcy, type CompareItem } from "@oneworld/shell";
...
export const rentalPriceDisplay = (
  ...
  if (p.currency?.toUpperCase() === "COP") {
    if (ccy === "COP" || !rate || !Number.isFinite(rate) || rate <= 0) {
      return { text: cop(p.price, 1), currency: "COP" as const };
    }
    return drawPriceDisplay(p.price / rate, ccy, rate);
  }
  return drawPriceDisplay(p.price, ccy, rate);
};
...
export const priceLabel = (
  ...
  `${rentalPriceDisplay(p, ccy, rate).text} / ${p.price_unit === "night"
...
) => rentalPriceDisplay(p, ccy, rate).text;
...
/* ── WHERE A PROPERTY IS, IN ONE SENTENCE ──────────────────────────────────────────────────
   Lee, 14 Sep 2026: *"We do need to show the city, state, and country."*

   Every screen used to build this by hand as `[neighbourhood, city].join(", ")` — four copies,
   no department, and no country at all. This is the only version now.

   RULES:
   · Neighbourhood, city and department print EXACTLY as they are stored, which is exactly as
     the host chose or typed them. No title-casing, no accent-stripping, no lookup table. If a
     host writes "el poblado" that is what their listing says, because it is their listing.
   · The country is the one segment that is NOT stored as typed — `country` is ISO-2, enforced
     by `rental_properties_country_iso2` — so it is expanded to a real name in the language the
     reader is using. "CO" is our storage; nobody typed it and nobody should read it.
   · Empty segments vanish rather than leaving ", ," behind, and a listing with nothing but a
     country still says the country.
   · A duplicate adjacent segment is collapsed: Bogotá's city and department are both
     "Bogotá, D.C.", and "Bogotá, D.C., Bogotá, D.C., Colombia" is how a real address reads
     when nobody checks. */
const COUNTRY_LOCALE: Record<string, string> = {
  en: "en", es: "es", co: "es-CO", de: "de", pt: "pt-BR", ru: "ru", zh: "zh-Hans",
};
...
export function countryName(code: string | null | undefined, lang = "en"): string {
  ...
  try {
    /* `Intl.DisplayNames` returns the CODE BACK when it does not know a region, so a bad code
       would print "XX" as if it were a place. Compare and drop it. */
    const name = new Intl.DisplayNames([COUNTRY_LOCALE[lang] ?? "en"], { type: "region" }).of(cc);
    return !name || name === cc ? "" : name;
  } catch {
    /* Older engines, or a locale the runtime has no data for. A missing country name is a
       shorter sentence, never a crash and never a raw code. */
    return "";
  }
}

export function locationSegments(
  p: { neighbourhood?: string | null; city?: string | null; region?: string | null; country?: string | null },
  lang = "en",
): string[] {
  const raw = [p.neighbourhood, p.city, p.region, countryName(p.country, lang)]
    .map(x => (x ?? "").trim())
    .filter(Boolean);
  const out: string[] = [];
...
export function locationLine(
  ...
  return locationSegments(p, lang).join(", ");
}

/* ⚠️ THE FULL LINE DOES NOT FIT ON A FEED CARD, and a card that says
   "El Poblado, Medellín, An…" is worse than one that never promised the department. Measured in
   a real browser at 390px: four segments clip mid-word inside the card's right-hand column.

   Lee's two rules collide here and both are right — *"we do need to show the city, state and
   country"* and *"text must fit its box"*. They resolve by SURFACE, not by shortening the truth:
   a card is a glance and carries the two segments that identify the place to someone already
   browsing Colombia; the detail page, the lease and anything printed carry all four. Same
   function, same stored values, one decision about how much room there is.

   Never "fix" a clipped card by making `locationLine` shorter — that would quietly strip the
   department and country off the lease, which is the one place they have to be unambiguous. */
export function locationShort(
  p: { neighbourhood?: string | null; city?: string | null; region?: string | null; country?: string | null },
  lang = "en",
): string {
  const segs = locationSegments(p, lang);
  /* Take the first two of neighbourhood/city — but if a listing has neither, fall back to what
     it does have rather than printing nothing. */
  return (segs.length <= 2 ? segs : segs.slice(0, 2)).join(", ");
}
