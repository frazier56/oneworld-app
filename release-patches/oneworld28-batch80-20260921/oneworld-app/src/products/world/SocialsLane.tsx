...
export function useSocialsLane(lang: string, query: string, enabled: boolean) {
  ...
  const postsQ = useAsyncResult(async () => {
  ...
  /* `posts` keeps its old shape for every reader below; the failure rides alongside it. */
  const posts = postsQ.data;

  /* ⚠️ THIS ONE MUST NOT THROW. It is decoration — the author behind a post — and the lane's `ready`
  ...
  return {
    ...
    /* ⚠️ `failed` MEANS "NOTHING TO SHOW", NOT "SOMETHING WENT WRONG". — 22 Sep 2026
       ...
       scroll, not a wipe. */
    failed: !!postsQ.error && cards.length === 0,
    pageFailed: !!postsQ.error && cards.length > 0,
    retry: postsQ.retry,
    /* ⚠️ `total` COUNTS CARDS; `atEnd` COUNTS ROWS. They are different numbers on this lane
       and only on this lane, so the asymmetry is deliberate and written down. — 22 Sep 2026

       `media_posts` is one row per FILE, and R24 collapses an upload into one card. `total`
       feeds the footer's "Showing 3 of 40", which is counting the things you can swipe
       through, so it has to be the grouped count — it was `posts.length`, which made a single
       ten-photo upload render "Showing 1 of 10" and read as nine cards that failed to load.

       `atEnd` is the pagination question — "did the server have more rows than I asked for" —
       and the query pages on ROWS. Grouping it would stop the lane fetching the moment ten
       photos collapsed below the page size. Leave it on the raw count. */
