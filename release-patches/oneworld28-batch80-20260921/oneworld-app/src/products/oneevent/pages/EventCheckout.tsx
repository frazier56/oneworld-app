...
const clearApplicationDraft = (eventId?: string) => {
  ...
  try {
    ...
    /* EV-09 (OneEvent 30 overlay 4): the resume token goes with the draft, so the next
       applicant on a shared device gets a fresh one instead of writing over this one's. */
    localStorage.removeItem(applicationDraftTokenKey(eventId));
        ...
        {isGuest && manualRail ? (
          ...
          <div className="rounded-2xl bg-card border border-border p-5 mb-4">
            ...
            <div className="flex items-center gap-3">
              {profile?.photo_url ? (
                <img decoding="async" src={profile.photo_url} alt="" className="w-10 h-10 rounded-full object-cover border border-border" />
              ) : (
