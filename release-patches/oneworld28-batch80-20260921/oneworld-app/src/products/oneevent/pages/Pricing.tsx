    ...
    <div className="mx-auto w-full max-w-5xl px-4 pb-32 pt-6 sm:pb-10">
      ...
      <div className="mx-auto mb-6 max-w-sm rounded-2xl border border-brand/15 bg-white/70 p-3 shadow-sm shadow-brand/5 backdrop-blur dark:border-white/10 dark:bg-white/[0.04]">
        ...
        <div className="flex gap-2">
          <input type="text" value={promoInput} onChange={(e) => { setPromoInput(e.target.value); setPromoMsg(""); }} onKeyDown={(e) => { if (e.key ==="Enter") { e.preventDefault(); applyPromo(); } }} placeholder="" disabled={!!appliedCode} className="min-w-0 flex-1 rounded-xl border ow-edge bg-white px-3 py-2 text-sm uppercase placeholder:normal-case placeholder:text-ink/40 focus:outline-none focus:ring-2 focus:ring-brand dark:bg-white/5 disabled:opacity-60" />
          {appliedCode ? (
 <button type="button" onClick={clearPromo} className="ow-edge rounded-xl border px-3 py-2 text-sm font-semibold text-ink/60 dark:text-white/60">Remove</button>
          ) : (
