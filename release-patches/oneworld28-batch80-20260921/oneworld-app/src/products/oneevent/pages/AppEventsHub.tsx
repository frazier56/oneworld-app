import { ScreenHeading, coverOf } from "@oneworld/shell";
...
import MyEventsEmptyState from "@evt/components/events/MyEventsEmptyState";
...
export default function AppEventsHub() {
  ...
  const formatEventDate = (startDate: string | null, endDate: string | null, isEvergreen?: boolean | null) => {
    if (isEvergreen) return t("de.anytime", "Available anytime");
    if (!startDate) return t("ev.date_tbd", "Date TBD");
    const start = new Date(startDate);
    const dateStr = new Intl.DateTimeFormat(locale, { month: "short", day: "numeric", year: "numeric" }).format(start);
    const timeStr = new Intl.DateTimeFormat(locale, { hour: "numeric", minute: "2-digit" }).format(start);
    if (endDate) {
      const end = new Date(endDate);
      const endTime = new Intl.DateTimeFormat(locale, { hour: "numeric", minute: "2-digit" }).format(end);
  ...
  const formatPrice = (ticketType: string | null, ticketPrice: number | null, currency?: string | null) => {
    if (ticketType === "free" || !ticketPrice || ticketPrice === 0) return t("checkout.free", "Free");
...
${event.description}` : `Event: ${event.title}`,
  ...
  const handleDuplicateEvent = async (event: EventCard) => {
    ...
    try {
      const { data: dup, error } = await supabase.from("events").insert({
        ...
        videos: (event as any).videos ?? [],
        cover_photo: (event as any).cover_photo ?? null,
        feed_preview: (event as any).feed_preview ?? null,
        ...
        allow_reentry: (event as any).allow_reentry === true,
        status: "draft",
        attendee_count: 0,
        revenue: 0,
        ga_sold: 0,
  ...
  const saveEvent = async (
    ...
    const eventPayload = {
      ...
      /* MEDIA (OneEvent 30): the OneHome shape. `cover_image_url` is written from the chosen cover
         on EVERY save — the trigger `event_media_choices_guard` enforces the same — so readers
         that still use it (share cards, calendar, WhatsApp, SEO) never see a stale still. */
      photos: data.photos || [],
      videos: data.videos || [],
      cover_photo: data.coverPhoto || null,
      feed_preview: data.feedPreview || null,
      cover_image_url: coverOf({ photos: data.photos, cover_photo: data.coverPhoto }) || data.coverImageUrl || null,
      ...
      allow_reentry: data.allowReentry === true,
      ...
      <AppLayout>
        <CreateEventForm
          ...
          initialData={editingEvent ? {
            ...
            /* An event saved before the media columns existed (or before the backfill ran) has a
               cover and no photos: the cover becomes photo number one so the deck shows it. */
            photos: Array.isArray((editingEvent as any).photos) && (editingEvent as any).photos.length
              ? (editingEvent as any).photos
              : (editingEvent.cover_image_url ? [editingEvent.cover_image_url] : []),
            videos: Array.isArray((editingEvent as any).videos) ? (editingEvent as any).videos : [],
            coverPhoto: (editingEvent as any).cover_photo ?? null,
            feedPreview: (editingEvent as any).feed_preview ?? null,
            ...
            allowReentry: (editingEvent as any).allow_reentry === true,
          ...
          <div>
            ...
            {showAddContact && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/70 backdrop-blur-sm p-4" onClick={() => setShowAddContact(false)}>
                <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md max-h-[90svh] overflow-y-auto bg-card border border-border shadow-card rounded-2xl">
            ...
            {rolodexLoading ? (
              ...
              <div className="space-y-2">
                {(() => {
                  ...
                  return (
                    <>
                      {/* EV-07 (OneEvent 30 overlay 5): at phone width the count, "Invite to event"
                          and "Remove" all wrapped mid-phrase with their icons stranded on the first
                          line. The count never wraps, the actions sit on their own row, and each
                          button is one unbreakable label with its icon beside it. */}
                          ...
                          <div className="mt-2 flex flex-wrap gap-2">
                            {selectedBlastEvent && (
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 whitespace-nowrap px-3 py-1.5 rounded-2xl text-xs font-semibold bg-primary text-primary-foreground hover:opacity-90"
>
                                {/* v33 (Lee): "I don't know what blast selected means" — say what it does. */}
                                <Send className="w-3.5 h-3.5 shrink-0" /> {m("Invite to event")}
                              </button>
                            )}
                            <button
                              onClick={handleBulkRemove}
                              className="inline-flex items-center gap-1.5 whitespace-nowrap px-3 py-1.5 rounded-2xl text-xs font-semibold bg-destructive/10 text-destructive border border-destructive/30 hover:bg-destructive/20"
>
                              <Trash2 className="w-3.5 h-3.5 shrink-0" /> {m("Remove")}
                            </button>
                            <button
                              onClick={() => setSelectedRolodex(new Set())}
                              className="inline-flex items-center whitespace-nowrap px-3 py-1.5 rounded-2xl text-xs font-semibold bg-secondary border border-border text-foreground"
>
                              {m("Clear")}
                            </button>
                          </div>
                        ...
                        return (
                          <div key={c.rolodex_id}
                            ...
                            <div onClick={() => setOpenRolodex({ id: c.rolodex_id, name: c.full_name })} className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">
                              <img decoding="async" src={c.photo_url || `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(c.full_name)}`}
                ...
                {requestedEvents.map(ev => (
                  <button key={ev.id} onClick={() => setPendingDialog(ev)}
                    ...
                    <div className="h-24 bg-secondary relative">
                      {ev.cover_image_url
                        ? <img decoding="async" src={ev.cover_image_url} alt="" className="w-full h-full object-cover" />
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(380px, 1fr))", gap: 16 }}>
            <AnimatePresence mode="popLayout">
              {filtered.length === 0 ? (
                <MyEventsEmptyState key="empty" perspective={perspective} onCreate={() => setShowCreateForm(true)} />
              ) : (
                filtered.map((event, i) => {
                  ...
                  return (
                    <motion.div key={event.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                      ...
                      title={perspective === "hosting" ? t("hub.events.view_live", "View Live") : undefined}
                      ...
                      <div className="relative" style={{ aspectRatio: "4/5" }}>
                        {event.cover_image_url ? (
                          <img decoding="async" src={event.cover_image_url} alt={event.title} className="w-full h-full object-cover" />
                        ) : (
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-4 text-white">
                          ...
                          <div className="mt-0.5 flex items-center gap-1.5 text-xs text-white/90">
                            <MapPin className="w-3.5 h-3.5 shrink-0 text-brand-light" /> {event.location || event.venue_name || t("hub.events.location_tbd", "Location TBD")}
                          </div>
                        </div>
                      </div>
                      <div className="p-4">
                            ...
                            <>
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-3">
                                <button
                                  ...
                                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-semibold bg-primary text-primary-foreground hover:opacity-90 transition-all"
>
                                  <Settings className="w-3 h-3" /> {t("hub.events.manage", "Manage")}
                                </button>
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-semibold bg-secondary text-foreground border border-border hover:bg-secondary/80 transition-colors"
>
                                  <Pencil className="w-3 h-3" /> {t("hub.events.edit", "Edit")}
                                </button>
                                <button
                                  onClick={(e) => { e.stopPropagation(); handleDuplicateEvent(event); }}
                                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-semibold bg-secondary text-foreground border border-border hover:bg-secondary/80 transition-colors"
>
                                  <Copy className="w-3 h-3" /> {t("hub.events.duplicate", "Duplicate")}
                                </button>
...
>
                                    {event.status === "published"
                                      ? (<><EyeOff className="w-3 h-3" /> {t("hub.events.unpublish", "Unpublish")}</>)
                                      : (<><Eye className="w-3 h-3" /> {t("hub.events.publish", "Publish")}</>)}
                                  </button>
                                )}
                                <button
                                  onClick={(e) => {
                                  ...
                                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-semibold bg-secondary text-foreground border border-border hover:bg-secondary/80 transition-colors"
>
                                  <Calendar className="w-3 h-3" /> {isEventInCalendar(event.id) ? t("hub.events.view_calendar", "View in Calendar") : t("hub.events.add_calendar", "Add to Calendar")}
                                </button>
                                {confirmDeleteId === event.id ? (
                                  <div className="flex items-center gap-1.5 ml-auto">
                                    <span className="text-xs text-destructive font-medium">{t("hub.events.delete_confirm", "Delete?")}</span>
                                    <button
...
>
                                      {deletingId === event.id ? "…" : t("hub.events.yes", "Yes")}
                                    </button>
                                    <button
...
>
                                    <Trash2 className="w-3 h-3" /> {t("hub.events.delete", "Delete")}
                                  </button>
                                )}
                              </div>
                            </>
