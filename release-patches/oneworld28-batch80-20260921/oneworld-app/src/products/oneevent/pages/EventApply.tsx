...
/* OneEvent 30 overlay 4 (EV-09, 20 Sep 2026) — DRAFT ISOLATION on a shared device.
   A draft is one person's. Before this, the browser draft was keyed by event only, so the
   next person to open the same form on the same phone or tablet saw the previous
   applicant's name and answers, and their first keystroke overwrote the previous
   applicant's server draft under the SAME resume token. Now:
     · the browser draft carries the token it belongs to; a draft whose token is not the
       token in use is discarded, never shown;
     · a resume link (?resumeDraft=…) names the identity — it replaces whatever token the
       device held, and a browser draft under the old token is discarded;
     · when an application completes, the token is cleared with the draft, so the next
       applicant on the device starts under a fresh token;
     · a recovered draft shows "Not you? Start a new application", which clears the
       draft and mints a fresh token — the previous applicant's server draft is untouched. */
const clearDraftStorage = (id: string, includeToken: boolean) => {
  try {
    localStorage.removeItem(draftKey(id));
    sessionStorage.removeItem(draftKey(id));
    if (includeToken) localStorage.removeItem(draftTokenKey(id));
  } catch { /* storage unavailable — nothing to clear */ }
};
/* Resolve the resume token FIRST (a link wins over the device, the device wins over a fresh
   mint) so the browser draft can be checked against it. Returns the token and whether the
   device's previous token was replaced by a link. */
function resolveResumeToken(id: string): { token: string; replaced: boolean } {
  try {
    ...
    if (UUID_RE.test(fromLink)) {
      const replaced = UUID_RE.test(stored) && stored.toLowerCase() !== fromLink.toLowerCase();
      localStorage.setItem(draftTokenKey(id), fromLink);
      return { token: fromLink, replaced };
    }
    if (UUID_RE.test(stored)) return { token: stored, replaced: false };
    const minted = crypto.randomUUID();
    localStorage.setItem(draftTokenKey(id), minted);
    return { token: minted, replaced: false };
  } catch {
    return { token: crypto.randomUUID(), replaced: false };
  }
}
type ApplyDraft = {
  t: number;
  /* EV-09: the resume token this draft belongs to. A draft without one predates overlay 4. */
  token?: string;
  answers?: AnswerMap;
...
function readDraft(id: string, token: string, tokenReplaced: boolean): ApplyDraft | null {
  try {
    ...
    /* EV-09: a browser draft belongs to exactly one token. A draft under another token, or a
       legacy draft on a device whose token a resume link just replaced, is someone else's. */
    if ((d?.token && d.token.toLowerCase() !== token.toLowerCase()) || (!d?.token && tokenReplaced)) {
      clearDraftStorage(id, false);
      return null;
    }
...
export default function EventApply() {
  ...
  /* EV-09: the token is resolved before the draft is read, so the draft can be checked
     against it (see resolveResumeToken / readDraft). */
  const [resumeToken] = useState(() => (id ? resolveResumeToken(id) : { token: "", replaced: false }));
  const [draft] = useState<ApplyDraft | null>(() => (id ? readDraft(id, resumeToken.token, resumeToken.replaced) : null));
  ...
  /* EV-09: a recovered name or answers came from this device's previous session. Show who
     it belongs to and offer a clean start; the offer disappears once the fields are edited
     or the recovered draft was empty. */
  const [recovered, setRecovered] = useState<{ name: string } | null>(() =>
    draft && (draft.guestName || Object.keys(draft.answers ?? {}).length > 0) ? { name: draft.guestName || "" } : null);
  const startFresh = () => {
    if (!id) return;
    clearDraftStorage(id, true);
    try {
      const url = new URL(window.location.href);
      url.searchParams.delete("resumeDraft");
      /* A changed URL (link param dropped) navigates; an unchanged one would be a no-op on a
         hash router, so reload instead. Either way the page re-mounts under a fresh token. */
  ...
  useEffect(() => {
    if (!id || !resumeToken.token || user?.id) return;
    let cancelled = false;
    void (async () => {
      try {
        const { data } = await (supabase as any).rpc("get_event_application_draft", {
          p_event_id: id,
          p_resume_token: resumeToken.token,
        });
        ...
        if (!cancelled && row) {
          if (row.applicant_name) {
            ...
            setRecovered({ name: String(row.applicant_name) });   // EV-09: name the recovered draft
          }
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(draftKey(id), JSON.stringify({
        t: Date.now(), token: resumeToken.token, guestName, answers: liveAnswers,
      } satisfies ApplyDraft));
    } catch { /* storage full/private mode — the form still works, just without draft safety */ }
  }, [id, resumeToken.token, guestName, liveAnswers]);
  ...
  useEffect(() => {
    if (!id || !resumeToken.token || user?.id || !guestFirstName.trim() || !guestLastName.trim()) return;
  ...
  }, [id, resumeToken.token, user?.id, guestFirstName, guestLastName, guestName, liveAnswers]);
  ...
  const handleSubmit = async (answers: AnswerMap) => {
    ...
    if (isGuest) {
      ...
      try {
        localStorage.setItem(draftKey(id), JSON.stringify({ t: Date.now(), token: resumeToken.token, guestName, answers } satisfies ApplyDraft));
    ...
    try {
      ...
      if (gatedFree) {
        ...
        clearDraftStorage(id, true);   // EV-09: the token goes with the draft
      } else {
...
>
        ...
        {!user?.id && (
          <div className={`rounded-2xl bg-card border p-5 mb-4 ${nameError ? "border-destructive" : "border-border"}`}>
            <h2 className="text-sm font-bold text-foreground">{t("apply.your_name", "Your name")}</h2>
            <p className="mt-1 text-[11px] text-muted-foreground">{t("apply.name_note", "We’ll save your progress under this name. You’ll choose Google or Email on the next screen.")}</p>
            {/* EV-09: a draft recovered on this device belongs to whoever typed it. One tap
                starts a clean application under a fresh token without touching theirs. */}
            {recovered && (
              <p className="mt-2 text-[11px] text-muted-foreground">
                {recovered.name
                  ? t("apply.recovered_for", "Picking up where {name} left off.").replace("{name}", recovered.name)
                  : t("apply.recovered", "Picking up a saved application.")}{" "}
                <button type="button" onClick={startFresh} className="font-semibold text-primary underline underline-offset-2">
                  {t("apply.not_you", "Not you? Start a new application")}
                </button>
              </p>
            )}
