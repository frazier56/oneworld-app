        ...
        <>
          ...
          <div className="card p-4">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {TIERS.map(t2 => (
                <button key={t2.tier} onClick={() => setContract(c => c === t2.tier ? null : t2.tier)} className={`rounded-full px-3.5 py-1.5 text-[13px] font-bold transition ${ contract === t2.tier ?"bg-teal text-white" :"border ow-edge"}`}>
                  {W(lang, t2.en, t2.es)}
                </button>
              ))}
            </div>
