...
export default function ListingEngagement({
  ...
  useEffect(() => {
    ...
    return () => { live = false; };
    /* ⚠️ `onComments` IS NOT A DEPENDENCY — ONLY WHETHER THERE IS ONE. — 22 Sep 2026
       This listed `onComments` itself, and the world feed passes it as an inline arrow
       (`onComments={() => setComments(true)}`), so it is a new function on every render of
       `WorldFeed` — and `WorldFeed` re-renders constantly: the chrome flips on every change of
       scroll direction, the slide index moves on every swipe, the comment count sets twice per
       card, the address updates per slide. Every one of those re-ran this effect, which is two
       Supabase round trips, plus the `saved_items` heal-upsert on surfaces that pass `savesAs`.
       A single lane swipe fired ten to twenty duplicate head-counts.

       Worse than the waste: `toggleLike` is optimistic, so a re-run landing inside a like's
       round trip repaints `liked` from the row that has not committed yet — the heart un-fills
       and fills again, which reads exactly like a tap that did not take.

       The body only ever asks IS there a separate comments surface, so the boolean is the real
       dependency and it is stable. Same class of bug as `Drawer`'s `onCloseRef`, one file over.
       eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [itemId, source, userId, !!onComments]);

  /* ⚠️ EVERY TRANSIENT NOTICE ON THIS CARD IS A TIMER, AND A CARD CAN DIE FIRST. — 22 Sep 2026
     Three `window.setTimeout(...)` calls here reset a notice after two or three seconds, and
     none of them were held or cancelled. On the world feed this component is mounted once per
     card and unmounted the instant you swipe past, so a failed like on a card you leave behind
     leaves a timer running against a component that no longer exists — React's
     update-on-unmounted warning, once per occurrence, and on a fast scroll they stack up.

     `flash` is the only way this file sets a self-clearing notice now: it cancels the previous
     one for the same notice (so a second failure shows the full three seconds rather than
     inheriting the remainder of the first) and registers the handle for the unmount sweep. */
  const flashes = useRef<Record<string, number>>({});
  useEffect(() => () => {
    for (const id of Object.values(flashes.current)) window.clearTimeout(id);
    flashes.current = {};
  }, []);
  const flash = (name: string, set: (on: boolean) => void, ms: number) => {
    set(true);
    if (flashes.current[name]) window.clearTimeout(flashes.current[name]);
    flashes.current[name] = window.setTimeout(() => { delete flashes.current[name]; set(false); }, ms);
  };
  ...
  async function toggleLike() {
    ...
    const { error } = await q;
    /* ⚠️ A SILENT REVERT IS INDISTINGUISHABLE FROM A MIS-TAP. The heart filled, then emptied
       half a second later, with the only signal being a colour change on an SVG — no message,
       nothing announced. `aria-live` on the notice below is what makes it reach somebody who
       is not watching that one glyph. — 22 Sep 2026 */
    if (error) {
      setLiked(was); setLikes(n => n + (was ? 1 : -1));
      flash("write", setWriteErr, 3000);
      return;
    }
    ...
    if (savesAs && userId) {
      ...
      if (saveError) {
        console.error("[engagement] saved list not updated:", saveError.code, saveError.message);
        /* The public like DID land, so the heart is telling the truth. What is untrue is the
           member's own Saved list, and until now nothing said so anywhere. */
        flash("save", setSaveErr, 3000);
      }
    }
  }
  ...
  /* One line, announced, and it never changes the rail's height — on the world feed the rail is
     a fixed column and anything that grows inside it pushes every other control up the screen.
     So it floats over the slide rather than joining the stack. */
  const notice = (writeErr || saveErr) ? (
    <p role="status" aria-live="polite"
      className={vertical
        ? "pointer-events-none absolute right-0 top-full mt-1 w-[150px] rounded-lg bg-black/72 px-2 py-1.5 text-right text-[11px] font-bold leading-snug text-white backdrop-blur-md"
        : "pt-1 text-[12px] font-semibold opacity-75"}>
      {writeErr ? W(lang, "That didn't save. Try again.", "No se guardó. Inténtalo de nuevo.")
                : W(lang, "Saved list didn't update.", "Tu lista de guardados no se actualizó.")}
    </p>
  ) : null;

  return (
    <div className={vertical ? "relative" : undefined}>
      {notice}
      {/* ⚠️ `gap-1` BETWEEN THESE THREE WAS THE REST OF THE SPACING BUG (R27), and the number
           came down again in R28. The feed's rail spaces ITS children, but like, comment and
           share are three children of THIS element, not of the rail — so they sat four pixels
           apart inside a column that was otherwise breathing. It is 16 px here and 16 px on the
           rail: one number, both places, or the column spaces itself unevenly. */}
      <div className={`flex ${vertical ? "flex-col items-center gap-4" : "items-center gap-1"} ${size} ${onDark ? "text-white" : ""}`}>
        ...
        {/* `data-ow` so the world feed can put focus BACK on this button when its comment sheet
            closes. It cannot do that the usual way: opening the sheet unmounts this entire rail,
            so the element that was focused leaves the document before the sheet's own effect
            runs, and what it captures is the body. On close the rail mounts a BRAND NEW button,
            which is why an element reference would not have helped either — it has to be found
            again. */}
        {!hideComments && (
        <button type="button" data-ow="comments"
          onClick={() => (onComments ? onComments() : setOpen(o => !o))}
...
function Comments({
  ...
  /* Bumped on every load; a response whose number is no longer the current one is dropped. */
  ...
  const load = async () => {
    ...
    /* ⚠️ A SLOW THREAD FOR ONE ITEM MUST NOT LAND ON ANOTHER. — 22 Sep 2026
       This ended with a bare `setRows` and `onCount` — and `onCount` writes the PARENT's
       comment number. With nothing cancelling an in-flight read, a slow response for item A
       arriving after the card had changed replaced item B's thread AND its badge with A's. The
       world feed never reaches this path (it passes `onComments`, so this inline panel never
       opens), but OneSocial and OneJob do. A monotonic sequence number is the same guard the
       counts at the top of this file already use, in the shape this function can carry. */
  ...
  async function post() {
    ...
    setBusy(false);
    /* ⚠️ THE FAILURE BRANCH DID NOT EXIST. — 22 Sep 2026
       `if (!error) { … }` and nothing else: a rejected insert left the text in the box, the
       button live, and absolutely nothing on screen. People tapped Post again, and again. */
    if (error) { setPostErr(true); return; }
    setBody(""); load();
  }
  ...
  return (
    <div className="mt-2 border-t border-ink/[0.07] pt-2 dark:border-white/10">
      {postErr && (
        <p role="alert" className="mb-2 rounded-lg bg-red-500/10 px-3 py-2 text-[12.5px] font-semibold text-red-600 dark:text-red-400">
          {W(lang, "That comment didn't post. Try again.", "El comentario no se publicó. Inténtalo de nuevo.")}
        </p>
      )}
