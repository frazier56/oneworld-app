...
import { IconChat, IconPlay, IconShare, IconX } from "./ActionIcons";
          ...
          <button key={url} type="button" onClick={() => setOpen(index)}
            ...
            <span className="pointer-events-none absolute inset-0 grid place-items-center">
              <span className="grid h-14 w-14 place-items-center rounded-full bg-black/45 text-white backdrop-blur">
                <IconPlay size={24} className="translate-x-[1px]" />
              </span>
            </span>
          </button>
        ))}
      ...
      <div className="mx-auto flex h-full max-w-xl snap-y snap-mandatory flex-col overflow-y-auto">
        {videos.map((url, index) => <article key={url} className="relative h-[100dvh] shrink-0 snap-start bg-black">
          ...
          <div className="absolute inset-x-0 top-0 flex items-center justify-between gap-3 p-4 text-white">
            ...
            <button type="button" onClick={() => setOpen(null)} className="grid h-11 w-11 place-items-center rounded-full bg-black/45 backdrop-blur" aria-label={W(lang, "Close", "Cerrar")}><IconX size={22} /></button>
          </div>
          <div className="absolute bottom-8 right-4 flex flex-col gap-3 text-white">
            ...
            <button type="button" onClick={() => void share()} className="grid h-12 w-12 place-items-center rounded-full bg-black/50 backdrop-blur" aria-label={W(lang, "Share", "Compartir")}><IconShare size={22} /></button>
            <a href={contactHref} className="grid h-12 w-12 place-items-center rounded-full bg-brand text-white shadow-lg" aria-label={W(lang, "Contact host", "Contactar anfitrión")}><IconChat size={22} /></a>
          </div>
        </article>)}
      </div>
    </div>}
