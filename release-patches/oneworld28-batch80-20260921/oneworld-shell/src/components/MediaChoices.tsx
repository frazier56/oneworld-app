...
import { IconCheck, IconPlay } from "./ActionIcons";
 ...
 */
export default function MediaChoices({ photos, videos, cover, onCover, preview, onPreview, lang, subject = "listing" }: {
  photos: string[];
  videos: string[];
  cover: string | null;
  ...
  lang: string;
  /** What the cover leads: a listing (rent, sale) or an event. Only the one note changes. */
  subject?: "listing" | "event";
}) {
    ...
    <div className="space-y-4">
      {showCover && (
        <Choice
          ...
          note={subject === "event"
            ? W(lang, "Leads the event page and your cards.", "Encabeza la página del evento y sus tarjetas.")
            : W(lang, "Leads the listing and your profile cards.", "Encabeza el anuncio y las tarjetas de su perfil.")}
...
function Choice({ lang, title, note, current, badge, open, onToggle, children }: {
 ...
 className="ow-edge ow-tap shrink-0 self-center rounded-full border px-3 py-1.5 text-[12px] font-bold">
