...
export function GooglePlacesAutocomplete({ value = "", onChange, onSelect, placeholder = "Address or city", className, error }: GooglePlacesAutocompleteProps) {
  return (
    <input value={value} onChange={(e) => onChange?.(e.target.value)} onBlur={(e) => onSelect?.(e.target.value)} placeholder={placeholder} className={cn("w-full rounded-xl border bg-white px-4 py-2.5 outline-none focus:ring-2 focus:ring-teal/40 dark:bg-white/5", error ? "border-red-400" : "ow-edge", className)} />
  );
}
