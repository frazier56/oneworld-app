...
export function DistanceFilter({ onDistanceChange }: { onDistanceChange: (d: number | null, coords: { lat: number; lng: number } | null) => void }) {
 ...
 return <button type="button" onClick={toggle} className={`ow-edge shrink-0 rounded-full border px-3.5 py-2 text-sm font-semibold ${on ? "border-teal bg-teal/10 text-teal" : " "}`}><MapPin size={14} className="mr-1 inline-block align-[-2px]" />Near me</button>;
}
