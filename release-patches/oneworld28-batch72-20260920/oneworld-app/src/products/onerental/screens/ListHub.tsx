...
export default function ListHub() {
  ...
  /* The tier, for the My plan row. `my_plan` resolves VIP-before-Pro and expiry server-side, so
     this screen never has to know the precedence rules — see `one_world_plan`. */
  ...
  return (
    ...
    <div className="space-y-4 pb-28">
      ...
      <HubPanel d={P.user} open={panel.open === "profile"} onToggle={() => panel.toggle("profile")}
        ...
        {/* ⚠️ MOVED HERE FROM THE PORTFOLIO PAGE (Lee, 20 September 2026): *"Your plan should be
            within your profile — when you go to your profile you should have my host profile, my
            renter profile, saved properties, calendar, and your plan."*

            He is correcting a call I made on 18 September. He had asked for a way in to
            subscriptions and I put it on the busiest page in the product; what you pay for is a
            fact about the ACCOUNT, not about the rental portfolio, which is the same argument
            that makes VIP account-wide in the first place. It belongs with the other things that
            are about the person. */}
        <HubRow d={P.card}
          title={W(lang, "My plan", "Mi plan")}
          sub={plan === "vip"
            ? W(lang, "VIP — every One World app.", "VIP — todas las apps de One World.")
            : plan === "pro"
              ? W(lang, "Pro on OneHome. See what VIP adds.", "Pro en OneHome. Vea qué agrega VIP.")
              : W(lang, "Free. See what Pro and VIP add.", "Gratis. Vea qué agregan Pro y VIP.")}
          onClick={() => nav(productHref("onerental", "/plans"))} />
      </HubPanel>
      ...
      <HubPanel d={P.home} open={panel.open === "rent"} onToggle={() => panel.toggle("rent")}
        ...
        {/* ⚠️ THE DRAFT'S NAME IS NOT THE SUBTITLE. Lee, 20 Sep 2026: *"It says pick up where you
            left off, and that's all you really need to say. You don't need to say 'apartamento de
            habitación' — that's just unnecessary. We can put a count… right next to draft
            listings in parentheses."*

            He is right twice over. It said nothing the row's own title did not, and a scraped
            Spanish listing name ran to three lines, which is what made this row taller than every
            other row in the panel. The count goes in the title; the sentence stays fixed. */}
        <HubRow d={P.rentSign}
          count={counts?.latestDraft ? counts.drafts : undefined}
          ...
          sub={counts?.latestDraft
            ? W(lang, "Pick up where you left off.", "Retome donde lo dejó.")
