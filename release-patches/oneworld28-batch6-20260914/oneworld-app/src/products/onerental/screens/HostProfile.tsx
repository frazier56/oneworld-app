...
import {
  GlassSelect, PlacesInput, ScreenHeading, W, productHref, supabase, useI18n, useOneId,
} from "@oneworld/shell";
...
/* The payment routes, in words a host would say out loud. One map, because the same four values
   are named in three places on this screen and a sentence that builds a label by interpolating a
   database value is a sentence that eventually prints a database value. */
const PAYOUT_LABEL: Partial<Record<HostPayoutMethod, { en: string; es: string }>> = {
  remitly: { en: "Remitly", es: "Remitly" },
  wise: { en: "Wise", es: "Wise" },
  bank_transfer: { en: "bank transfer", es: "transferencia bancaria" },
  /* ⚠️ "STRIPE" IS NOT A WORD A MEMBER SEES. Lee, 14 Sep 2026: *"No one knows Stripe."* It is
     our payment processor, which is our business and not the host's; what the host is choosing is
     "the card or bank account OneHome pays me into". The stored VALUE stays `stripe`, because the
     readiness function and `stripe_connect_accounts` key on it — only the words change. */
  stripe: { en: "card or bank", es: "tarjeta o banco" },
};
  ...
  return (
    <div className="space-y-4 pb-28">
      ...
      <HostSection title={W(lang, "How you receive rent", "Cómo recibe el arriendo")}>
        <p className="text-[11.5px] leading-relaxed opacity-60">{W(lang,
          "OneHome never stores your card or bank numbers. Choosing card or bank opens our payment partner's own secure form, and only they hold the numbers — we get back nothing but the last four digits. For Remitly, Wise or a bank transfer, just confirm your receiving account is ready.",
          "OneHome nunca guarda los números de su tarjeta ni de su cuenta. Si elige tarjeta o cuenta bancaria, se abre el formulario seguro de nuestro aliado de pagos y solo ellos guardan los números — a nosotros solo nos llegan los últimos cuatro dígitos. Para Remitly, Wise o transferencia, solo confirme que su cuenta receptora está lista.")}</p>
        <GlassSelect<HostPayoutMethod> value={payoutMethod} onChange={value => { setPayoutMethod(value); setManualConfirmed(false); }} ariaLabel={W(lang, "Rent payment route", "Forma de recibir el arriendo")} options={[
          ...
          { value: "stripe", label: W(lang, "Card or bank account", "Tarjeta o cuenta bancaria") },
        ]} />
        {payoutMethod === "stripe" ? (
          payout.payouts_enabled
            ...
            : <button type="button" className="btn-primary w-full" disabled={busy || payout.loading} onClick={() => void connectStripe()}>{W(lang, "Add card or bank account", "Agregar tarjeta o cuenta bancaria")}</button>
        ) : (
          <label className="flex min-h-12 cursor-pointer items-start gap-3 rounded-xl border border-ink/12 p-3 text-[12.5px] dark:border-white/15">
            ...
            {/* ⚠️ THIS SENTENCE READ "I confirm my undefined receiving account is ready."
                It interpolated the raw database value straight into a sentence, so a profile with
                no payout method yet — which is every profile before somebody picks one — printed
                the word `undefined` to the host, inside a legal confirmation they are ticking.
                Caught in a screenshot, not in a type check: the column is typed non-null and the
                row simply had not been written yet. A label map cannot print a column name. */}
            <span>{W(lang,
              `I confirm my ${PAYOUT_LABEL[payoutMethod]?.en ?? "receiving"} account is ready.`,
              `Confirmo que mi cuenta de ${PAYOUT_LABEL[payoutMethod]?.es ?? "recepción"} está lista.`)}</span>
          </label>
        )}
      </HostSection>
