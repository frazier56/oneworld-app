...
export default function RentalRequestSheet({
  ...
  /* ⚠️ THE CLEANING FEE IS PART OF THE PRICED TOTAL, AND THE ONEHOME FEE IS TAKEN ON IT.
     `create_rental_booking_request` computes `quoted_total = price * nights + cleaning_fee` and
     splits the fee on that. The moment that changed, this sheet started quoting a smaller number
     than the database was about to stamp — the exact disagreement that makes somebody distrust
     every figure on the screen. Monthly stays are not affected: the cleaning fee is not charged
     there, because it would recur every month of a tenancy. */
  const cleaning = monthly ? 0 : Math.max(Number(cleaningFee ?? 0) || 0, 0);
  const base = stay.total + cleaning;
        ...
        <div className="mt-4">
          <StayFactsTable lang={lang} money={money} property={property ?? null}
            facts={{ starts_on: stay.starts_on, ends_on: stay.ends_on, nights: stay.nights,
              ...
              cleaning_fee: monthly ? null : cleaningFee,
