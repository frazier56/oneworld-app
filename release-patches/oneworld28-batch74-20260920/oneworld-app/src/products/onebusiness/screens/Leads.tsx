...
export default function Leads() {
 ...
 <div className="ow-edge mt-2 flex gap-2"><button type="button" disabled={busy || !(f.name || f.phone || f.email)} onClick={add} className="btn-primary flex-1 !py-2 text-[12.5px] disabled:opacity-50">{t("save")}</button><button type="button" aria-label={t("cancelAddLead")} onClick={() => setAdding(false)} className="ow-tap rounded-xl border px-3 text-[12.5px] font-bold">✕</button></div>
            </div>
