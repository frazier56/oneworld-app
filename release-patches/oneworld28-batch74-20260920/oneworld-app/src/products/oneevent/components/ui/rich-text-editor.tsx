...
//
// v23 CV/CT (Lee's UAT, 18 Aug 2026): upgraded to the OneJob toolbar model — Word-style
// list glyphs instead of the old "•"/"1." text buttons, a real required-field error ring,
// and ONE visible resize grip (three bold diagonal lines on a soft chip, native resizer
// painted out via .ow-rte-body::-webkit-resizer in oneevent.css) that a thumb can find.
...
/* Word-style list icons — three rule-lines with a bullet / numeral marker, the standard
   MS-Word ribbon glyphs. Replaces the old "•" and "1." text buttons. (OneJob #62 port) */
...
export function RichTextEditor({ value = "", onChange, placeholder, className, error }: RichTextEditorProps) {
  ...
  const Btn = ({ cmd, val, title, children, extra }: { cmd: string; val?: string; title: string; children: React.ReactNode; extra?: string }) => (
    <button type="button" title={title}
      ...
      className={cn("grid h-7 w-7 shrink-0 place-items-center rounded text-sm hover:bg-teal/10", extra)}>
      {children}
    </button>
  );
  ...
  return (
    <div className={cn(
      ...
      <div className="flex flex-nowrap items-center gap-1 overflow-visible border-b border-ink/10 p-1.5 dark:border-white/10">
        ...
        <div className="relative shrink-0" onMouseDown={saveSel}>
          <button type="button" title="Text size"
            ...
            className="flex h-7 items-center gap-0.5 rounded px-1.5 hover:bg-teal/10">
            ...
            <div className="glass-modal absolute left-0 top-9 z-50 w-28 rounded-xl p-1 shadow-xl" onClick={(e) => e.stopPropagation()}>
              {SIZES.map((s, i) => (
                <button key={s.val} type="button"
                  ...
                  className="flex w-full items-center rounded-lg px-2.5 py-1.5 text-left hover:bg-teal/10"
        ...
        <label title="Text color" className="relative grid h-7 w-7 shrink-0 cursor-pointer place-items-center rounded hover:bg-teal/10">
          <span className="text-[15px] font-bold leading-none" style={{ color: "#14B8A6" }}>A</span>
          <span className="pointer-events-none absolute bottom-1 h-[3px] w-4 rounded-full" style={{ background: "#14B8A6" }} />
          <input type="color" defaultValue="#14B8A6"
            onMouseDown={saveSel}
      ...
      {/* THE grip (v23 CT — Lee: "three prominent diagonal lines… so someone can grab that
          corner with their thumb"). Chrome's native resizer is painted out in oneevent.css
          (.ow-rte-body::-webkit-resizer), so this chip is the only thing you see — but the
          native resizer is still underneath doing the actual drag (pointer-events-none). */}
      <span aria-hidden
        className="pointer-events-none absolute bottom-[3px] right-[3px] grid h-[22px] w-[22px] place-items-center rounded-[8px] rounded-br-[11px] bg-ink/[0.07] text-ink/55 dark:bg-white/10 dark:text-white/60">
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
          <path d="M12.5 2.5 L2.5 12.5" /><path d="M12.5 7 L7 12.5" /><path d="M12.5 11 L11 12.5" />
        </svg>
      </span>
    </div>
  );
