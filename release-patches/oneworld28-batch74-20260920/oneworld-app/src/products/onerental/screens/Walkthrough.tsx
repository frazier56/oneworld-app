    ...
    <div className="space-y-4">
      ...
      <div className="mt-3 flex gap-2">
        {(["move_in", "move_out"] as const).map(k => (
          <button key={k} type="button" onClick={() => setKind(k)} className={`flex-1 rounded-xl border px-3 py-2 text-[13px] font-bold transition ${ kind === k ? "border-transparent bg-ink text-paper dark:bg-white dark:text-ink" : "ow-edge opacity-70"}`}>
...
function PhotoRow({ p, draft, lang, mine, frozen, src, onReview }: {
  ...
  return (
    <article className="card overflow-hidden p-0">
      {src
        ? <img decoding="async" src={src} alt={p.caption ?? ""} loading="lazy"
 ...
 className="ow-edge ow-tap flex-1 rounded-xl border py-2 text-[13px] font-bold">
