...
/**
 * ONEHOME'S MONTHLY FEE SPLIT: HOST 1.50% + GUEST 7.49% = 8.99% TOTAL.
 * ============================================================================================
 ...
 */
/* ── 1.50 HOST + 7.49 GUEST = 8.99 (Lee, 14 Sep 2026) ────────────────────────────────────────
   *"I would prefer that it'd be eight point nine nine. Ideally I would just make it one and a
   half and then make the other side a seven point four nine."*

   ⚠️ THESE TWO LINES ARE NOT THE CHANGE. The database is the authority on what is actually
   charged, and it carried the old number in FIVE places: `rental_guest_fee_rate()`, three writer
   functions with the rate typed out as literals (`create_rental_booking_request`,
   `rental_private.request_monthly_rental`, `set_rental_contract_fee_allocation`), and the
   `rental_booking_requests_fee_allocation_ck` constraint. Editing this file alone would have
   changed nothing but the words on the screen; editing the function alone would have made every
   new booking fail its own CHECK. Migration `onehome_fee_899_single_source_of_truth`, applied
   14 Sep 2026, leaves the rate in exactly one place server-side and proved 8 of 8 cases in a
   self-rolling-back block before it was applied. Keep these constants equal to
   `rental_host_fee_rate()` and `rental_guest_fee_rate()`; they exist only to quote a price before
   a row exists. */
export const RENTAL_HOST_FEE_RATE = 0.0150;
export const RENTAL_GUEST_FEE_RATE = 0.0749;
