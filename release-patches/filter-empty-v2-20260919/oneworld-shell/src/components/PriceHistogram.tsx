...
import { useI18n, WA } from "../lib/i18n";
    ...
    <div className="ow-hist select-none">
      ...
      <div className="relative mt-1 h-6">
        ...
        <input
          ...
          aria-label={WA(lang, { en: "Minimum price", es: "Precio mínimo", co: "Precio mínimo", de: "Mindestpreis", pt: "Preço mínimo", ru: "Минимальная цена", zh: "最低价格" })}
        ...
        <input
          ...
          onPointerUp={commit} onKeyUp={commit} onBlur={commit}
          aria-label={WA(lang, { en: "Maximum price", es: "Precio máximo", co: "Precio máximo", de: "Höchstpreis", pt: "Preço máximo", ru: "Максимальная цена", zh: "最高价格" })}
          className="ow-hist-thumb absolute inset-x-0 top-0 h-6 w-full appearance-none bg-transparent"
          style={{ zIndex: 4 }} />
      </div>
      ...
      <div className="mt-1.5 flex items-baseline justify-between gap-2 text-[12px] font-bold tabular-nums">
        ...
        <span className="text-[11.5px] font-semibold opacity-50">
          {WA(lang, { en: "{shown} of {total}", es: "{shown} de {total}", co: "{shown} de {total}", de: "{shown} von {total}", pt: "{shown} de {total}", ru: "{shown} из {total}", zh: "{shown} / {total}" }).replace("{shown}", String(shown)).replace("{total}", String(model.total))}
        </span>
