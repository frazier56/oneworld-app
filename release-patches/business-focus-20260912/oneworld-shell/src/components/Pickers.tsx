...
export function GlassDate({ value, onChange, invalid, min, max }:
  ...
  const locale = lang === "es" || lang === "co" ? "es" : "en";
  ...
  const label = value
    ...
    : W(lang, "Pick a date", "Elige una fecha");
      ...
      {open && (
        <Sheet onClose={() => setOpen(false)}>
          <div className="mb-2 flex items-center justify-between px-1">
            <button type="button" onClick={() => setView(v => ({ y: v.m === 0 ? v.y - 1 : v.y, m: v.m === 0 ? 11 : v.m - 1 }))}
            ...
            <button type="button" onClick={() => setView(v => ({ y: v.m === 11 ? v.y + 1 : v.y, m: v.m === 11 ? 0 : v.m + 1 }))}
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)}
        ...
        <span>🕐 {value ? fmt(value) : W(lang, "Pick a time", "Elige una hora")}</span><Chevron color="var(--brand-deep)" />
      </button>
      {open && (
        <Sheet onClose={() => setOpen(false)}>
          <p className="mb-2 px-1 font-bold">{W(lang, "Pick a time", "Elige una hora")}</p>
...
export function GlassTimePicker({ value, onChange, defaultTime = "09:00" }: { value: string; onChange: (v: string) => void; defaultTime?: string }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <GlassSelect value={String(parsed.h) as any} ariaLabel="Hour"
      ...
      <GlassSelect value={parsed.m as any} ariaLabel="Minute"
...
export function GlassSelect<T extends string>({ value, options, onChange, ariaLabel, className = "", triggerLabel, searchable }:
  ...
  return (
    <>
      ...
      {open && (
        <Sheet onClose={close}>
          <div ref={panel} role="dialog" aria-modal="true" aria-label={ariaLabel ?? W(lang, "Choose an option", "Elija una opción")} onKeyDown={e => {
          ...
          {searchable && (
            <input value={q} onChange={e => setQ(e.target.value)} autoFocus placeholder={W(lang, "Search…", "Buscar…")} aria-label={W(lang, "Search options", "Buscar opciones")}
