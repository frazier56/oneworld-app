...
export default function Feed({ query = "", filter = "all" }: { query?: string; filter?: FeedFilter }) {
 ...
 <button onClick={() => setComposer(null)} className="ow-edge grid h-8 w-8 place-items-center rounded-full border text-lg " aria-label="Close">×</button>
            </div>
                      ...
                      {thumbs.map((th, i) => (
                        <button key={i} onClick={() => setViewer({ items: slides, start: th.slideIdx })}
                          ...
                          {/video/i.test(th.type)
                            ...
                            : <img decoding="async" src={th.url} className="h-full w-full object-cover" loading="lazy" alt="" />}
 ...
 <button onClick={() => setPendingMedia(null)} className="ow-edge grid h-8 w-8 place-items-center rounded-full border text-lg " aria-label="Close">×</button>
            </div>
              ...
              {pendingMedia.map((f, i) => (
                <div key={i} className="h-20 w-20 shrink-0 overflow-hidden rounded-lg bg-ink/5 dark:bg-white/5">
                  {f.type.startsWith("video")
                    ...
                    : <img decoding="async" src={URL.createObjectURL(f)} className="h-full w-full object-cover" alt="" />}
                </div>
              ))}
            </div>
