...
import {
  type Property, type RentalAgent, PROPERTY_COLUMNS, CO_CITIES, priceLabel, cop, locationShort,
} from "../lib/rental";
...
export default function Feed() {
  ...
  }, [list, sort, filters, filterCount, agentScores]);
...
export function ListingCard({ l, agent, lang, layout = "big", ccy = "USD", fx, propertyRatings, hostRatings }:
  ...
  return (
    ...
    <article className="ow-feed-card relative overflow-hidden rounded-2xl border border-ink/[0.08] bg-ink/5 dark:border-white/10 dark:bg-white/5">
      {slides.length > 0 ? (
        <div
          ref={strip}
          onScroll={e => {
      ...
      {/* ⚠️ THE HEART LIVES HERE NOW. Lee, 16 Sep 2026: *"Relocate the heart to the top right,
          just under where it says how many pictures it is... then that'll leave room for the
          location to be wrapped."*

          Two problems, one move. It was squeezing the right-hand column, which is why the city
          arrived as "Poblado del Sur, Santa…" with the rest cut off — and it was the last thing
          still floating over the photograph. Under the counter it is out of the text's way and
          the corner already belongs to card controls rather than to content.

          ⚠️ It must stop the tap: this sits over the media, and the media opens the player.
          Liking a place must not also open it. */}
      <div className="absolute right-2 top-11 z-10"
        ...
        <ListingEngagement
          ...
          allowShare={false}
          hideComments
          /* ⚠️ Keep `onComments` even with no comment button drawn: inside ListingEngagement it is
             what makes the count query SKIP media_comments. Drop it and every card in the feed
             runs a count for a button nobody sees. */
          onComments={() => {}}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 px-3 pb-2 pt-16 text-white"
        ...
        <TapTarget video={slides[idx]?.kind === "video"} href={href} onPlay={() => setPlaying(idx)}>
          {/* ⚠️ ONE ROW, ONE SIZE. Lee: *"the price should be on the same exact row as the top
              line where it says house, three bedroom, four bathroom — and the same size font too,
              so it looks the same."* What it is and what it costs are two halves of one sentence;
              splitting them across two heights made the eye do the joining. */}
          <div className="flex items-end justify-between gap-3">
            <p className="min-w-0 flex-1 text-[14px] font-black leading-snug">{facts}</p>
            <p className="shrink-0 text-[15px] font-black leading-snug tracking-tight">{priceLabel(l, lang, ccy, fx)}</p>
          </div>
          {l.display_currency === "COP" && l.display_fx_rate && (
            <p className="mt-0.5 text-right text-[11px] leading-none opacity-75">≈ {cop(l.price, l.display_fx_rate)}</p>
          )}
          {/* ⚠️ THE CITY WAS BEING CUT OFF, AND THAT IS WHY IT MOVED DOWN HERE. Lee: *"the city is
              cut off — Poblado del Sur, comma, then Santa, dot dot dot. That doesn't look clean at
              all. Move the location below and wrap it. City on one line, the state on the next.
              You don't even need the country."*

              A place name squeezed into a right-hand column shares its width with a price, so the
              longer the name the worse it reads — and a Colombian neighbourhood plus municipality
              is long. On its own full-width line underneath it simply fits. Split at the LAST
              comma so the trailing administrative part drops to line two and the city keeps line
              one whole. */}
          {where && (() => {
            const cut = where.lastIndexOf(",");
            const city = cut === -1 ? where : where.slice(0, cut);
            const region = cut === -1 ? "" : where.slice(cut + 1).trim();
            return (
              <p className="mt-1 text-[12px] font-semibold leading-tight opacity-90">
                {city}{region && <><br />{region}</>}
              </p>
            );
          })()}

          {/* The heart moved to the top-right corner, under the photo counter — see the note
              beside <PhotoCount>. Reviews keep this line to themselves. */}
          <ListingRatings propertyId={l.id} hostId={l.agent_id} property={propertyRatings} host={hostRatings} lang={lang} />
