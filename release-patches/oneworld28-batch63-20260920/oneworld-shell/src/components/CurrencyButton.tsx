...
export default function CurrencyButton() {
  const { lang } = useI18n();
  const currencyName = (code: string, fallback: string) => {
    try { return new Intl.DisplayNames([lang === "co" ? "es-CO" : lang], { type: "currency" }).of(code) ?? fallback; } catch { return fallback; }
  };
  ...
  return (
    <div className="relative" ref={box}>
      <button type="button" onClick={() => setOpen(o => !o)}
        ...
        aria-label={W(lang, `Currency: ${currencyName(active.code, active.name)}`, `Moneda: ${currencyName(active.code, active.name)}`)}>
      ...
      {open && (
        <div role="listbox"
          ...
          className="ow-sheet absolute right-0 z-50 mt-2 max-h-[62svh] w-60 overflow-y-auto rounded-2xl p-1.5">
          ...
          {CURRENCIES.map(c => {
            ...
            return (
              <button key={c.code} type="button" role="option" aria-selected={c.code === ccy}
                ...
                <img decoding="async" src={`https://flagcdn.com/w40/${c.flag}.png`} alt=""
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[11px] opacity-55">{currencyName(c.code, c.name)}</span>
                </span>
                {!usable && (
