...
export default function Pipeline() {
 ...
 <button type="button" onClick={() => setDraft(x => { const won = x.findIndex(y => y.terminal); const n = { key: `s${Date.now().toString(36)}`, label: "", terminal: null }; return won < 0 ? [...x, n] : [...x.slice(0, won), n, ...x.slice(won)]; })} className="ow-edge ow-tap w-full rounded-xl border border-dashed py-2 text-[12.5px] font-bold">{t("addStage")}</button>
            {err && <p role="alert" className="text-[12.5px] font-bold text-rose-600 dark:text-rose-300">{businessError(err, locale)}</p>}
 <div className="ow-edge flex gap-2"><button type="button" disabled={busy} onClick={save} className="btn-primary flex-1 !py-2 text-[12.5px] disabled:opacity-50">{t("save")}</button><button type="button" aria-label={t("cancelEditStages")} onClick={() => setEditing(false)} className="ow-tap rounded-xl border px-3 text-[12.5px] font-bold">✕</button></div>
          </div>
