...
export default function Opener({ replayAlways }: {
  ...
  const [box, setBox] = useState(() => {
    try { return pillBox(window.innerWidth, window.innerHeight); } catch { return { right: 16, top: 56 }; }
  });
  ...
  useEffect(() => {
    if (!armed || done) return;
    const onResize = () => setBox(pillBox(window.innerWidth, window.innerHeight));
  ...
  return (
    ...
    >
      ...
      {/* SKIP. Positioned where the film's own art expects it, clamped onto the glass on phones
          taller than 9:16, and given a real 44-point target rather than the word alone. */}
      <button
        type="button"
        data-ow="opener-skip"
        onClick={finish}
