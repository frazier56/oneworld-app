...
import type { LanePin } from "./LaneMap";
...
export function useEventsLane(lang: string, query: string, enabled: boolean) {
  ...
  /* R18 note 4 — the feed's map shows the lane you came from, so every lane that has real
     coordinates hands them over. `events` already carries `latitude`/`longitude` and this lane
     already selects `*`, so this is a projection of rows it has, not a second read. Events with
     no coordinates are simply not plotted; the map says so rather than inventing a position. */
  const pins: LanePin[] = useMemo(() => shown
    ...
    .map(e => ({
      ...
      label: e.title, note: ticketLine(e, lang),
      href: productHref("oneevent", `/e/${e.id}`),
    })), [shown, lang]);
