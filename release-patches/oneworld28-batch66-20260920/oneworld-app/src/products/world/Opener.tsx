...
/* ── V7, NARRATED, SEVEN LANGUAGES (PUB30, 21 September 2026) ───────────────────────────────
   One World Reimagined delivered v7 as seven cuts — the same eighteen seconds, narrated by the
   same Colombian-accent voice in each language — and nothing in the app chose between them, so
   ninety-two megabytes of film would have shipped with only v5 ever playing. This is the
   smallest wiring that makes the delivery real: pick the cut, fall back to English.

   It is WORLD FEED 30's file and this is an additive edit, declared to them. The full R16 rule
   (an explicit language switch inside the opener) and R17 (the single Skip pill) stay theirs.

   Colombian Spanish is a separate cut because the narrator is Colombian and the difference is
   audible to the audience that matters most here, so `es-CO` is matched before plain Spanish. */
