...
export function useJobsLane(lang: string, query: string, enabled: boolean) {
  ...
  /* R18 note 4 — job postings carry real coordinates, so the Jobs lane plots them. The people
     half has none: a professional is not at an address, they travel to one, and putting a pin on
     somebody's home would be both wrong and a privacy problem. So the map is empty on that half
     and says so. */
  const pins: LanePin[] = useMemo(() => (kind === "jobs" ? (jobs ?? []) : [])
    ...
    .map(j => ({
      ...
      label: j.title, note: payLine(j, lang),
      href: productHref("onejob", `/j/${j.id}`),
    })), [jobs, kind, lang]);
