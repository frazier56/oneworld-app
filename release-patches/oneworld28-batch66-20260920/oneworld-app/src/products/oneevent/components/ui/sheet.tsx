...
export function SheetContent({ side = "right", className, children }: { side?: "right" | "left" | "bottom" | "top"; className?: string; children: React.ReactNode }) {
  ...
  if (!open) return null;
  const pos = side === "right" ? "right-0 top-0 h-full w-[92vw] max-w-md" : side === "left" ? "left-0 top-0 h-full w-[92vw] max-w-md" : side === "bottom" ? "bottom-0 inset-x-0 max-h-[88svh] rounded-t-3xl" : "top-0 inset-x-0 max-h-[88svh] rounded-b-3xl";
  return createPortal(
