  ...
  if (impersonating) {
    return (
      <button onClick={returnToAdmin} disabled={busy === "return"}
        ...
        {busy === "return" ? copy.returning : copy.returnAccount}
      </button>
    );
  }
  ...
  return (
    <div>
      ...
      <button
        ...
        {copy.viewAsUser}
        <span className="ml-auto grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-ink/5 dark:bg-white/10">
      ...
      {open && (
        <div ref={panelRef} className="mt-1 space-y-1 rounded-xl border border-ink/10 p-2 dark:border-white/10" style={{ scrollMarginTop: 12 }}>
          <input value={q} onChange={e => setQ(e.target.value)} placeholder={copy.searchUsers} aria-label={copy.searchUsers}
            ...
            {rows.map(r => (
              <button key={r.id} onClick={() => impersonate(r)} disabled={!!busy}
                ...
                <span className="min-w-0">
                  <span className="block truncate font-medium">{busy === r.id ? copy.switching : (r.full_name || "—")}</span>
                  <span className="block truncate text-[11px] opacity-55">{r.email}</span>
                </span>
              </button>
            ))}
            {rows.length === 0 && <p className="px-2 py-1.5 text-xs opacity-50">{copy.noUsers}</p>}
          </div>
        </div>
      )}
    </div>
