      ...
      {request && (
        <div id="lease-request-lifecycle" tabIndex={-1} className="card p-4 outline-none focus:ring-2 focus:ring-brand/40">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            {/* ⚠️ THESE TWO TILES HAD `border-white/55` ON A NEAR-WHITE CARD, so their edges
                faded out and the right-hand one read as clipped in a screenshot. Measured: no
                overflow at all — both are 158px inside a 324px row. It was contrast, not layout.
                Lee's rule is that a border has to be visible enough to see the object it is
                around; `border-ink/12` is what the rest of this app uses and it survives both
                themes. **Measure before "fixing" a layout that is not broken.** */}
            <div id="lease-request-identity" tabIndex={-1} className="rounded-2xl border border-ink/12 bg-white/45 p-3 outline-none focus:ring-2 focus:ring-red-400/50 dark:border-white/15 dark:bg-white/[0.06]">
            ...
            <div id="lease-request-payment" tabIndex={-1} className="rounded-2xl border border-ink/12 bg-white/45 p-3 outline-none focus:ring-2 focus:ring-red-400/50 dark:border-white/15 dark:bg-white/[0.06]">
