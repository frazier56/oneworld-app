...
export function VaiaPill({ compactOnNarrow = false }: { compactOnNarrow?: boolean }) {
  ...
  return (
    <button aria-label={t("vaiaInsightsAria")}
      ...
      {/* ⚠️ THE FACE STAYS WHEN THE WORDS GO. Lee asked for *"Tap for insights"* to be removed
          when a title needs the room — not for VAIA to disappear. Compacting used to hide the
          face too, which left a bare sparkle that reads as decoration rather than as a control
          somebody can tap. The label is ~95px and the face is 32px, so dropping the words alone
          already buys back everything a cut-off title needs. */}
...
export default function ScreenHeading({
  ...
  /* ── ⚠️ "TAP FOR INSIGHTS" GIVES WAY BEFORE THE TITLE DOES — ON EVERY SCREEN ───────────────
     Lee, 14 September 2026, on the host profile: *"The header is cut off, it says 'Your host, PR'.
     Anytime that's an issue, you need to remove where it says 'Tap for insights'."* And then the
     rule behind it: *"Obviously that change belongs on every page, not just this page. I shouldn't
     have to keep making the same requirements just because it's a different screen."*

     So this is not a fix to the host profile. `titlePriority` already existed and did exactly this
     — but it is a prop, which means it is a DECISION EACH SCREEN MAKES, and every screen that
     forgets it reintroduces the defect in whichever language happens to be long. There are ~40
     headings across six products and seven languages; nobody is going to get that right by hand.

     So the heading decides for itself, by measuring. A truncating element reports `scrollWidth`
     greater than `clientWidth` — that IS "the title is cut off", exactly, in any language, at any
     width. When it is, the pill drops its words and keeps its face, which buys back about 100px.

     Three details that matter:
     · It runs in `useLayoutEffect` with no dependency array, so the measurement and the re-render
       both flush BEFORE paint — the label is never seen appearing and disappearing.
     · It latches. Once compacted the title fits, so a naive re-measure would put the label back,
       overflow again, and oscillate forever. `tight` is only ever cleared by something that
       genuinely changes the answer: a new title, or a new container width.
     · The ResizeObserver watches the ROW, whose width comes from the page and does not change
       when the pill compacts. Observing the pill instead would be the feedback loop again. */
  const row = useRef<HTMLDivElement>(null);
  const titleRef = useRef<HTMLSpanElement>(null);
  const [tight, setTight] = useState(false);
  const titleKey = String(children ?? "");

  useLayoutEffect(() => { setTight(false); }, [titleKey]);
  useLayoutEffect(() => {
    if (tight || titlePriority) return;
    const el = titleRef.current;
    if (el && el.scrollWidth - el.clientWidth > 1) setTight(true);
  });
  useLayoutEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      ...
      setTight(false);          /* let the next layout pass decide again at the new width */
    });
    ro.observe(box);
    return () => ro.disconnect();
  }, []);

  const compact = titlePriority || tight;

  return (
    <div ref={row} className={`mb-3 flex items-center justify-between gap-3 ${className}`}>
      ...
      <h1 className={`flex min-w-0 items-center gap-2.5 text-[clamp(19px,5.4vw,24px)] font-extrabold leading-tight ${titlePriority ? "flex-1" : ""}`}>
        {icon}
        <span ref={titleRef} className={titlePriority ? "min-w-0 whitespace-normal break-words" : "truncate"}>{children}</span>
      </h1>
      <div className="flex shrink-0 items-center gap-2">
        {right}
        <VaiaPill compactOnNarrow={compact} />
      </div>
    </div>
  );
}
