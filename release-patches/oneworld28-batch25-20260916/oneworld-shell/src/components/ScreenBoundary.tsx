 ...
 *     and an outage.
 */
/* `resetKey` replaces the `key={loc.pathname}` AppShell used to put on this boundary. Same job —
   a screen that threw must not keep showing its error after you navigate away — but done by
   comparing a value instead of destroying and rebuilding the subtree. The old spelling cost a
   full unmount of every screen, and with it the Suspense boundary, on every single tab tap. */
type Props = { children: ReactNode; onReset?: () => void; resetKey?: string };
type State = { error: Error | null; seen?: string };
...
export default class ScreenBoundary extends Component<Props, State> {
  ...
  static getDerivedStateFromProps(props: Props, state: State): State | null {
    /* First render just records the key. After that, a CHANGED key clears any caught error — and
       only then, so a re-render with the same route never disturbs a boundary that is fine. */
    if (state.seen === undefined) return { ...state, seen: props.resetKey };
    if (props.resetKey !== state.seen) return { error: null, seen: props.resetKey };
    return null;
  }
