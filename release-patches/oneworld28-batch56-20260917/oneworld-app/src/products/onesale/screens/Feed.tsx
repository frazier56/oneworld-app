...
import {
  ...
  useViewerCcy, type ViewerCcy, drawPrice, drawPriceDisplay, NoticeRow,
...
export default function Feed() {
  ...
  const list = useMemo(() => sortListings(narrow(filters), sort, trm?.rate),
  ...
  return (
    ...
    <HomeTop
      ...
      searchAside={
        <label className="relative flex h-full items-center rounded-2xl border ow-edge bg-white/70 pl-2.5 pr-7 dark:bg-white/10">
      ...
      feedSlot={
        <div className="space-y-3 pb-28">
          ...
          <ControlRow filters={filters} sort={sort} view={view}
            ...
            /* ⛔ THE CURRENCY CONTROL CAME OFF THIS ROW. Lee, 20 September 2026, on a phone:
                 *"there's three icons to the far right, right next to USD… the third one is cut
                 off because it's just not at the proper width… the other option is to move the
                 currency thing and put it into the hamburger icon. That's probably the best
                 option."*

                 The row had grown to four groups — List/Map, Filters, Sort, currency, and three
                 layout tiles — and at 390px the last tile ran off the edge. A control you cannot
                 reach is worse than one you have to go and find.

                 Currency was the right thing to drop, because it is the only one of the five that
                 is NOT about this screen: it is a property of the reader, it already sits in the
                 header beside the bell and the flag on EVERY screen, and it has since August. The
                 copy here was added on 17 September for discoverability and turned out to cost the
                 layout picker a third of itself. Same setting, same stored value, one place. */
            />
          ...
          {listings === undefined ? [0, 1].map(i => <div key={i} className="card ow-shimmer h-64" />)
            : !list.length ? (listings.length ? <div className="card p-8 text-center">
            ...
            : view === "map" ? <ListingMap pins={pins} onOpen={p => nav(p.href)} />
            : (
              <>
...
export function SaleCard({ l, agent, lang, ccy = "USD", fx, hostRatings }:
  ...
  return (
    <article className="card overflow-hidden rounded-[26px] p-0">
      {l.photos.length > 0 && (
          ...
          <Link to={productHref("onesale", `/s/${l.id}`)} className="block">
            <div
              ...
              {l.photos.slice(0, 10).map((src, i) => (
                <img decoding="async" key={i} src={src} alt="" loading="lazy"
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/55 to-transparent pt-20">
        {/* ── v90 · U36 · THE HEART, ALONE ────────────────────────────────────────────────
            ...
            drop it and every card in the feed runs a count for a button nobody sees.

            ⚠️ THIS WAS `absolute right-2 top-[58%]` AND IT SAT ON TOP OF THE PRICE on the rent
            card — measured at 390px, covering the last forty pixels of the second-currency line.
            These are twin cards (Lee, 12 Aug: *"literally copy the code and make them identical
            when they have the same things"*), so the sale card carried the same latent collision
            and gets the same cure rather than waiting to be reported. IN FLOW, right-aligned at
            the top of the scrim. Never give this an `absolute` with a percentage again. */}
        <div className="pointer-events-auto mb-1.5 flex justify-end px-3.5">
          <ListingEngagement
        ...
        <Link to={productHref("onesale", `/s/${l.id}`)}
          ...
          {/* ⚠️ THE SAME COMPONENT AS RENT, NOT A COPY OF IT. Lee, 16 Sep 2026: *"What I'm
              saying to you needs to be consistent, 100% consistent across sales and rentals."*
              Two hand-maintained copies is what kept dropping one side of every instruction. */}
          <CardText
            {...factLines(listingFacts(KIND_LABEL[l.kind][es ? "es" : "en"], l, lang), KIND_LABEL[l.kind][es ? "es" : "en"], lang)}
            where={where}
            priceMain={drawPrice(l.asking_price, ccy, fx)}
            /* ⚠️ THE SALE CARD HAD NO SECOND CURRENCY AT ALL. Lee: *"On the right side of the
               sale listing you don't have the equivalent price comparison. For the rent side you
               do."* Same rule as rent now: when the seller quotes in pesos, the reader sees their
               own currency on top and the pesos underneath. */
            priceAlt={altPrice(l.asking_price, l.country, drawPriceDisplay(l.asking_price, ccy, fx).currency, fx ?? l.display_fx_rate, cop, n => drawPrice(n, "USD", null))}
            onDark
            extra={l.status === "under_offer" ? (
              <span className="mt-1 rounded-full border border-amber-300/70 px-2 py-0.5 text-[10.5px] font-black uppercase text-amber-200">
