...
import { HUB_PANEL } from "../../shared/HubPanels";
...
export default function MyProperties() {
  ...
  return (
    ...
    <div className="space-y-4 pb-28">
      ...
      {/* ── THE PERMANENT WAY IN TO PLANS ──────────────────────────────────────────────────
          Lee, 18 September 2026: *"we need to make sure that there's a section to actually get
          to subscription services. Like how does someone even subscribe?"*

          The plans screen was only ever reachable from the drawer, behind the hamburger, and Lee
          — who commissioned it — could not find it. The limits in the listing form now each carry
          a link, which catches somebody at the moment they want more. This catches the other
          moment: a host standing in their own portfolio, deciding whether this is worth paying
          for. It states what they are on rather than only advertising what they are not, because
          a Pro subscriber needs to be able to confirm they are on Pro. */}
      <Link to={productHref("onerental", "/plans")}
        className="ow-tap card flex items-center justify-between gap-3 px-4 py-3">
        <span className="min-w-0">
          <span className="block text-[11.5px] font-black uppercase tracking-wide opacity-50">
            {W(lang, "Your plan", "Tu plan")}
          </span>
          <span className="block text-[14.5px] font-black">
            {plan === "vip" ? "VIP" : plan === "pro" ? "Pro" : W(lang, "Free", "Gratis")}
            {plan === "vip" && (
              <span className="ml-2 text-[11.5px] font-bold opacity-60">
                {W(lang, "every One World app", "todas las apps de One World")}
              </span>
            )}
          </span>
        </span>
        <span className="shrink-0 text-[12.5px] font-bold text-brand-deep underline decoration-dotted underline-offset-4 dark:text-brand-light">
          {plan === "vip" ? W(lang, "Manage", "Gestionar") : W(lang, "See plans", "Ver planes")}
        </span>
      </Link>
      ...
      <Link to={productHref("onerental", "/requests")}
        className="ow-tap card flex items-center gap-3 px-4 py-3.5">
        ...
        {pending > 0 && (
          <span className="grid h-[22px] min-w-[22px] shrink-0 place-items-center rounded-full bg-brand px-1.5 text-[11.5px] font-black tabular-nums text-white">
            {pending}
          </span>
        )}
      </Link>
      ...
      {mine && mine.length> 0 && (
        <div className={`${HUB_PANEL} flex gap-1.5 overflow-x-auto p-1.5`}>
          {([
...
function Metric({ value, label, brand, warn }: { value: number; label: string; brand?: boolean; warn?: boolean }) {
  ...
  return (
    /* ⚠️ 45 PERCENT WHITE ON THE AURORA. Lee, 20 Sep 2026, about this page: *"we got a lot of
       this whole 70, 80, 90 percent translucency here, and this is not correct… it's so
       transparent you can't tell what's going on."*

       The same correction as the property hub: a surface that holds content is solid, and the
       lighter weights are for things INSIDE it. `HUB_PANEL` is that weight, imported rather than
       re-typed so the two screens cannot drift apart again. */
    <div className={`${HUB_PANEL} p-3`}>
