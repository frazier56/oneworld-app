 ...
 *   · Stripe — ready ONLY when Stripe itself reports `payouts_enabled`. "Charges enabled" is a
 *     different capability (taking card payments) and never counts here.
    ...
    <div className="space-y-3" data-testid="payout-methods" data-for-user={userId}>
      ...
      <div className="flex items-center gap-3 rounded-xl border border-ink/10 p-3 dark:border-white/12">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[12.5px] font-black">{W(lang, "Bank payouts through Stripe", "Pagos a su banco por Stripe")}</p>
          <p className="text-[11.5px] leading-relaxed opacity-60" data-stripe-for={stripeBound ? stripe.user_id ?? "" : ""}>
            {!stripeBound ? W(lang, "Checking…", "Verificando…")
              : stripe.error ? W(lang, "Could not check Stripe just now.", "No se pudo consultar Stripe ahora.")
              : stripe.payouts_enabled
                ? `${W(lang, "Payouts enabled", "Pagos habilitados")}${stripe.bank_last4 ? ` · •••• ${stripe.bank_last4}` : ""}`
                : stripe.has_account
                  ? W(lang, "Set-up not finished — Stripe has not enabled payouts yet.", "Configuración incompleta — Stripe aún no habilitó los pagos.")
      ...
      {!compact && (
        <p className="text-[11.5px] leading-relaxed opacity-60">
          {W(lang,
            `Stripe pays into your bank. A PayPal or Wise link is "on file", not verified — the other person pays you through that provider and the money never passes through ${productLabel}. Remitly, bank transfer and cash are paid outside the platform and are never stored here.`,
            `Stripe paga a su banco. Un enlace de PayPal o Wise queda "registrado", no verificado — la otra persona le paga por ese proveedor y el dinero nunca pasa por ${productLabel}. Remitly, transferencia y efectivo se pagan fuera de la plataforma y nunca se guardan aquí.`)}
        </p>
      )}
