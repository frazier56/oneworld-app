    ...
    <div>
      ...
      {mode === "cover" && (
        <div className="relative overflow-hidden rounded-2xl border border-ink/[0.08] dark:border-white/10">
          <div
            ...
            {photos.map((src, i) => (
              <button key={src} type="button" onClick={() => open(i)} aria-label={photoLabel(lang, i, photos.length)}
                className="ow-tap block h-full w-full shrink-0 snap-center">
                <img decoding="async" src={src} alt={i === 0 ? alt : ""} loading={i === 0 ? "eager" : "lazy"}
      ...
      {mode === "grid" && (
        <div className="grid gap-1.5"
          ...
          {photos.map((src, i) => (
            <button key={i} type="button" onClick={() => open(i)} aria-label={photoLabel(lang, i, photos.length)}
              className="ow-tap overflow-hidden rounded-xl border border-ink/[0.08] dark:border-white/10">
              <img decoding="async" src={src} alt="" loading={i < 6 ? "eager" : "lazy"}
