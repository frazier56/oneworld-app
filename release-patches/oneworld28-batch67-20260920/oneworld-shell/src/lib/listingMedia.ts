 ...
 * Promoted from `onerental/lib/media.ts` into the shell on 20 Sep 2026 (OneEvent 30) so rent, sale
 * and events share ONE definition. `events` carries the same five columns (migration
 * `oneevent_media_cover_and_feed_preview`, trigger `event_media_choices_guard`).
 * ============================================================================================
...
/** Keep the host's chosen preview first in every feed layout, then retain all other
 * public videos and cover-ordered photos. The same list drives swiping and fullscreen. */
export function feedSlides(p: MediaRow): FeedPreview[] {
  const lead = feedLead(p);
  const slides: FeedPreview[] = lead ? [lead] : [];
  const seen = new Set(slides.map(x => x.url));
  for (const media of [
    ...(p.videos ?? []).map(url => ({ kind: "video" as const, url })),
    ...orderedPhotos(p).map(url => ({ kind: "photo" as const, url })),
  ]) {
...
export function mediaRowOf(p: MediaRow): MediaRow {
