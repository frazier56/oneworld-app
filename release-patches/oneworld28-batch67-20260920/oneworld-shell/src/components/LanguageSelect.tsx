...
export default function LanguageSelect() {
 ...
 className="ow-edge ow-tap flex min-h-[44px] items-center gap-2 rounded-xl border px-3 py-1.5 text-[13px] font-semibold ">
        <img decoding="async" src={flagSrc(current?.cc)} alt="" className="h-3 w-4.5 rounded-[2px] object-cover" />
        {current?.country}
        ...
        <div role="listbox" aria-label={t("language")}
          ...
          {list.map(l => (
            <button key={l.country} role="option" aria-selected={lang === l.code}
              ...
              <img decoding="async" src={flagSrc(l.cc)} alt="" className="h-3 w-4.5 shrink-0 rounded-[2px] object-cover" />
              {l.country}
            </button>
          ))}
        </div>
