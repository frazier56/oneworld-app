 ...
 */
/* Which currency does the PROPERTY's own country use. */
function localCcyOf(country: string | null | undefined): "USD" | "COP" {
  const c = (country || "").trim().toLowerCase();
  return c === "co" || c === "col" || c === "colombia" ? "COP" : "USD";
}

export function altPrice(amount: number, country: string | null | undefined, primaryCurrency: string, rate: number | null | undefined, cop: (usd: number, rate: number) => string, usd: (n: number) => string, sourceCurrency = "USD"): string | null {
  /* ⚠️ I KEYED THIS OFF THE LISTING'S QUOTED CURRENCY AND THAT IS WHY LEE STILL SAW ONE FIGURE.
     16 Sep 2026: *"Why is the Santa Rosa de Osos listing $4,500 a month — where is the equivalent
     COP? We just went over this."*

     His rule was always about the PROPERTY'S COUNTRY: *"the passive value will be the location of
     the property."* I wrote it against `currency`, the field holding what the HOST chose to type
     the number in. That host typed dollars — so by my code the property "was" a dollar property,
     the reader was also in dollars, and the second line correctly concluded there was nothing to
     convert. Correct logic, wrong input. A place in Colombia is a Colombian place whatever
     currency its owner prefers to quote. */
  const local = localCcyOf(country);
  // Compare the denomination actually rendered above, including missing-FX fallback.
  if (local === primaryCurrency.toUpperCase()) return null;
  if (!Number.isFinite(amount)) return null;
  // Country selects the secondary denomination; source currency defines the stored amount.
  const source = sourceCurrency.toUpperCase();
  if (source !== "USD" && source !== "COP") return null;
  if (source === local) return local === "COP" ? `≈ ${cop(amount, 1)}` : `≈ ${usd(amount)} USD`;
  if (!rate || !Number.isFinite(rate) || rate <= 0) return null;
  if (local === "COP") {
    return `≈ ${cop(amount, rate)}`;
  }
  return `≈ ${usd(amount / rate)} USD`;
}
