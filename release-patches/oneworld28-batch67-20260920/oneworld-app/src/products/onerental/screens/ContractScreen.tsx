...
import {
  ...
  RENTAL_HOST_FEE_RATE, RENTAL_GUEST_FEE_RATE, RENTAL_FEE_RATE,
} from "@oneworld/shell";
import {
  ...
  rentalError, locationLine, type Property, type BillInterval, PROPERTY_COLUMNS,
...
import { agreementDestination, agreementDraftFields } from "../lib/agreementDestinations";
import MonthlyRentalPanel from '../components/MonthlyRentalPanel';
import { formatStoredAddress } from "../lib/storedAddress";
...
export default function ContractScreen() {
  ...
  /* Take the host to a block further down this page and leave the focus there, so a keyboard
     or screen-reader user arrives where a tapping user's eye arrives. */
  ...
  /* ── THE PROPERTY'S CLAUSES SEED THE AGREEMENT (Lee, 14 Sep 2026) ────────────────────────
     *"We obviously need to have the addendums as a part of the property."* Set once on the
     listing, ticked here automatically, still editable per agreement — and once the host has
     touched them on this agreement, or a draft exists, the property never overrides them again.
     `pickedSeeded` is what makes that "once": without it, every re-render of the property query
     would re-tick a clause the host had just deliberately unticked. */
  const pickedSeeded = useRef(false);
  useEffect(() => {
    if (pickedSeeded.current || !property) return;
    pickedSeeded.current = true;
    const fromProperty = Array.isArray(property.addendum_keys) ? property.addendum_keys : [];
    if (fromProperty.length) setPicked(fromProperty);
  }, [property]);
  ...
  /* ── "MODIFY THIS REQUEST" IS A NAVIGATION, NOT A PIECE OF LOCAL STATE ──────────────────
     Lee, 14 September 2026: *"The back button from the modify — when you click on 'Modify this
     request', you wanna click back. It doesn't really work that well."*

     It could not work at all. Edit mode was a `useState` boolean, so the browser's Back — and the
     phone's back gesture, which is the one people actually use — had no idea the screen had
     changed. Pressing it left the contract entirely, from what looked like a sub-screen you had
     opened. The way out was one specific button at the bottom of a long page.

     This is the same scar as the listing form's first tap, and it has the same fix, written down
     in `ListHub.tsx`: component state cannot survive a remount and cannot be navigated; the URL
     can do both. Entering pushes `?edit=1`, so Back returns to the review. Leaving replaces it,
     so the history does not fill up with a toggle. */
  const editingLeaseDetails = !selectedRequest || searchParams.get("edit") === "1";
  const setEditingLeaseDetails = (next: boolean | ((value: boolean) => boolean)) => {
    const value = typeof next === "function" ? next(editingLeaseDetails) : next;
    const updated = new URLSearchParams(searchParams);
    if (value) updated.set("edit", "1"); else updated.delete("edit");
    setSearchParams(updated, { replace: !value });
  };
  ...
  const restoredRequestTerms = useRef({ startsOn: false, months: false, nights: false });
  const requestTermsSeededKey = useRef<string | null>(null);
  const [requestTermsReadyKey, setRequestTermsReadyKey] = useState<string | null>(null);
  const ownerTermsAllowed = property?.owner_terms_enabled === true;

  /* ── ⚠️ "FILL THEM IN" USED TO DO NOTHING, SILENTLY ──────────────────────────────────────
     Lee, 14 Sep 2026: *"It's not correct where you say 'fill them in'… clicking those little
     things, it doesn't necessarily work. The landlord passport does not work because it's on
     different pages."*

     He is exactly right, and the mechanism is these four keys. The landlord's legal name, ID type,
     ID number and notification address are NOT typed on this screen — they are read from the host
     profile, which is a different page. So `document.getElementById("lease-field-arrendador_doc")`
     returns null, `null?.focus()` is a no-op, and the button reports success by saying nothing.

     A control that half works is worse than one that refuses: the host taps it, the page does not
     move, and they conclude the app is broken — which, from where they are standing, it is.

     Two fixes, and both are needed:
       1. `focusField` now REPORTS whether it found anything, so a miss can never be mistaken for
          a jump that simply had nowhere to scroll.
       2. A key owned by another screen is routed to that screen, carrying a `return` so Save
          brings the host straight back here. The host profile already honours `?return=`. */
  const HOST_PROFILE_KEYS = new Set([
    "arrendador_nombre", "arrendador_doc_tipo", "arrendador_doc", "arrendador_direccion",
  ]);

  const agreementQuery = new URLSearchParams();
  if (selectedRequest) agreementQuery.set("request", selectedRequest);
  if (editingLeaseDetails) agreementQuery.set("edit", "1");
  const contractPath = `/rentals/r/${id}/contract${agreementQuery.size ? '?' + agreementQuery.toString() : ''}`;
  const listingEditPath = (step: string) => productHref("onerental", `/list?form=1&edit=${id}&step=${step}&return=${encodeURIComponent(contractPath)}`);
  const hostProfilePath = productHref("onerental", `/host-profile?return=${encodeURIComponent(contractPath)}`);

  const focusField = (key: string): boolean => {
    const container = document.getElementById(`lease-field-${key}`);
    if (!container) return false;
    const target = container.matches("input:not([disabled]),textarea:not([disabled]),button:not([disabled]),select:not([disabled]),a[href]")
      ? container as HTMLElement
      : container.querySelector<HTMLElement>("input:not([disabled]),textarea:not([disabled]),button:not([disabled]),select:not([disabled]),a[href]")
        || (container.hasAttribute('tabindex') ? container : null);
  ...
  /** The one entry point for "take me to this blank", wherever it lives. */
  const jumpToBlank = (key: string) => {
    const destination = agreementDestination(key, !!request);
    if (destination.screen === "host") { nav(hostProfilePath); return; }
    if (destination.screen === "listing") { nav(listingEditPath(destination.target)); return; }
    setEditingLeaseDetails(true);
    /* Two frames: the first commits the edit mode, the second paints the field it reveals. */
    requestAnimationFrame(() => requestAnimationFrame(() => {
      if (focusField(destination.target)) return;
      /* Neither here nor on a screen we know about. Say so — never fail quietly again. */
      setErr(W(lang,
        "This required detail could not be opened. Please review the agreement details before continuing.",
        "No se pudo abrir este dato obligatorio. Revise los detalles del contrato antes de continuar."));
    }));
  };
  ...
  useEffect(() => {
    ...
    setF(current => ({
      ...current,
      direccion: current.direccion || formatStoredAddress(property.address_line ?? "") || "",
  ...
  useEffect(() => {
    ...
    requestTermsSeededKey.current = null;
    restoredRequestTerms.current = { startsOn: false, months: false, nights: false };
    setRequestTermsReadyKey(null);
    ...
    try {
      ...
      if (raw) {
        ...
        setF({ ...DEFAULT_CONTRACT_FIELDS, ...agreementDraftFields(saved.f) });
        ...
        if (typeof saved.startsOn === "string") { setStartsOn(saved.startsOn); restoredRequestTerms.current.startsOn = true; }
        if (typeof saved.months === "string") { setMonths(saved.months); restoredRequestTerms.current.months = true; }
        if (typeof saved.nights === "string") { setNights(saved.nights); restoredRequestTerms.current.nights = true; }
        /* A saved draft wins: the host already made a choice on THIS agreement. Only a draft
           that predates the property-level list falls through to the property's default below. */
        if (Array.isArray(saved.picked)) { setPicked(saved.picked.filter((key: unknown) => typeof key === "string")); pickedSeeded.current = true; }
  ...
  /* Load the scoped draft before seeding request defaults. Query completion or a guest
     refresh must not replace the host's saved term. A monthly renewal retains its fixed start. */
  useEffect(() => {
    if (hydratedDraftKey !== draftKey || !request || !property || property.id !== id) return;
    if (selectedRequest && request.id !== selectedRequest) return;
    setTenantName(request.guest_name ?? "");
    if (requestTermsSeededKey.current === draftKey) return;
    requestTermsSeededKey.current = draftKey;
    if (request.monthly || !restoredRequestTerms.current.startsOn) setStartsOn(request.starts_on);
    if (property.price_unit === "night") {
      if (!restoredRequestTerms.current.nights) setNights(String(request.nights || 1));
    } else if (!restoredRequestTerms.current.months) {
    ...
    setRequestTermsReadyKey(draftKey);
  }, [hydratedDraftKey, draftKey, request?.id, request?.guest_name, property?.id, id, selectedRequest, displayName]);

  useEffect(() => {
    if (hydratedDraftKey !== draftKey) return;
    // Wait for the render containing request defaults before saving a request-linked draft.
    if (selectedRequest && requestTermsReadyKey !== draftKey) return;
    try {
      ...
      const {
        ...
        direccion: _propertyAddress,
        cuenta_devolucion: _refundAccount,
        titular_cuenta: _refundAccountHolder,
        ...draftFields
      } = f;
  ...
  }, [hydratedDraftKey, draftKey, selectedRequest, requestTermsReadyKey, f, tenantEmail, tenantName, startsOn, months, nights, picked,
  ...
  useEffect(() => {
    if (!property || hydratedDraftKey !== draftKey || !isResidential) return;
  ...
  }, [isResidential, property, hydratedDraftKey, draftKey]);
  ...
  const tv = useMemo(() => ({
    ...
    ...f,
    direccion: formatStoredAddress(property?.address_line ?? "") || "",
    ...
    fecha_fin: endsOn,
    arrendatario_nombre: request ? (request.guest_name ?? "") : tenantName,
    ...
    request?.id, request?.guest_name, request?.monthly, contractCurrency, rent]);
  ...
  const activeMissingKeys = [...new Set([...(useTemplate ? missingKeys : optionalMissingKeys),
    ...(request && !request.guest_name?.trim() ? ['arrendatario_nombre'] : [])])];
  const blanks = activeMissingKeys.length;
  /* Which of the blanks this screen can actually fill, and which belong to the host profile.
     See the note on `focusField` — this split is what makes "Fill them in" a true sentence. */
  const hostProfileBlanks = activeMissingKeys.filter(k => HOST_PROFILE_KEYS.has(k));
  const listingBlanks = activeMissingKeys.filter(k => agreementDestination(k, !!request).screen === "listing");
  const guestBlanks = activeMissingKeys.filter(k => agreementDestination(k, !!request).screen === "guest");
  const onScreenBlanks = activeMissingKeys.filter(k => agreementDestination(k, !!request).screen === "field");
  ...
  /* ⚠️ THE RATE IS NOT TYPED HERE, AND IT USED TO BE — 12 Sep 2026.
     These two lines read `? 0.09 : 0.015` and `? 0 : 0.075`, which is the same defect the
     database has in `set_rental_contract_fee_allocation`: the rate has one home, and this
     bypassed it. The consequence here is worse in one way, because this is the number a HOST
     READS on the contract preview. After a rate change the screen would quote the old rate
     while the row stamped the new one, and the two would disagree on the same page.

     `RENTAL_FEE_RATE` is the sum of the two, never a third constant, so the host-pays-
     everything branch tracks the split automatically.

     A saved request still wins over all of it: a quoted rate belongs to the quote. */
  const contractHostRate = request ? Number(request.host_fee_rate)
    : listingFee.waived ? 0
    : (property?.host_pays_guest_fee ? RENTAL_FEE_RATE : RENTAL_HOST_FEE_RATE);
  const contractGuestRate = request ? Number(request.guest_fee_rate)
    : listingFee.waived ? 0
    : (property?.host_pays_guest_fee ? 0 : RENTAL_GUEST_FEE_RATE);
  ...
  async function send() {
    ...
    if (activeMissingKeys.length) {
      ...
      /* ⚠️ "The first one is ready for you below" was a promise this could not keep when the
         first blank was a host-profile field — the screen did not move and nothing was below.
         The sentence now matches where the jump is actually about to take them. */
      const first = onScreenBlanks[0] ?? activeMissingKeys[0];
      setErr(W(lang, "Complete the required details using the highlighted field or correction destination.", "Complete los datos obligatorios usando el campo resaltado o el destino de corrección."));
      requestAnimationFrame(() => jumpToBlank(first));
      return;
    }
    ...
    const { data, error } = await supabase.from("rental_contracts").insert({
      ...
      tenant_name: (request ? (request.guest_name ?? "") : tenantName).trim() || null,
      tenant_email: request ? null : tenantEmail.trim() || null,
      starts_on: startsOn,
      ends_on: endsOn,
    ...
    const snapshot = buildSnapshot({
      ...
      /* A lease names the property. Naming it "Bright two-bedroom — El Poblado, Medellín"
         and stopping there leaves out the department and the country, which is precisely
         what a document served on somebody has to be unambiguous about. */
      property: `${property.title} — ${locationLine(property, lang)}`,
  ...
  return (
    <div className="space-y-4">
      ...
      <div id="lease-host-profile" tabIndex={-1} className={`card flex items-center gap-3 p-3 outline-none focus:ring-2 focus:ring-brand/40 ${hostSetupReady ? "" : "border border-amber-400/35"}`}>
        ...
        <Link className="shrink-0 text-[12px] font-black text-brand underline" to={hostProfilePath}>{hostSetupReady ? W(lang, "Review", "Revisar") : W(lang, "Complete", "Completar")}</Link>
      </div>

      {property.price_unit==='month' && <MonthlyRentalPanel propertyId={property.id} userId={userId} hostId={property.agent_id} amount={property.price} currency={property.currency || 'COP'} lang={lang} selectedRequestId={request?.id} terms={property}/>}
      ...
      {request && (
        <div id="lease-request-lifecycle" tabIndex={-1} className="card p-4 outline-none focus:ring-2 focus:ring-brand/40">
          ...
          {/* ⚠️ THIS ROW USED TO BE A TINTED PILL WITH ITS OWN 12px SIDE PADDING, inside a card
              whose every other line sits flush at the card's edge. Lee, 14 Sep 2026: *"you got
              some stuff that's further left justified than the other stuff."* He was looking at
              exactly this — the name, the heading and the dates start at one margin and the money
              started 12 pixels inside it. A tint is not worth breaking the one straight left edge
              the block has, so the emphasis comes from a rule above it instead. */}
          <div className="mt-3 flex flex-wrap items-baseline justify-between gap-x-3 border-t border-ink/10 pt-2.5 text-sm dark:border-white/15">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            {/* ── THE TWO STATUS TILES ────────────────────────────────────────────────────
                Three things Lee asked for on 15 Sep, and one he did not have to:

                1. ⚠️ **THE WRAP.** It read "Submitted · review / needed" — the line broke
                   BETWEEN two words that mean one thing. *"Review needed"* is the detail;
                   *"Submitted"* is the status. They are two separate elements now, so the
                   split is structural and cannot land in the wrong place at any width.
                   His rule, said many times: break where the MEANING breaks.
                2. **A timestamp.** "Submitted" without "when" is half a fact. Taken from the
                   document's own `created_at`, and from `payment_declared_at` /
                   `payment_received_at` — the real events, never invented.
                3. **They are doors.** The host could already open the tenant's ID — the review
                   block sits further down this page — but nothing on the tile said so, so the
                   status read as a dead end. Tapping now takes you to it.
                4. Subtle depth rather than a flat outline: a soft shadow and a light glass
                   fill. It is a box inside a box, so it stays quiet.

                ⚠️ A TILE IS ONLY A BUTTON WHEN THERE IS SOMEWHERE TO GO. With nothing to
                open it renders as plain text — never a control that looks live and does
                nothing, which is the thing he has said outright never to ship. */}
            <StatusTile
              id="lease-request-identity"
              label={W(lang, "Identity", "Identidad")}
              status={request.identity_status === "approved" ? W(lang, "Approved", "Aprobada")
                : request.identity_status === "submitted" ? W(lang, "Submitted", "Enviada")
                : W(lang, "Not submitted", "No enviada")}
              detail={request.identity_status === "submitted" ? W(lang, "Review needed", "Falta revisar") : null}
              when={identityDocument?.created_at ?? null}
              tone={request.identity_status === "approved" ? "good"
                : request.identity_status === "submitted" ? "attention" : "idle"}
              openLabel={W(lang, "Open ID", "Ver identificación")}
              onOpen={identityDocument ? () => jumpTo("lease-identity-review") : null}
              lang={lang}
            />
            <StatusTile
              id="lease-request-payment"
              label={W(lang, "Payment", "Pago")}
              status={request.payment_status === "authorized" ? W(lang, "Card authorized", "Tarjeta autorizada")
                : request.payment_status === "received" ? W(lang, "Received", "Recibido")
                : request.payment_declared_at ? W(lang, "Sent", "Enviado")
                : W(lang, "Pending", "Pendiente")}
              detail={request.payment_status === "received" ? null
                : request.payment_declared_at ? W(lang, "Confirm receipt", "Confirmar recepción")
                : W(lang, "Not sent yet", "Aún no enviado")}
              when={request.payment_received_at ?? request.payment_declared_at ?? null}
              tone={request.payment_status === "received" ? "good"
                : request.payment_declared_at ? "attention" : "idle"}
              openLabel={W(lang, "Details", "Detalles")}
              onOpen={request.payment_declared_at ? () => jumpTo("lease-payment-detail") : null}
              lang={lang}
            />
          </div>

          {request.payment_rail !== 'stripe' && request.payment_declared_at && (
            <div id="lease-payment-detail" tabIndex={-1} className="scroll-mt-24 outline-none focus:ring-2 focus:ring-brand/40 rounded-2xl">
              <ReservationPaymentHelp requestId={request.id} lang={lang} />
            </div>
          )}
          ...
          {(requestStage ?? request.approval_stage) === "requested" && request.state === 'requested' && new Date(request.expires_at).getTime() > Date.now() && (
            <div id="lease-request-preapproval" tabIndex={-1} className="mt-3 rounded-2xl border border-sky-500/20 bg-sky-500/5 p-3 outline-none focus:ring-2 focus:ring-red-400/50">
              ...
              <button type="button" disabled={busy} onClick={preapproveRequest}
                ...
                {/* A LABEL, NOT A SENTENCE — Lee's standing rule, and this one wrapped onto two lines
                    at phone width. What it does is already spelled out in the paragraph directly
                    above it ("Pre-approval sends the tenant one request for their identity and
                    payment"), so repeating it on the button was redundancy AND a wrap. */}
                {busy ? W(lang, "Saving…", "Guardando…") : W(lang, "Pre-approve", "Preaprobar")}
              </button>
            </div>
          )}
          ...
          {identityDocument && request.identity_status === "submitted" && (
            <div id="lease-identity-review" tabIndex={-1} className="mt-3 scroll-mt-24 rounded-2xl border border-ink/12 bg-white/45 p-3 shadow-sm outline-none focus:ring-2 focus:ring-brand/40 dark:border-white/15 dark:bg-white/[0.06]">
      ...
      {request && !request.guest_name?.trim() && <div id="lease-field-guest-identity" tabIndex={-1} className="card p-4 outline-none focus:ring-2 focus:ring-amber-400">
        <p className="font-bold">{W(lang, "The tenant's One ID is missing their name", "Falta el nombre en el One ID del arrendatario")}</p>
        <p className="mt-2 text-sm">{W(lang, "The tenant must complete their own profile name. You cannot replace their identity in this agreement. Once they have updated it, refresh the request below.", "El arrendatario debe completar su nombre en su propio perfil. Usted no puede reemplazar su identidad en este contrato. Cuando lo actualice, recargue la solicitud abajo.")}</p>
        <button type="button" className="btn-ghost mt-3 min-h-11" onClick={() => setRequestRefresh(v => v + 1)}>{W(lang, "Refresh tenant details", "Actualizar datos del arrendatario")}</button>
      </div>}
      {/* ── WHO ──────────────────────────────────────────────────────────────────────────── */}
      {!request && <Section title={W(lang, "The tenant", "El arrendatario")}>
        <div className="space-y-2">
          <input id="lease-field-arrendatario_nombre" aria-label={W(lang, "Tenant full name", "Nombre completo del arrendatario")} className="input w-full" value={tenantName} readOnly={!!request} onChange={e => setTenantName(e.target.value)}
      ...
      {(!request || editingLeaseDetails) && <Section title={W(lang, "Term", "Término")}
        ...
        {request && !editingLeaseDetails ? (
          ...
          <label id="lease-field-fecha_inicio" tabIndex={-1} className="block">
          ...
          <label id="lease-field-duracion" tabIndex={-1} className="block">
      ...
      {/* ── MONEY COMES BEFORE THE AGREEMENT (Lee, 14 September 2026) ─────────────────────
         *"The agreements are probably after the price."* He is right, and it is the order of
         the decision rather than the order of the document: a host deciding whether to accept a
         tenant asks what they will be paid first, and reads the clauses they are agreeing to
         second. Money sat below the agreement, the addendums and eleven optional clauses — so
         the one number the whole screen exists to answer was the furthest thing down it. */}
      {/* ── MONEY, STATED BEFORE ANYONE SIGNS ────────────────────────────────────────────── */}
      <Section title={W(lang, "Money", "Dinero")}>
        {/* ── THIS SECTION IS FOR THE HOST. IT WAS WRITTEN FOR AN ACCOUNTANT ─────────────────
            Lee, 14 September 2026, reading it on a real request:

              *"You got something like ten lines… 'Payments over the term' — who the hell knows
              what that even means? 'You receive per payment' doesn't make any sense… And the
              guest fee per payment — do they even need to know the guest fee? No. The host needs
              to know their fee. The guest needs to know their fee… 'Guest pays per payment' right
              below 'You received per payment'. Like, come on. Don't be confusing."*

            He is right on every count, and the nine rows had one cause: the screen showed BOTH
            sides of the transaction to ONE side of it. A host reading their own contract does not
            need the renter's fee, the renter's total, or the sum of the two — those are the
            renter's business, shown to the renter on the renter's screen. Every row here answers
            a question the person looking at it actually has:

              what is the rent · what is the deposit · what does OneHome take · what do I get.

            Three other fixes ride along:
            · "Payments over the term: 1" was a row printing the number one. It now only appears
              when there IS more than one payment, and it appears as part of the rent line where
              it means something.
            · "− $0" — a minus sign in front of nothing. When the fee is zero, and during the beta
              it is zero for everybody, the whole fee block collapses to one plain sentence.
            · Absorbing the renter's fee was invisible here. The toggle exists on the listing form
              and the contract never mentioned it. It now says so in words, with the amount, not
              a percentage — Lee: *"it shows up in the actual amount, not a percentage."* */}
        <Row label={W(lang, "Rent", "Canon")}
          value={cycles > 1
            ? W(lang, `${money(rent)} × ${cycles} payments`, `${money(rent)} × ${cycles} pagos`)
            : money(rent)} />
        {!!dep && <Row label={W(lang, "Deposit", "Depósito")} value={money(dep)} />}
        {!request && listingFee.pending
          ? <p role="status">{W(lang, "Checking listing fees…", "Consultando comisiones…")}</p>
          : !request && listingFee.error
            ? <button className="btn-ghost" onClick={listingFee.retry}>{W(lang, "Fees unavailable. Try again", "Comisiones no disponibles. Reintentar")}</button>
            : hostFee(rent, contractHostRate) <= 0
              ? <p className="text-[12.5px] leading-relaxed opacity-65">
                  {W(lang, "No OneHome fee on this letting. You receive the full rent.",
                           "Sin comisión de OneHome en este arriendo. Usted recibe el canon completo.")}
                </p>
              : <>
                  <Row label={property?.host_pays_guest_fee
                    ? W(lang, "OneHome fee — you are covering the renter's share too",
                             "Comisión de OneHome — usted también cubre la parte del arrendatario")
                    : W(lang, "OneHome fee", "Comisión de OneHome")}
                    value={`− ${money(hostFee(rent, contractHostRate))}`} />
                  <Row bold label={W(lang, "You receive", "Usted recibe")}
                    value={money(netToManager(rent, contractHostRate))} />
                  {cycles > 1 && (
                    <Row label={W(lang, `You receive over ${cycles} payments`, `Usted recibe en ${cycles} pagos`)}
                      value={money(rent * cycles - hostTotalFee)} />
                  )}
                </>}
        {!!dep && (
      ...
      <Section title={W(lang, "The agreement", "El acuerdo")}>
        {request && !editingLeaseDetails ? <>
          {/* ⚠️ THIS SENTENCE USED TO SAY "ALREADY FILLED" WHILE THE PREVIEW BELOW IT SHOWED
              ROWS OF UNDERSCORES. Lee, 14 September 2026: *"The contract itself has dashes and
              gaps in it. I don't know how you think you can have a contract that has blanks in
              it. That's stupid. When you edit it, you gotta be able to edit the pieces with the
              blanks in it."*

              Both halves were real. Sending was already blocked — `attempt()` refuses and focuses
              the first empty field — so a contract with blanks could never actually reach a
              tenant. What it could do was sit in front of the host looking finished and broken at
              the same time, with a line of copy insisting it was complete and no way in from
              here to the field that would fix it.

              A blank is `fillTemplate` substituting "____" for a value nobody supplied. So the
              screen now counts them, says so instead of reassuring, and opens the editor on the
              first one. The count is live: fill the last blank and this turns back into the
              sentence it used to be, which is the only condition under which that sentence was
              ever true. */}
          {blanks > 0 ? (
            <div className="rounded-xl border border-amber-400/40 bg-amber-400/[0.08] p-3">
              <p className="text-[12.5px] font-bold text-amber-800 dark:text-amber-200">
                {W(lang,
                  `${blanks} detail${blanks === 1 ? " is" : "s are"} still blank in this agreement.`,
                  `${blanks === 1 ? "Falta un dato" : `Faltan ${blanks} datos`} en este contrato.`)}
              </p>
              {/* ── ⚠️ SAY WHERE THEY ARE, NOT JUST HOW MANY ────────────────────────────────
                  A single count over a single button is what made this untrustworthy: some of
                  those blanks are on the host profile, so one button could never satisfy the
                  sentence it was making. The count is now split by the screen that owns it, and
                  each half gets the button that actually goes somewhere. */}
              <p className="mt-1 text-[12px] leading-relaxed opacity-75">{W(lang,
                "Each missing detail must be completed where it is managed. The agreement stays in draft until all required details are available.",
                "Cada dato faltante se completa donde se administra. El contrato sigue en borrador hasta que estén todos los datos obligatorios.")}</p>
              {listingBlanks.length > 0 && <button type="button" className="btn-ghost mt-2 min-h-11 w-full" onClick={() => jumpToBlank(listingBlanks[0])}>{W(lang, "Complete listing details", "Completar datos del anuncio")}</button>}
              {guestBlanks.length > 0 && <button type="button" className="btn-ghost mt-2 min-h-11 w-full" onClick={() => jumpToBlank(guestBlanks[0])}>{W(lang, "Review missing tenant name", "Revisar nombre faltante del arrendatario")}</button>}
              {onScreenBlanks.length > 0 && (
                <button type="button" className="btn-ghost mt-2 min-h-11 w-full"
                  onClick={() => jumpToBlank(onScreenBlanks[0])}>
                  {W(lang, "Fill them in", "Completarlos")}
                </button>
              )}
              {hostProfileBlanks.length > 0 && (
                /* A LABEL, NOT A SENTENCE — and it names the destination, so nobody taps it
                   expecting to stay on this page. Save there returns here automatically. */
                <Link to={hostProfilePath} className="btn-ghost mt-2 flex min-h-11 w-full items-center justify-center">
                  {W(lang, "Open host profile", "Abrir perfil de anfitrión")}
                </Link>
              )}
            </div>
          ) : (
          ...
          <>
            ...
            {(editingLeaseDetails || !request) && <div className="space-y-3">
              ...
              <p className="rounded-xl border border-ink/10 bg-ink/[0.025] p-3 text-[11.5px] leading-relaxed opacity-70 dark:border-white/12 dark:bg-white/[0.04]">
                ...
                {formatStoredAddress(property?.address_line ?? "") || W(lang, "Complete this on the listing.", "Complétela en el anuncio.")} <Link className="font-bold underline" to={listingEditPath("where")}>{W(lang, "Edit listing details", "Editar datos del anuncio")}</Link>
              </p>
            ...
            <div>
              ...
              <LeasePreview lang={L} values={tv} addendumKeys={picked}
                ...
                onBlankClick={key => { setShowValidation(true); jumpToBlank(key); }} />
            </div>
          </>
        ) : (
          <>
...
/**
 * A STATUS TILE — status, detail, when, and a way in.
 *
 * Lee, 15 Sep 2026, on "Submitted · review / needed": *"Review needed are the words that go
 * together, so naturally those two words should be on the same line… be more thoughtful when
 * you're wrapping text that you wrap text that belongs together."*
 *
 * The old tile put status and detail in ONE string joined by a bullet and let the browser break
 * it wherever it ran out of room — which is how "review" ended up on one line and "needed" on
 * the next. They are separate elements here, so the split is structural: the status is always
 * its own line, the detail is always its own line, at every width, in every language. There is
 * no arrangement of characters that can break them the wrong way.
 *
 * ⚠️ IT IS A BUTTON ONLY WHEN `onOpen` IS GIVEN. With nowhere to go it renders as plain text —
 * never a control that looks live and does nothing.
 */
function StatusTile({ id, label, status, detail, when, tone, openLabel, onOpen, lang }: {
  id: string; label: string; status: string; detail: string | null;
  when: string | null; tone: "good" | "attention" | "idle";
  openLabel: string; onOpen: (() => void) | null; lang: string;
}) {
  const es = lang === "es" || lang === "co";
  const stamp = when
    ? new Date(when).toLocaleDateString(es ? "es-CO" : "en-US", { day: "numeric", month: "short" })
    : null;
  const dot = tone === "good" ? "bg-emerald-500" : tone === "attention" ? "bg-amber-500" : "bg-ink/25 dark:bg-white/25";

  const inner = (
    <>
      <span className="flex items-center gap-1.5">
        <span aria-hidden className={`h-1.5 w-1.5 shrink-0 rounded-full ${dot}`} />
        <span className="text-[10.5px] font-black uppercase tracking-wide opacity-55">{label}</span>
      </span>
      {/* Status on its own line. Detail on its own line. Never one string that wraps. */}
      <span className="mt-1 block text-[13px] font-bold leading-snug">{status}</span>
      {detail && <span className="block text-[11.5px] font-semibold leading-snug opacity-70">{detail}</span>}
      {stamp && <span className="mt-1 block text-[10.5px] tabular-nums opacity-45">{stamp}</span>}
      {onOpen && (
        <span className="mt-1.5 block text-[11px] font-black text-brand-deep dark:text-brand-light">
          {openLabel} →
        </span>
      )}
    </>
  ...
  /* A soft shadow and a light glass fill instead of a bare outline — depth, but quiet, because
     this is a box inside a box (Lee: *"it doesn't need to be anything major"*). */
  const shell = "block w-full rounded-2xl border border-ink/12 bg-white/55 p-3 text-left shadow-sm backdrop-blur-[2px] outline-none scroll-mt-24 dark:border-white/15 dark:bg-white/[0.07]";

  if (!onOpen) {
    return <div id={id} tabIndex={-1} className={`${shell} focus:ring-2 focus:ring-brand/40`}>{inner}</div>;
  }
  return (
    <button type="button" id={id} onClick={onOpen}
      className={`${shell} ow-tap transition hover:bg-white/75 focus:ring-2 focus:ring-brand/40 active:scale-[.99] dark:hover:bg-white/[0.11]`}>
      {inner}
    </button>
  );
}
