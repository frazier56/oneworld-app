...
export default function RentalRequests({ conversationId, heading = true }: { conversationId?: string; heading?: boolean }) {
  ...
  return <section className="space-y-3" aria-label={W(lang, 'Rental requests', 'Solicitudes de arriendo')}>
    ...
    {state.key === key && state.error ? <div className="ow-panel rounded-3xl p-4" role="alert">
    ...
    </div> : !rows ? <div className="ow-panel ow-shimmer h-24 rounded-3xl" /> : !shown?.length ? <p className="text-sm opacity-60">{history ? W(lang, 'No past requests.', 'Sin solicitudes anteriores.') : W(lang, 'No current rental requests.', 'Sin solicitudes de arriendo actuales.')}</p> : shown.map(r => <article key={r.id} className="ow-panel space-y-2 rounded-3xl p-4">
