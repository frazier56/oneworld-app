...
export function ListingCard({ l, agent, lang, layout = "big", ccy = "USD", fx, propertyRatings, hostRatings }:
  ...
  if (layout === "row") {
    return (
      <Link to={href}
        className="ow-tap flex items-stretch gap-3 overflow-hidden rounded-[22px] border border-ink/[0.08] p-2 dark:border-white/10">
        {lead
  ...
  if (layout === "grid") {
    return (
      <Link to={href}
        className="ow-tap relative block overflow-hidden rounded-[26px] border border-ink/[0.08] dark:border-white/10">
        {lead
  ...
  return (
    ...
    <article className="ow-feed-card relative overflow-hidden rounded-[26px] border border-ink/[0.08] bg-ink/5 dark:border-white/10 dark:bg-white/5">
      {slides.length > 0 ? (
        <div
          ref={strip}
          onScroll={e => {
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 px-3 pb-2 pt-16 text-white"
        ...
        <TapTarget video={slides[idx]?.kind === "video"} href={href} onPlay={() => setPlaying(idx)}>
          {/* ⚠️ I PUT THE LOCATION ON THE WRONG SIDE AND LEE CAUGHT IT. 16 Sep 2026:
              *"You made a mistake and you put the location on the left side of the card. On the
              left is the type of property, bedroom, bathroom count, square footage, furnished —
              and below that the reviews. On the right is the price and the equivalent amount, and
              below that the city and the state."*

              The rule underneath it: the left column is WHAT THIS IS, the right column is WHAT IT
              COSTS AND WHERE. Every line belongs to one of those two questions, so a reader's eye
              only has to learn the split once and then it holds for every card.

              Both columns are bottom-aligned as one row, so the two blocks share a baseline
              however many lines either one carries. */}
          <div className="flex items-end justify-between gap-3">
            {/* ⚠️ THE REVIEWS LEFT THE FEED TILE. Lee, 16 Sep 2026: *"We're going to remove the
                property review and the host review from the actual tile on the feed page. We're
                going to reserve that for the actual property listing page."*

                Right, and it is the same argument that took the title and the heart off this
                card: a feed tile answers "is this worth opening", and for that the attributes and
                the price do the work. Two star ratings are how you judge a place you are already
                considering — they belong where the decision is actually made. */}
            <div className="flex min-w-0 flex-1 flex-col gap-1">
              <p className="text-[14px] font-black leading-snug">{facts}</p>
            </div>
            <div className="flex shrink-0 flex-col items-end text-right">
              <p className="text-[15px] font-black leading-snug tracking-tight">{priceLabel(l, lang, ccy, fx)}</p>
              {l.display_currency === "COP" && l.display_fx_rate && (
                <p className="mt-0.5 text-[11px] leading-none opacity-75">≈ {cop(l.price, l.display_fx_rate)}</p>
              )}
              {/* ⚠️ CITY ON ONE LINE, REGION ON THE NEXT, NO COUNTRY. Lee: *"the city is cut off —
                  Poblado del Sur, comma, then Santa, dot dot dot. That doesn't look clean."* Split
                  at the LAST comma so the administrative tail drops to line two and the city keeps
                  line one whole. The country is dropped: by the time somebody is reading a price
                  in their own currency, which country it is in is not news. */}
              {where && (() => {
                const cut = where.lastIndexOf(",");
                const city = cut === -1 ? where : where.slice(0, cut);
                const region = cut === -1 ? "" : where.slice(cut + 1).trim();
                return (
                  <p className="mt-1 max-w-[42vw] text-[12px] font-semibold leading-tight opacity-90">
