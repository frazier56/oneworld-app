    ...
    <article className="card overflow-hidden rounded-[26px] p-0">
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/55 to-transparent pt-20">
        ...
        <Link to={productHref("onesale", `/s/${l.id}`)}
          ...
          {/* ⚠️ SALE NOW LOOKS EXACTLY LIKE RENT, BECAUSE LEE ASKED FOR ONE CARD. 16 Sep 2026:
              *"All properties for sale and all properties for rent should have this update. Right
              now the properties for sale don't look like this at all — they have a whole other
              setup and they should not."*

              Two different card layouts for the same act — deciding whether to open a place — is
              two things to learn for no gain. The shape is rent's, and the rule under it is the
              one Lee set: LEFT is what this is and where; RIGHT is what it costs.

              Title and host stars are gone here too, for the same reasons they left the rent
              card: a host's title is free text and often marketing, and a star rating is how you
              judge a place you are already considering, not how you decide to look. */}
          <div className="flex items-end justify-between gap-3">
            <div className="flex min-w-0 flex-1 flex-col gap-1">
              {/* Attributes wrap naturally: type, beds and baths fill the first line, size and
                  furnished drop to the second. No forced break — the widths differ per listing
                  and a hard split leaves one line short on half of them. */}
              <p className="text-[14px] font-black leading-snug">
                {listingFacts(KIND_LABEL[l.kind][es ? "es" : "en"], l, lang).join(" · ")}
              </p>
              {/* ⚠️ CITY ON ONE LINE, REGION ON THE NEXT, NO COUNTRY — split at the LAST comma so
                  the administrative tail drops and the city keeps its line whole. */}
              {where && (() => {
                const cut = where.lastIndexOf(",");
                const city = cut === -1 ? where : where.slice(0, cut);
                const region = cut === -1 ? "" : where.slice(cut + 1).trim();
                return (
                  <p className="text-[12px] font-semibold leading-tight opacity-90">
            ...
            <div className="flex shrink-0 flex-col items-end text-right">
              <p className="text-[15px] font-black leading-snug tracking-tight">{drawPrice(l.asking_price, ccy, fx)}</p>
              {l.display_currency === "COP" && l.display_fx_rate && (
                <p className="mt-0.5 text-[11px] leading-none opacity-75">≈ {cop(l.asking_price, l.display_fx_rate)}</p>
              )}
