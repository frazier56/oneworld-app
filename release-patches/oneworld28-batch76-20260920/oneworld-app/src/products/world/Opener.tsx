...
/* v5.2 — the drawn Skip pill removed, and the last second dissolves into the signed-out home
   instead of the Create form. Each cut ships under its OWN name: staging is add-only, so writing
   over a path already in the repo is a hard stop, and it would break rollback for every batch
   still pointing at the older file. v5 and v5.1 stay where they are, unreferenced. */
...
export default function Opener({ replayAlways }: {
  ...
  /* ── THE SCREEN THAT IS WAITING UNDERNEATH ────────────────────────────────────────────────
     Signed out, the film hands over to the signed-out One World home — `/yourworld`, the splash
     with **Create account** and **Sign in** on it. NOT `/join`.

     ⚠️ THIS WAS `/join` AND IT WAS WRONG (R13, 21 September 2026). Lee, on v5.1: the film ended
     on the Create-your-One-ID FORM, *"as if the person had already chosen to sign up."* Landing
     a first-time visitor mid-signup takes the choice away from them before they have been asked
     — and the form has no back control, so there was no way out of it either. v5.2's last second
     dissolves into the splash instead, and this navigate now matches the frame the film ends on,
     which is the whole point of crossfading rather than cutting.

     `/yourworld` is the existing route for that screen — `OneWorldEntry`, the same `Splash` the
     eight products use, whose own buttons go to `/join` and `/signin`. No new route, no copy of
     the screen written for the film, and no auto-forward (that route mounts it bare).
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      navigate("/yourworld", { replace: true });
