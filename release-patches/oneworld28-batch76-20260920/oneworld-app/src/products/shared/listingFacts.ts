...
export function listingFacts(kindLabel: string | null | undefined, l: FactSource, lang: string): string[] {
  return [
    ...
    l.bathrooms != null ? `${l.bathrooms} ${W(lang, "bath", l.bathrooms === 1 ? "baño" : "baños")}` : null,
    l.area_m2 != null ? `${l.area_m2} m²` : null,
    /* ⚠️ THIS WAS `Estrato N` AND ALMOST NOBODY READING THE FEED KNOWS WHAT THAT MEANS.
       Lee, 16 Sep 2026: *"A lot of people aren't going to know what Estrato One is. That's the
       problem... I guess you could put whether it's furnished or not. That's a pretty key
       metric."* He is right on both halves. Estrato is a Colombian utility-subsidy bracket — it
       is meaningful to a Colombian and opaque to everybody else, and the feed is the one place
       where a term you have to look up costs you the reader.

       Furnished is the opposite: it changes whether somebody can even consider the place, it
       needs no explanation in any country, and it is a column we already have and already
       select. Estrato is still on the listing page, where there is room to be specific. */
    l.furnished == null ? null : (l.furnished ? W(lang, "Furnished", "Amoblado") : W(lang, "Unfurnished", "Sin amoblar")),
