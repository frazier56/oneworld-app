...
/* ── THREE MORE, 20 Sep 2026 — the media stack moved into the shell (OneEvent 30) ─────────────
   `PublicVideoDeck`, `FeedVideoPlayer` and `PublicVideoViewer` came from OneHome drawn with
   lucide-react, which is banned in this package. Same stroke weight as everything above. */
export const IconTrash = ({ size = 18, className }: P) =>
  wrap(size, className, <><path d="M4 7h16M9.5 7V4.6h5V7M6.2 7l.8 12.2A1.6 1.6 0 0 0 8.6 21h6.8a1.6 1.6 0 0 0 1.6-1.8L17.8 7" /><path d="M10 11v6M14 11v6" /></>);

export const IconUpload = ({ size = 18, className }: P) =>
  wrap(size, className, <><path d="M12 16V4M7.5 8.5 12 4l4.5 4.5" /><path d="M4 15.5v2.9A1.6 1.6 0 0 0 5.6 20h12.8a1.6 1.6 0 0 0 1.6-1.6v-2.9" /></>);

export const IconX = ({ size = 18, className }: P) =>
  wrap(size, className, <path d="M6 6l12 12M18 6 6 18" />);
