  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-0.5 grid grid-cols-7 gap-0.5 text-center text-[10px]">
        {cells.map((d, i) => (
          <span key={i} className={`relative grid h-5 place-items-center rounded ${d === today ? "bg-brand-deep font-bold text-white" : ""}`}>
            {d ?? ""}
            {d && (jobDays.includes(d) || eventDays.includes(d)) && (
              <span className={`absolute bottom-0 h-1 w-1 rounded-full ${jobDays.includes(d) ? "bg-brand" : "bg-amber-400"} ${d === today ? "bg-white" : ""}`} />
            )}
          </span>
        ))}
      </div>
